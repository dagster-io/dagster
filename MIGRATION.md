...
- By default, resources defined on `Definitions` are now automatically bound to jobs. This will result in a change in behavior if you a) have a job with no "io_manager" defined in its `resource_defs` and b) have supplied an "io_manager" to the `resource_defs` of your Definitions. Prior to 1.3.0, this would result in the job using the default filesystem-based io manager for outputs using the default io_manager key. In 1.3.0, this will result in the "io_manager" supplied to your Definitions being used in these cases. The `BindResourcesToJobs` wrapper, introduced in 1.2 to simulate this behavior, no longer has any effect.
