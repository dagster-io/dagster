...
class PDFTextExtractor(dg.ConfigurableResource):
    ...
    def preprocess_image(self, image_path: str, save_preprocessed: bool = True) -> tuple:
        ...
        if save_preprocessed:
            ...
            preprocessed_path = os.path.join(
                image_dir, 
                f"preprocessed_{image_filename}"
            )
    ...
    def convert_pdf_to_images(self, pdf_path: str, output_folder: str = None, pages: List[int] = None) -> Dict[str, Any]:
        """
    ...
    def extract_text_from_images(self, conversion_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        self.logger.info(f"Extracting text using OCR")
        ...
        for i, image_path in enumerate(image_paths):
            ...
            try:
                ...
                logger.error(f"OCR error on page {page_number}: {str(e)}")
                results.append({
                    "page": page_number,
                    "text": "",
                    "error": str(e),
    ...
    def validate_with_openai(self, extraction_result: Dict[str, Any], 
                           context: str = "", expected_info: List[str] = None) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(f"Validating extraction with OpenAI")
        ...
        # Read the full text
        with open(extraction_result["text_file"], 'r', encoding='utf-8') as f:
            all_text = f.read()
        ...
        try:
            ...
            logger.error(f"Error validating with OpenAI: {str(e)}")
            error_result = {
                "error": str(e),
                "file": extraction_result["file"],
                "validation_performed": False
...
class PdfExtraction(Component, Resolvable):
    ...
    def build_defs(self, context: ComponentLoadContext) -> dg.Definitions:
        ...
        pdf_extractor_resource = PDFTextExtractor(
            language="eng",
            dpi=300,
            openai_api_key=dg.EnvVar("OPEN_AI_API_KEY")
        )

        @dg.asset(name=f"{Path(self.pdf_path).stem}_convert_to_image",
                  specs=self.asset_specs)
        def convert_to_image(context: dg.AssetExecutionContext, pdf_extractor_resource):
            pdf_path = self.pdf_path

            image_paths, number_of_pages = pdf_extractor_resource.convert_pdf_to_images(pdf_path)
            return image_paths, number_of_pages


        @dg.asset(name=f"{Path(self.pdf_path).stem}_extract_text", 
                  specs=self.asset_specs,
                  deps=[f"{Path(self.pdf_path).stem}_convert_to_image"])
        def extract_text(context: dg.AssetExecutionContext, pdf_extractor_resource):
        ...
        return dg.Definitions(assets=[convert_to_image,
                                       extract_text], resources=pdf_extractor_resource())
