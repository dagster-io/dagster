...
from docs_snippets.guides.build.ops_jobs_graphs.graph_provides_config import (
    celsius_to_fahrenheit,
)
from docs_snippets.guides.build.ops_jobs_graphs.graph_provides_config_mapping import (
    to_fahrenheit,
)
from docs_snippets.guides.build.ops_jobs_graphs.nested_graphs import (
    all_together_nested,
    subgraph_config_job,
    subgraph_multiple_outputs_job,
)
from docs_snippets.guides.build.ops_jobs_graphs.unnested_ops import (
...
def test_composite_config():
    with open(
        file_relative_path(
            __file__,
            "../../../../docs_snippets/guides/build/ops_jobs_graphs/composite_config.yaml",
        ),
        encoding="utf8",
    ) as fd:
...
def test_config_mapping():
    ...
    with open(
        file_relative_path(
            __file__,
            "../../../../docs_snippets/guides/build/ops_jobs_graphs/composite_config_mapping.yaml",
        ),
        encoding="utf8",
    ) as fd:
        run_config = yaml.safe_load(fd.read())
