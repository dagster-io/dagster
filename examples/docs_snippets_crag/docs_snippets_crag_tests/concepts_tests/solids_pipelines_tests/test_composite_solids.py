...
from docs_snippets_crag.concepts.solids_pipelines.composite_solids import (
    all_together_nested,
    subgraph_config_job,
    subgraph_multiple_outputs_job,
)
from docs_snippets_crag.concepts.solids_pipelines.graph_provides_config import celsius_to_fahrenheit
from docs_snippets_crag.concepts.solids_pipelines.graph_provides_config_mapping import to_fahrenheit
from docs_snippets_crag.concepts.solids_pipelines.unnested_ops import (
...
def test_composite_config():
    with open(
        file_relative_path(
            __file__, "../../../docs_snippets_crag/concepts/solids_pipelines/composite_config.yaml"
        ),
        "r",
    ) as fd:
...
def test_config_mapping():
    ...
    with open(
        file_relative_path(
            __file__,
            "../../../docs_snippets_crag/concepts/solids_pipelines/composite_config_mapping.yaml",
        ),
        "r",
    ) as fd:
        run_config = yaml.safe_load(fd.read())
