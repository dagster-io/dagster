...
@job_failure_sensor
def my_slack_on_job_failure(context: JobFailureSensorContext):
...
# start_slack_marker
from dagster_slack import make_slack_on_job_failure_sensor

slack_on_job_failure = make_slack_on_job_failure_sensor("#my_channel", os.getenv("MY_SLACK_TOKEN"))
...
# start_email_marker
from dagster.utils import make_email_on_job_failure_sensor
...
def my_slack_on_job_success(context: RunStatusSensorContext):
    slack_client = WebClient(token=os.environ["SLACK_DAGSTER_ETL_BOT_TOKEN"])
...
@repository
def my_repository():
    return my_pipelines + [my_slack_on_job_success]
