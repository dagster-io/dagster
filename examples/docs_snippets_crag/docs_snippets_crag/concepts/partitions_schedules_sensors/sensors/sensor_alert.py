...
def my_slack_on_pipeline_failure(context: PipelineFailureSensorContext):
    ...
    slack_client.chat_postMessage(
        ...
        message=f'Pipeline "{context.dagster_run.target.name}" failed. Error: {context.failure_event.message}',
    )
...
def my_slack_on_pipeline_success(context: RunStatusSensorContext):
    ...
    slack_client.chat_postMessage(
        channel="#alert-channel",
        message=f'Pipeline "{context.dagster_run.target.name}" succeeded.',
    )
