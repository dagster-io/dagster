...
# IAM role for Dagster ECS tasks
# with access to S3, ECS and SecretsManager

data "aws_s3_bucket" "storage" {
  bucket = var.s3_bucket
}

# generate a unique 8 letter string for the role name
    ...
    Statement = [
      # full access to specific s3 bucket
      {
        ...
        Resource = [
          ...
          "${data.aws_s3_bucket.storage.arn}/*" # Add object-level access
        ]
      },
