...
def parse_pagerank_data_final(context, pagerank_data):
...
def compute_links_final(_context, urls):
...
def define_pipeline():
    return PipelineDefinition(
        name='pyspark_pagerank_example',
        context_definitions={
            'local': PipelineContextDefinition(resources={'spark': spark_session_resource})
        },
        solids=[parse_pagerank_data_final, compute_links_final, calculate_ranks, log_ranks],
        dependencies={
            'compute_links_final': {'urls': DependencyDefinition('parse_pagerank_data_final')},
            'calculate_ranks': {'links': DependencyDefinition('compute_links_final')},
