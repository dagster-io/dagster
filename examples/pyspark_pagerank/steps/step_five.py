...
@solid(
    inputs=[InputDefinition('urls', SparkRDD)],
    outputs=[OutputDefinition(SparkRDD)],
)
...
@solid(
    inputs=[InputDefinition(name='ranks', dagster_type=SparkRDD)], outputs=[]
)
...
def define_pipeline():
    return PipelineDefinition(
        ...
        dependencies={
            'compute_links': {
                'urls': DependencyDefinition('parse_pagerank_data')
            },
            'calculate_ranks': {
                'links': DependencyDefinition('compute_links')
            },
            'log_ranks': {
                'ranks': DependencyDefinition('calculate_ranks', 'ranks')
            },
        },
    )
