...
@dg.asset_check(asset=shipments, description="Validate shipments schema against data contract")
def validate_shipments_data_contract(
    ...
    schema_metadata = (
        latest_materialization.dagster_event.event_specific_data.materialization.metadata.get(
    ...
    passed = len(mismatches) == 0 and len(missing_columns) == 0 and len(extra_columns) == 0
