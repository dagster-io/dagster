...
def test_dag_peering(
    ...
    dag_spec: AssetSpec = [  # noqa
        assets_def
        for assets_def in assets_defs
        if assets_def.key == AssetKey(["airflow_instance", "dag", "print_dag__successful_run"])  # type: ignore
    ][0]
    assert dag_spec.key == AssetKey(["airflow_instance", "dag", "print_dag__successful_run"])
    spec_metadata = dag_spec.metadata
