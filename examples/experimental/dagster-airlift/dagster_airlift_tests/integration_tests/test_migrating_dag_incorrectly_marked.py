...
def test_migrating_dag(airflow_instance: None) -> None:
    ...
    response = requests.get("http://localhost:8080/api/v1/importErrors", auth=("admin", "admin"))
    assert response.status_code == 200
    import_errors = response.json()["import_errors"]
    assert len(import_errors) == 1
    assert "marked.py" in import_errors[0]["filename"]
    assert "No dags were marked as migrating" in import_errors[0]["stack_trace"]
