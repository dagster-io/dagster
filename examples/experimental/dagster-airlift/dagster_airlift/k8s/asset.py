...
def k8s_pod_defs(
    ...
    Many arguments are supported indirectly via the pod_spec_config argument.
    ...
    The following arguments are supported under the nested ['containers'] key of the pod_spec_config argument:
    ...
    def _k8s_multi_asset(context: AssetExecutionContext):
        return (
            PipesK8sClient(
                load_incluster_config=in_cluster,
                kubeconfig_file=config_file,
                kube_context=cluster_context,
            )
            .run(
                context=context.op_execution_context,
                base_pod_meta=pod_metadata,
                base_pod_spec=pod_spec_config,
            )
            .get_results()
        )
