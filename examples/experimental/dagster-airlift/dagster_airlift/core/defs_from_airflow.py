...
def create_defs_from_airflow_instance(
    ...
    specs_by_dag_and_task_id: Dict[str, Dict[str, List[AssetSpec]]] = defaultdict(
        ...
        for asset in orchestrated_defs.assets:
            ...
            if (
                ...
                for spec in assets_def.specs:
                    ...
                    all_asset_keys_per_dag_id[task_info.dag_id].add(new_spec.key)
                    for dep in new_spec.deps:
                        downstreams_asset_dependency_graph[dep.asset_key].add(new_spec.key)
                        upstreams_asset_dependency_graph[spec.key].add(dep.asset_key)
                    specs_by_dag_and_task_id[task_info.dag_id][task_info.task_id].append(new_spec)
...
def build_airflow_polling_sensor(
    ...
    peered_task_specs: Dict[str, Dict[str, List[AssetSpec]]],
    ...
    def airflow_dag_sensor(context: SensorEvaluationContext) -> SensorResult:
        ...
        for dag_id, dag_spec in peered_dag_assets_defs.items():
            ...
            task_keys = {spec.key for _, spec_list in task_specs.items() for spec in spec_list}
            topo_order_keys = [
                *toposort_task_keys(task_keys, upstreams_asset_dependency_graph),
                dag_spec.key,
            ]
...
```
        ...
        spec = AssetSpec(
            ...
            deps=[AssetDep(asset=key) for key in leaf_keys_per_dag_id.get(dag_info.dag_id, [])],
        )
...
def toposort_task_keys(
    ...
    narrowed_asset_dependency_graph = {
        key: {dep for dep in upstreams_asset_dependency_graph[key] if dep in task_asset_keys}
        for key in task_asset_keys
    }
