...
class AirflowCacheableAssetsDefinition(CacheableAssetsDefinition):
    ...
    def build_definitions(
        ...
        metadata: Mapping[str, Any] = check.not_none(
            ...
            "Expected cacheable data to have extra_metadata field set.",
        )
        ...
        for dag_spec in cacheable_specs.dag_asset_specs.values():
            ...
            new_assets_defs.append(
                build_airflow_asset_from_specs(
                    specs=[with_tags(dag_spec.to_asset_spec({}), {DAG_ID_TAG: dag_id})],
...
def with_tags(asset_spec: AssetSpec, tags: Dict[str, str]) -> AssetSpec:
    return asset_spec._replace(tags={**asset_spec.tags, **tags})
