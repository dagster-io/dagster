...
from ingestion.github_gql_queries import GITHUB_DISCUSSIONS_QUERY, GITHUB_ISSUES_QUERY
...
class GithubResource(ConfigurableResource):
    ...
    def _fetch_results(self, query_str: str, object_type: str) -> List[dict]:
        ...
        while True:
            ...
            #print("Fetching results from Github: {object_type} with cursor: {cursor}")
            query = gql.gql(
    ...
    def _convert_discussions_to_documents(self, items: List[Dict[str, Any]]) -> List[Document]:
        ...
        """
        documents = []
        print(f"Starting conversion of {len(items)} discussions to documents")
        ...
        for item in items:
            try:
                ...
                print(f"Processed discussion #{item.get('number')}: "
        ...
        print(f"Successfully converted {len(documents)} discussions")
        return documents
    ...
    def _convert_issues_to_documents(self, items: List[Dict[str, Any]]) -> List[Document]:
        ...
        """
        documents = []
        print(f"Starting conversion of {len(items)} issues to documents")
        ...
        for item in items:
            try:
                ...
                # Debug information for verification
                print(f"Processed issue #{item.get('number')}: {item.get('title')} ({metadata['state']})")
        ...
        print(f"Successfully converted {len(documents)} issues")
        return documents


github_resource = GithubResource(github_token=os.getenv("GITHUB_TOKEN"))