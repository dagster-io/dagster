...
class DuckDBCSVIOManager(IOManager):
    ...
    def handle_output(self, context, obj):
        if obj is not None:  # if this is a dbt output, then the value will be None
            ...
            con.execute(f"create schema if not exists {context.asset_key.path[-2]};")
            con.execute(
    ...
    def _table_path(self, context):
        # TODO: override if schema is in metadata
        metadata_schema = context.metadata.get("schema")
        metadata_tablename = context.metadata.get("table")
