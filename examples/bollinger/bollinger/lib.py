...
def compute_bollinger_bands_multi(df: pd.DataFrame, dropna: bool = True, rate: int = 30, sigma: float = 2.0):
    ...
    odf = df.groupby("name").apply(lambda idf: compute_bollinger_bands(idf, dropna=False, rate=rate, sigma=sigma))
