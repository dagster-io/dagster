...
class LakehouseComponent(Component, LakehouseComponentParams, Resolvable):
    def build_defs(self, context) -> Definitions:
        ...
        def silver_sensor_data(
            ...
            df_clean = df_clean.drop_duplicates(subset=["sensor_id", "timestamp"])
            df_clean = df_clean[
                (df_clean["temperature"] >= -50) & (df_clean["temperature"] <= 100)
            ]
            df_clean = df_clean[
        ...
        def gold_sensor_summary(
            ...
            context.log.info(
                f"Aggregated to gold layer: {table_path} ({len(df_agg)} summaries)"
            )
            context.add_output_metadata(
                {
