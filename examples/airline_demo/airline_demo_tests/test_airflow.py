...
class TestAirflowPython_0IngestExecution(object):
    ...
    environment_yaml = [
        script_relative_path(os.path.join('..', 'airline_demo', 'environments', 'local_base.yaml')),
        script_relative_path(os.path.join('..', 'airline_demo', 'environments', 's3_storage.yaml')),
        script_relative_path(
            os.path.join('..', 'airline_demo', 'environments', 'local_fast_ingest.yaml',)
        ),
    ]
    mode = 'local'
...
class TestAirflowPython_1WarehouseExecution(object):
    ...
    environment_yaml = [
        ...
        script_relative_path(os.path.join('..', 'airline_demo', 'environments', 's3_storage.yaml')),
        script_relative_path(
            os.path.join('..', 'airline_demo', 'environments', 'local_warehouse.yaml',)
        ),
    ]
    mode = 'local'
