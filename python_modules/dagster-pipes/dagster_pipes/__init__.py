...
class PipesProcessContextData(TypedDict):
...
def _assert_single_asset(data: PipesProcessContextData, key: str) -> None:
...
def _assert_single_partition(data: PipesProcessContextData, key: str) -> None:
...
class PipesProcessContextLoader(ABC):
    @abstractmethod
    @contextmanager
    def load_context(self, params: PipesParams) -> Iterator[PipesProcessContextData]: ...
...
class DefaultPipesProcessContextLoader(PipesProcessContextLoader):
    FILE_PATH_KEY = "path"
    DIRECT_KEY = "data"
    ...
    def load_context(self, params: PipesParams) -> Iterator[PipesProcessContextData]:
        if self.FILE_PATH_KEY in params:
            ...
            yield cast(PipesProcessContextData, data)
        else:
            raise DagsterPipesError(
...
class DbfsPipesProcessContextLoader(PipesProcessContextLoader):
    @contextmanager
    def load_context(self, params: PipesParams) -> Iterator[PipesProcessContextData]:
...
def init_dagster_pipes_process(
    *,
    context_loader: Optional[PipesProcessContextLoader] = None,
    message_writer: Optional[ExtMessageWriter] = None,
    params_loader: Optional[PipesParamsLoader] = None,
) -> "PipesProcessContext":
    if PipesProcessContext.is_initialized():
        return PipesProcessContext.get()
    ...
    if is_dagster_pipes_process():
        ...
        context_loader = context_loader or DefaultPipesProcessContextLoader()
        ...
        context = PipesProcessContext(context_data, message_channel)
    else:
        _emit_orchestration_inactive_warning()
        context = _get_mock()
    PipesProcessContext.set(context)
    return context


class PipesProcessContext:
    _instance: ClassVar[Optional["PipesProcessContext"]] = None
    ...
    @classmethod
    def set(cls, context: "PipesProcessContext") -> None:
        cls._instance = context

    @classmethod
    def get(cls) -> "PipesProcessContext":
