...
def test_basic_preset_query_no_presets(graphql_context):
...
def test_basic_preset_query_with_presets(graphql_context, snapshot):
    result = execute_preset_query('csv_hello_world', graphql_context)

    assert [preset_data['name'] for preset_data in result.data['pipelineOrError']['presets']] == [
        'prod',
        'test',
        'test_inline',
    ]
