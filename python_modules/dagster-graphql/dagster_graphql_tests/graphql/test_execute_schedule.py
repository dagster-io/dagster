...
def test_just_basic_start_scheduled_execution(graphql_context):
    ...
    assert result.data['startScheduledExecution']['run']['pipeline']['name'] == 'no_config_pipeline'
...
def test_basic_start_scheduled_execution_with_run_launcher(graphql_context):
    result = execute_dagster_graphql(
        graphql_context,
        START_SCHEDULED_EXECUTION_QUERY,
        variables={'scheduleName': 'no_config_pipeline_hourly_schedule'},
...
def test_basic_start_scheduled_execution_with_environment_dict_fn(graphql_context):
    result = execute_dagster_graphql(
        graphql_context,
        START_SCHEDULED_EXECUTION_QUERY,
...
def test_start_scheduled_execution_with_should_execute(graphql_context):
    result = execute_dagster_graphql(
        graphql_context,
        START_SCHEDULED_EXECUTION_QUERY,
...
def test_partition_based_execution(graphql_context):
    result = execute_dagster_graphql(
        graphql_context,
        START_SCHEDULED_EXECUTION_QUERY,
    ...
    assert any(
        tag['key'] == 'dagster/schedule_name' and tag['value'] == 'partition_based' for tag in tags
    )
...
def test_partition_based_custom_selector(graphql_context):
    result = execute_dagster_graphql(
        graphql_context,
        START_SCHEDULED_EXECUTION_QUERY,
    ...
    tags = result.data['startScheduledExecution']['run']['tags']
    assert any(
        tag['key'] == 'dagster/schedule_name' and tag['value'] == 'partition_based_custom_selector'
        for tag in tags
    )
    assert any(tag['key'] == 'dagster/partition' and tag['value'] == '9' for tag in tags)
    assert any(
...
def test_partition_based_decorator(graphql_context):
    result = execute_dagster_graphql(
        graphql_context,
        START_SCHEDULED_EXECUTION_QUERY,
...
def test_solid_subset_schedule_decorator(schedule_name, graphql_context):
    result = execute_dagster_graphql(
        graphql_context, START_SCHEDULED_EXECUTION_QUERY, variables={'scheduleName': schedule_name},
    )
...
def test_partition_based_multi_mode_decorator(graphql_context):
    result = execute_dagster_graphql(
        graphql_context,
        START_SCHEDULED_EXECUTION_QUERY,
...
def test_tick_success(graphql_context, snapshot):
...
def test_tick_skip(graphql_context, snapshot):
    ...
    # Check tick data and stats through gql
    result = execute_dagster_graphql(graphql_context, SCHEDULE_TICKS_QUERY)
    schedule_result = next(
        x
...
def test_should_execute_scheduler_error(graphql_context, snapshot):
    instance = graphql_context.instance
    repository = graphql_context.legacy_get_repository_definition()
    reconcile_scheduler_state("", "", repository, instance)
...
def print_stack(root_field):
    for i, line in enumerate(root_field['stack']):
        print('Line {}: {}'.format(i, line))


def test_tags_scheduler_error(graphql_context, snapshot):
    instance = graphql_context.instance
    repository = graphql_context.legacy_get_repository_definition()
    reconcile_scheduler_state("", "", repository, instance)
...
def test_environment_dict_scheduler_error(graphql_context, snapshot):
    instance = graphql_context.instance
    repository = graphql_context.legacy_get_repository_definition()
    reconcile_scheduler_state("", "", repository, instance)
...
def test_environment_dict_scheduler_error_serialize_cause(graphql_context):
    instance = graphql_context.instance
    repository = graphql_context.legacy_get_repository_definition()
    reconcile_scheduler_state("", "", repository, instance)
...
def test_query_multiple_schedule_ticks(graphql_context, snapshot):
    instance = graphql_context.instance
    repository = graphql_context.legacy_get_repository_definition()
    reconcile_scheduler_state("", "", repository, instance)
...
def test_tagged_pipeline_schedule(graphql_context):
    result = execute_dagster_graphql(
        graphql_context,
        START_SCHEDULED_EXECUTION_QUERY,
    ...
    assert not result.errors
    assert result.data['startScheduledExecution']['__typename'] == 'StartPipelineRunSuccess'
    assert result.data['startScheduledExecution']['run']['pipeline']['name'] == 'tagged_pipeline'
...
def test_tagged_pipeline_override_schedule(graphql_context):
    result = execute_dagster_graphql(
        graphql_context,
        START_SCHEDULED_EXECUTION_QUERY,
...
def test_tagged_pipeline_scheduled_execution_with_run_launcher(graphql_context):
    result = execute_dagster_graphql(
        graphql_context,
        START_SCHEDULED_EXECUTION_QUERY,
        variables={'scheduleName': 'tagged_pipeline_schedule'},
...
def test_invalid_config_schedule_error(graphql_context, snapshot):
    repository = graphql_context.legacy_get_repository_definition()
    instance = graphql_context.instance
    reconcile_scheduler_state("", "", repository, instance)
    result = execute_dagster_graphql(
    ...
    assert result.data['startScheduledExecution']['__typename'] == 'PipelineConfigValidationInvalid'
