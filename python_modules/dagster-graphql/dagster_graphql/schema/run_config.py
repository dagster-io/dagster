...
class GrapheneRunConfigSchema(graphene.ObjectType):
    ...
    def resolve_isRunConfigValid(
        self, graphene_info: ResolveInfo, runConfigData: Optional[GrapheneRunConfigData] = None
    ):
        return resolve_is_run_config_valid(
            graphene_info,
            self._represented_pipeline,
            self._mode,
            parse_run_config_input(runConfigData or {}, raise_on_error=False),  # type: ignore  # fmt: skip
        )
