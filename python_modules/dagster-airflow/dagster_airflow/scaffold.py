...
def _make_editable_scaffold(pipeline_name, pipeline_description, env_config, static_scaffold, default_args):

    pipeline_description = '***Autogenerated by dagster-airflow***' + ('''

    {pipeline_description}
    '''.format(pipeline_description=pipeline_description) if pipeline_description else '')
    ...
    with IndentingBlockPrinter() as printer:
        ...
        printer.line('DAG_DESCRIPTION = \'\'\'{pipeline_description}\'\'\''.format(
...
def _make_static_scaffold(pipeline_name, env_config, execution_plan, image, editable_scaffold):
    with IndentingBlockPrinter() as printer:
        ...
        printer.line('def make_dag(dag_id, dag_description, dag_kwargs, s3_conn_id, modified_docker_operator_kwargs):')
        with printer.with_indent():
...
def scaffold_airflow_dag(
    pipeline, env_config, image, output_path=None, dag_kwargs=None
):
    ...
    if output_path is None:
        static_path = os.path.join(
            os.getcwd(),
            '{pipeline_name}_static__scaffold.py'.format(pipeline_name=pipeline_name)
        )
        editable_path = os.path.join(
            os.getcwd(),
            '{pipeline_name}_editable__scaffold.py'.format(pipeline_name=pipeline_name)
        )
    ...
    default_args = dict(
        dict(
            DEFAULT_ARGS, start_date=datetime.datetime.utcnow()
