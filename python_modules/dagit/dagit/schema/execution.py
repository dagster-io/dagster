...
class ExecutionPlan(graphene.ObjectType):
    ...
    def __init__(self, pipeline, execution_plan):
        ...
        self.pipeline = check.inst_param(pipeline, 'pipeline', pipeline.Pipeline)
...
class ExecutionStepOutput(graphene.ObjectType):
    ...
    type = graphene.Field(graphene.NonNull(lambda: pipeline.Type))
    ...
    def resolve_type(self, _info):
        return pipeline.Type.from_dagster_type(dagster_type=self.execution_step_output.dagster_type)
...
class ExecutionStepInput(graphene.ObjectType):
    ...
    def resolve_type(self, _info):
        return pipeline.Type.from_dagster_type(dagster_type=self.execution_step_input.dagster_type)
...
class ExecutionStep(graphene.ObjectType):
    ...
    solid = graphene.NonNull(lambda: pipeline.Solid)
    ...
    def resolve_solid(self, _info):
        return pipeline.Solid(self.execution_step.solid)
