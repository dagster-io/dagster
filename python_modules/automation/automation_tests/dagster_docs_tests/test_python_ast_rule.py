...
class TestIntegrationWithValidationPipeline:
    ...
    def test_integration_multiple_validation_rules(self):
        ...
        has_section_issue = any("malformed section header" in msg.lower() or 
                               "possible malformed section header" in msg.lower() 
                               for msg in all_messages)
        has_python_issue = any("python code block syntax error" in msg.lower() 
                              for msg in all_messages)
...
{indented_code}
    ...
    @pytest.mark.parametrize("invalid_syntax", [
