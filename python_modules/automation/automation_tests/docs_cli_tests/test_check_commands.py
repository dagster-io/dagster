...
class TestCheckDocstringsCommands:
    ...
    def test_check_docstrings_symbol_automation_validator(self):
        ...
        result = self.runner.invoke(
            check, ["docstrings", "--symbol", "automation.docstring_lint.DocstringValidator"]
        )

        # Should complete successfully
        assert result.exit_code == 0
        assert (
            "Validating docstring for: automation.docstring_lint.DocstringValidator"
            in result.output
        )
    ...
    def test_check_docstrings_package_automation(self):
        """Test validating all docstrings in automation.docstring_lint package."""
        result = self.runner.invoke(check, ["docstrings", "--package", "automation.docstring_lint"])
