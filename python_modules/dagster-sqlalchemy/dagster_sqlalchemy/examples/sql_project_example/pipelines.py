...
from dagster import DependencyDefinition, InputDefinition
...
def define_full_pipeline():
    ...
    insert_into_sum_table_solid = _get_project_solid(
        'insert_into_sum_table', inputs=[InputDefinition(populate_num_table_solid.name)]
    )
...
def define_truncate_pipeline():
    ...
    return dagster.PipelineDefinition(
        ...
        description='Truncates all tables that are populated by the pipeline. Preserves source tables',
        solids=[truncate_solid],
    )


def define_rerun_pipeline():
    insert_into_sum_table_solid = _get_project_solid('insert_into_sum_table', inputs=None)
    ...
    return dagster.PipelineDefinition(
        ...
        dependencies={
            insert_into_sum_sq_table_solid.name: {
                insert_into_sum_table_solid.name: DependencyDefinition(
                    insert_into_sum_table_solid.name
                )
            }
        },
