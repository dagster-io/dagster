...
def test_materialization_of_source_asset(executor):
    """Asset graph:
        a_source_asset (asset in other repository)
        |
        depends_on_source
    Sensor for depends_on_source
    Tests that when a_source_asset is materialized in the other repo, that depends_on_source is materialized
        by the sensor
    """
    freeze_datetime = to_timezone(
        create_pendulum_time(year=2019, month=2, day=27, tz="UTC"),
    ...
    with instance_with_multiple_repos_with_sensors() as (
        ...
        with pendulum.test(freeze_datetime):
            the_sensor = asset_sensor_repo.get_external_sensor("source_asset_sensor")
            instance.start_sensor(the_sensor)
