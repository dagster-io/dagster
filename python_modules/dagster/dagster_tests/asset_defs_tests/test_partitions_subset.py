...
def test_all_partitions_subset_static_partitions_def() -> None:
    ...
    round_trip_subset = dg.deserialize_value(
        dg.serialize_value(all_subset.to_serializable_subset())  # type: ignore
    )
...
def test_all_partitions_subset_time_window_partitions_def() -> None:
    with freeze_time(create_datetime(2020, 1, 6, hour=10)):
        time_window_partitions_def = dg.DailyPartitionsDefinition(start_date="2020-01-01")
