from dagster._core.workspace.load_target import (
    get_origins_from_toml,
    is_valid_modules_list,
)
...
def test_load_multiple_modules_from_toml():
    origins = get_origins_from_toml(
        file_relative_path(__file__, "multiple_modules.toml")
    )
...
def test_load_mixed_modules_and_module_name_from_toml():
    with raises(
        ...
        get_origins_from_toml(
            file_relative_path(__file__, "mixed_modules_and_module_name.toml")
        )
...
def test_load_invalid_empty_modules_from_toml():
    with raises(ValueError, match="'modules' list should not be empty if specified."):
        get_origins_from_toml(
            file_relative_path(__file__, "invalid_empty_modules.toml")
        )
...
def test_is_valid_modules_list_from_toml():
    ...
    with raises(
        ValueError, match="Dictionary at index 0 does not contain the key 'type'."
    ):
...
def test_is_valid_modules_list_type_not_string():
    ...
    with raises(
        ValueError, match="The 'type' value in dictionary at index 0 is not a string."
    ):
        is_valid_modules_list(modules)
...
def test_is_valid_modules_list_missing_name():
    modules = [{"type": "module"}]
    with raises(
        ValueError, match="Dictionary at index 0 does not contain the key 'name'."
    ):
        is_valid_modules_list(modules)
...
def test_is_valid_modules_list_name_not_string():
    ...
    with raises(
        ValueError, match="The 'name' value in dictionary at index 0 is not a string."
    ):
        is_valid_modules_list(modules)
