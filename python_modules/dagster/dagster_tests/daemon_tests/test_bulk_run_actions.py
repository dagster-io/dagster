...
from dagster.core.execution.bulk_actions import BulkActionStatus, BulkActionType, BulkRunAction
...
def test_bulk_action():
    with instance_for_test(
        overrides={
            "run_launcher": {
                "module": "dagster_tests.daemon_tests.test_bulk_run_actions",
                "class": "TestRunLauncher",
            }
        }
    ) as instance:
        list(
            execute_bulk_run_action_iteration(
                instance, get_default_daemon_logger("BulkActionsDaemon")
            )
        )
        instance.run_storage.add_bulk_run_action(
            BulkRunAction(
        ...
        assert instance.run_storage.get_bulk_run_action("foo").status == BulkActionStatus.COMPLETED
        starting_runs = [
            create_run_for_test(
        ...
        assert instance.run_storage.get_bulk_run_action("bar").status == BulkActionStatus.COMPLETED
        assert all(
