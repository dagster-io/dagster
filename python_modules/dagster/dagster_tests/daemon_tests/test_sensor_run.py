...
def test_custom_interval_sensor(external_repo_context):
    ...
    with instance_with_sensors(external_repo_context) as (
        ...
        with pendulum.test(freeze_datetime):
            ...
            ticks = instance.get_job_ticks(external_sensor.get_external_origin_id())
            assert len(ticks) == 1
            validate_tick(ticks[0], external_sensor, freeze_datetime, InstigationTickStatus.SKIPPED)
        ...
        with pendulum.test(freeze_datetime):
            ...
            validate_tick(
                ticks[0], external_sensor, expected_datetime, InstigationTickStatus.SKIPPED
            )
