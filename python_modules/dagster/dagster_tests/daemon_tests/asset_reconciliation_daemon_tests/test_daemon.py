# from dagster._daemon.asset_reconciliation_daemon import AssetReconciliationDaemon


# def test_daemon(workspace_context):
#     list(AssetReconciliationDaemon(interval_seconds=10).run_iteration(workspace_context))
...
import itertools
import os
from typing import Iterable, List, Mapping, NamedTuple, Optional, Sequence, Set, Tuple, Union
...
from dagster._daemon.asset_reconciliation_daemon import (
    AssetReconciliationCursor,
    execute_reconciliation_loop,
)
...
class AssetReconciliationScenario(NamedTuple):
    ...
    def do_scenario(self, instance, name) -> Tuple[List[DagsterRun], AssetReconciliationCursor]:
        instance.run_launcher.queue().clear()
        ...
        with pendulum.test(test_time) if self.current_time else contextlib.nullcontext():
            ...
            if self.cursor_from is not None:
                _, cursor = self.cursor_from.do_scenario(instance, name)
                instance.run_launcher.queue().clear()
        ...
        with pendulum.test(test_time):
            ...
            def create_workspace_load_target(attribute: Optional[str] = "the_repo"):
                # return ModuleTarget(
                #     module_name=(
                #         "dagster_tests.daemon_tests.asset_reconciliation_daemon_tests.test_daemon"
                #     ),
                #     attribute="johann_repo",
                #     working_directory=os.path.dirname(__file__),
                #     location_name="test_location",
                # )
                globals()["defs"] = Definitions(assets=self.assets)
        ...
        return instance.run_launcher.queue(), new_cursor
...
# scenarios = {
#     "one_asset_never_materialized": scenarios["one_asset_never_materialized"],
# }
...
for scenario_name, scenario in scenarios.items():
    ...
    globals()[scenario_name] = d.get_repository_def()
...
@pytest.mark.parametrize("scenario_item", list(scenarios.items()), ids=list(scenarios.keys()))
def test_reconciliation(scenario_item, instance):
    scenario_name, scenario = scenario_item

    if scenario_name == "diamond_never_materialized":
        print("hi")

    runs, _ = scenario.do_scenario(instance, scenario_name)

    assert len(runs) == len(scenario.expected_run_requests)

    def sort_runs_fn(run: DagsterRun):
        return (min(check.not_none(run.asset_selection)), run.tags.get(PARTITION_NAME_TAG))

    def sort_run_request_key_fn(run_request):
        return (min(run_request.asset_selection), run_request.partition_key)

    sorted_runs = sorted(runs, key=sort_runs_fn)
