...
get_unique_asset_identifier = lambda asset: (
    asset.node_def.name if isinstance(dg.asset, dg.AssetsDefinition) else asset.key
)
...
def test_load_assets_from_package_name():
    ...
    assets_1 = [get_unique_asset_identifier(dg.asset) for dg.asset in assets_defs]

    assets_defs_2 = dg.load_assets_from_package_name(asset_package.__name__)
    assert len(assets_defs_2) == 11

    assets_2 = [get_unique_asset_identifier(dg.asset) for dg.asset in assets_defs]
...
def test_load_assets_from_package_module():
    ...
    assets_1 = [get_unique_asset_identifier(dg.asset) for dg.asset in assets_1]

    assets_2 = dg.load_assets_from_package_module(asset_package)
    assert len(assets_2) == 11

    assets_2 = [get_unique_asset_identifier(dg.asset) for dg.asset in assets_2]
...
def test_load_assets_from_modules(monkeypatch):
    ...
    assets_1 = [get_unique_asset_identifier(dg.asset) for dg.asset in collection_1]

    collection_2 = dg.load_assets_from_modules([asset_package, module_with_assets])

    assets_2 = [get_unique_asset_identifier(dg.asset) for dg.asset in collection_2]
    ...
    with monkeypatch.context() as m:
        ...
        assert [get_unique_asset_identifier(dg.asset) for dg.asset in assets] == assets_1
        with pytest.raises(
            dg.DagsterInvalidDefinitionError,
        ):
            dg.load_assets_from_modules([asset_package, module_with_assets], include_specs=True)
...
def test_load_assets_from_current_module():
    ...
    assets = [get_unique_asset_identifier(dg.asset) for dg.asset in assets]
