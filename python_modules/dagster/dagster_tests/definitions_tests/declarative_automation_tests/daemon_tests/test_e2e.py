...
def test_asset_daemon_with_sensor() -> None:
    time = get_current_datetime()
    with get_workspace_request_context(["check_after_parent_updated"]) as context:
        assert _get_latest_evaluation_ids(context) == {0}
        assert _get_runs_for_latest_ticks(context) == []

        with freeze_time(time):
            _execute_ticks(context)
