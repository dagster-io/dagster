...
def test_serialize_definitions_with_user_code_asset_condition() -> None:
    ...
    assert isinstance(automation_condition, UnexecutableAutomationCondition)
    assert len(automation_condition.children) == 2
    assert automation_condition.children[1].name == "MyAutomationCondition"
