...
def _create_plan_and_run(job_def: dg.JobDefinition, run_config: dict[str, object] | None = None):
...
@pytest.fixture
def simple_plan_and_run(simple_job_def):
...
@pytest.fixture
def multi_asset_step_context(multi_asset_job_def, dagster_instance):
    with _step_context_for_job(multi_asset_job_def, dagster_instance) as context:
        yield context


@pytest.fixture
def multi_asset_compute_context(multi_asset_step_context):
    with enter_execution_context(multi_asset_step_context) as compute_context:
        yield compute_context


@pytest.fixture
def step_context_factory(dagster_instance):
    def _factory(
        job_def: dg.JobDefinition,
        step_key: str | None = None,
        run_config: dict[str, object] | None = None,
