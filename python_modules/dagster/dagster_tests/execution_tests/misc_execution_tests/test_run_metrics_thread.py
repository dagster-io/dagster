    ...
    with patch.object(dagster_instance.run_storage, "supports_run_telemetry", return_value=True):
        with patch(
            ...
            with patch(
                ...
                assert thread.is_alive()  # ty: ignore[unresolved-attribute]
                assert "Starting run metrics thread" in caplog.messages[0]

                time.sleep(0.1)
                shutdown.set()  # ty: ignore[unresolved-attribute]

                thread.join()  # ty: ignore[unresolved-attribute]
                assert thread.is_alive() is False  # ty: ignore[unresolved-attribute]
