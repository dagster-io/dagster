...
def test_single_solid_pipeline_success():
    ...
    start_event = single_dagster_event(events, ExecutionStepEventType.STEP_START)
    ...
    output_event = single_dagster_event(events, ExecutionStepEventType.STEP_OUTPUT)
    assert output_event
    ...
    success_event = single_dagster_event(events, ExecutionStepEventType.STEP_SUCCESS)
    ...
    assert isinstance(success_event.dagster_event.step_success_data.time_ms, float)
    assert success_event.dagster_event.step_success_data.time_ms > 0.0
...
def test_single_solid_pipeline_failure():
    ...
    failure_event = single_dagster_event(events, ExecutionStepEventType.STEP_FAILURE)
