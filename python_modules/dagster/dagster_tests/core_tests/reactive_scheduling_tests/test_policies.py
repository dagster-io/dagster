...
class AlwaysLaunchSchedulingPolicy(SchedulingPolicy):
    ...
    def schedule_launch(
        ...
        return ScheduleLaunchResult(launch=True)
...
class AlwaysLaunchLatestTimeWindowSchedulingPolicy(SchedulingPolicy):
    ...
    def schedule_launch(
        ...
        return ScheduleLaunchResult(
            launch=True, explicit_launching_slice=asset_slice.latest_complete_time_window
        )
...
class LatestRequiredRunTags(SchedulingPolicy):
    ...
    def evaluate(
        ...
        if not all(
            record for record in current_slice.compute_entries_by_asset_partition().values()
        ):
        ...
        for asset_partition, record in current_slice.compute_entries_by_asset_partition().items():
...
def build_test_context(
    ...
    return SchedulingExecutionContext.create(
        ...
        effective_dt=tick_dt or pendulum.now(),
        last_event_id=last_storage_id,
    )
