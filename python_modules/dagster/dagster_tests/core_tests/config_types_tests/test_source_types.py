...
from dagster import Array, BoolSource, FloatSource, IntSource, Noneable, StringSource
...
def test_float_source():
    assert process_config(FloatSource, 1).success
    assert not process_config(FloatSource, "foo").success

    assert not process_config(FloatSource, {"env": 1}).success

    assert "DAGSTER_TEST_ENV_VAR" not in os.environ
    assert not process_config(FloatSource, {"env": "DAGSTER_TEST_ENV_VAR"}).success

    assert (
        'You have attempted to fetch the environment variable "DAGSTER_TEST_ENV_VAR" '
        "which is not set. In order for this execution to succeed it must be set in "
        "this environment."
        in process_config(FloatSource, {"env": "DAGSTER_TEST_ENV_VAR"}).errors[0].message
    )

    with environ({"DAGSTER_TEST_ENV_VAR": "4"}):
        assert process_config(FloatSource, {"env": "DAGSTER_TEST_ENV_VAR"}).success
        assert process_config(FloatSource, {"env": "DAGSTER_TEST_ENV_VAR"}).value == 4.0

    with environ({"DAGSTER_TEST_ENV_VAR": "four"}):
        assert not process_config(FloatSource, {"env": "DAGSTER_TEST_ENV_VAR"}).success
        assert (
            'Value "four" stored in env variable "DAGSTER_TEST_ENV_VAR" cannot '
            "be coerced into a float."
            in process_config(FloatSource, {"env": "DAGSTER_TEST_ENV_VAR"}).errors[0].message
        )
