...
def test_filter_out_resources():
    @op
    def requires_resource_a(context, a: Injected[str]):
        assert a
        assert context.resources.a
        assert not hasattr(context.resources, "b")

    @op
    def requires_resource_b(context, b: Injected[str]):
        assert b
...
def test_init_resources():
    ...
    @op
    def consumes_resource_a(a: Injected[str]):
        assert a == "A"

    @op
    def consumes_resource_b(b: Injected[str]):
        assert b == "B"
...
def test_ops_with_dependencies():
    ...
    @op
    def first_op(foo: Injected[str]):
        assert foo == "foo"
        completed.add("first_op")
        return "hello"

    @op
    def second_op(foo: Injected[str], first_op_result: str):
        assert foo == "foo"
    ...
    @op
    def fourth_op(
        context, second_op_result: str, foo: Injected[str], third_op_result: str
...
def test_assets():
    ...
    @asset
    def the_asset(context, foo: Injected[str]):
    ...
    @asset
    def the_other_asset(context, the_asset, foo: Injected[str]):
        assert context.resources.foo == "blah"
        assert foo == "blah"
    ...
    # Ensure ordering of resource args doesn't matter
    @asset
    def the_third_asset(context, the_asset, foo: Injected[str], the_other_asset):
        assert context.resources.foo == "blah"
        assert foo == "blah"
        assert the_asset == "hello"
...
def test_multi_assets():
    ...
    def two_assets(context, foo: Injected[str]):
        assert context.resources.foo == "blah"
        assert foo == "blah"
...
def test_resource_not_provided():
    ...
    def consumes_nonexistent_resource(
        not_provided: Injected[str],  # pylint: disable=unused-argument
    ):
        pass
...
def test_both_decorator_and_argument_error():
    with pytest.raises(
        ...
        def my_asset(bar: Injected[Any]):  # pylint: disable=unused-argument
            pass
    ...
    with pytest.raises(
        ...
        def my_assets(bar: Injected[Any]):  # pylint: disable=unused-argument
            pass
    ...
    with pytest.raises(
        ...
        def my_op(bar: Injected[Any]):  # pylint: disable=unused-argument
            pass
...
def test_asset_with_structured_config():
    ...
    @asset
    def the_asset(context, config: AnAssetConfig, foo: Injected[str]):
        assert context.resources.foo == "blah"
        assert foo == "blah"
...
def test_no_err_builtin_annotations():
    ...
    @asset
    def the_other_asset(context, the_asset: list[dict[str, str]], foo: Injected[str]):
        assert context.resources.foo == "blah"
        assert foo == "blah"
