...
def test_filter_out_resources():
    @op
    def requires_resource_a(context, a: FromResources[str]):
        assert a
        assert context.resources.a
        assert not hasattr(context.resources, "b")

    @op
    def requires_resource_b(context, b: FromResources[str]):
        assert b
...
def test_init_resources():
    ...
    @op
    def consumes_resource_a(a: FromResources[str]):
        assert a == "A"

    @op
    def consumes_resource_b(b: FromResources[str]):
        assert b == "B"
...
def test_ops_with_dependencies():
    ...
    @op
    def first_op(foo: FromResources[str]):
        assert foo == "foo"
        completed.add("first_op")
        return "hello"

    @op
    def second_op(foo: FromResources[str], first_op_result: str):
        assert foo == "foo"
    ...
    @op
    def fourth_op(context, second_op_result: str, foo: FromResources[str], third_op_result: str):
        assert foo == "foo"
...
def test_assets():
    ...
    @asset
    def the_asset(context, foo: FromResources[str]):
    ...
    @asset
    def the_other_asset(context, the_asset, foo: FromResources[str]):
        assert context.resources.foo == "blah"
        assert foo == "blah"
    ...
    # Ensure ordering of resource args doesn't matter
    @asset
    def the_third_asset(context, the_asset, foo: FromResources[str], the_other_asset):
        assert context.resources.foo == "blah"
        assert foo == "blah"
        assert the_asset == "hello"
...
def test_multi_assets():
    ...
    def two_assets(context, foo: FromResources[str]):
        assert context.resources.foo == "blah"
        assert foo == "blah"
...
def test_resource_not_provided():
    ...
    def consumes_nonexistent_resource(
        not_provided: FromResources[str],
    ):
        pass
...
def test_resource_class():
    ...
    @op
    def do_something_op(my_resource: FromResources[MyResource]):
    ...
    def consumes_nonexistent_resource_class(
        not_provided: FromResources[MyResource],
    ):
        pass
...
def test_both_decorator_and_argument_error():
    with pytest.raises(
        ...
        def my_asset(bar: FromResources[Any]):
            pass
    ...
    with pytest.raises(
        ...
        def my_assets(bar: FromResources[Any]):
            pass
    ...
    with pytest.raises(
        ...
        def my_op(bar: FromResources[Any]):
            pass
...
def test_asset_with_structured_config():
    ...
    @asset
    def the_asset(context, config: AnAssetConfig, foo: FromResources[str]):
        assert context.resources.foo == "blah"
        assert foo == "blah"
...
def test_no_err_builtin_annotations():
    ...
    @asset
    def the_other_asset(context, the_asset: list[dict[str, str]], foo: FromResources[str]):  # type: ignore  # (python <3.9 compat)
        assert context.resources.foo == "blah"
        assert foo == "blah"
