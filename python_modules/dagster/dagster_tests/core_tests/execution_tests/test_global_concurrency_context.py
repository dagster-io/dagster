...
import pytest
from dagster._core.execution.plan.global_concurrency_context import (
    DEFAULT_CONCURRENCY_CLAIM_BLOCKED_INTERVAL,
    GlobalConcurrencyContext,
)
...
def test_custom_interval(concurrency_custom_sleep_instance):
    ...
    with GlobalConcurrencyContext(concurrency_custom_sleep_instance, run_id) as context:
        assert context.claim("foo", "a")
        assert not context.claim("foo", "b")
