...
def test_external_repository_data(snapshot):
    @repository
    def repo():
        return [a_job_no_partitions, a_schedule, a_job]

    external_repo_data = external_repository_data_from_def(repo)
    assert external_repo_data.get_external_pipeline_data("a_job_no_partitions")
...
def test_external_pipeline_data(snapshot):
    snapshot.assert_match(serialize_pp(external_pipeline_data_from_def(a_job_no_partitions)))
