...
def test_materialize_in_process_executor():
    result = module_level_repo_in_process_executor.materialize("asset1")
...
def test_materialize_multiprocess_executor():
    with instance_for_test():
        result = module_level_repo_multiprocess_executor.materialize("asset1")
        assert result.success
...
def test_materialize_partitions():
    with instance_for_test():
        with pytest.raises(DagsterInvalidSubsetError):
            module_level_repo_with_partitioned_assets.materialize(["asset1", "asset2"])
...
def test_repo_not_module_scoped():
    ...
    with instance_for_test():
        with pytest.raises(
            ...
            repo_multiprocess_executor.materialize("asset1")
