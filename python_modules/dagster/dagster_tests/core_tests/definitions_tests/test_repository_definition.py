...
def test_bad_coerce():
    ...
    with pytest.raises(DagsterInvalidDefinitionError, match="Failed attempting to coerce graph 'bar' into a job using provided repo-level defaults: resource key 'x' is required by op 'foo', but is not provided."):
