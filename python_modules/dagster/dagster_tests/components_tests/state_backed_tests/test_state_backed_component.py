...
class MyStateBackedComponent(StateBackedComponent, dg.Model, dg.Resolvable):
    state_refresh_defs: ResolvedStateRefreshDefs = None
    ...
    def write_state_to_path(self, state_path: Path) -> str:
    ...
    def build_defs(self, context: dg.ComponentLoadContext) -> dg.Definitions:
        return dg.Definitions.merge(
            ...
            self.state_refresh_defs(context, self) if self.state_refresh_defs else dg.Definitions(),
        )


def test_simple_state_backed_component() -> None:
    REFRESH_STATE_JOB_NAME = "my_fancy_refresh_job_name"
    with instance_for_test() as instance, create_defs_folder_sandbox() as sandbox:
        component_path = sandbox.scaffold_component(
            ...
            defs_yaml_contents={
                "type": "dagster_tests.components_tests.state_backed_tests.test_state_backed_component.MyStateBackedComponent",
                "attributes": {
                    "state_refresh_defs": {
                        "job": {
                            "name": REFRESH_STATE_JOB_NAME,
                        }
                    }
                },
            },
        ...
        with sandbox.load_component_and_build_defs(defs_path=component_path) as (_, defs):
            ...
            refresh_job = defs.get_job_def(REFRESH_STATE_JOB_NAME)
            refresh_job.execute_in_process(instance=instance)

        # now we reload the definitions, state should be updated
        with sandbox.load_component_and_build_defs(defs_path=component_path) as (_, defs):
            specs = defs.get_all_asset_specs()
            spec = specs[0]
