...
def executable_in_same_run(
    ...
    from dagster._core.definitions.remote_asset_graph import RemoteGlobalAssetGraph
    from dagster._core.definitions.time_window_partition_mapping import TimeWindowPartitionMapping

    child_node = asset_graph.get(child_key)
    parent_node = asset_graph.get(parent_key)

    # if operating on a RemoteGlobalAssetGraph, must share a repository handle
    if isinstance(asset_graph, RemoteGlobalAssetGraph):
