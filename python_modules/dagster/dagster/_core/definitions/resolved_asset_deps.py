...
def resolve_storage_match(
    ...
    Args:
        dep_storage: The StorageMetadataSet extracted from the dependency
        index: Mapping from storage_address to list of StorageMatch
        dep_key: The asset key of the dependency (for error messages)
        spec_key: The asset key of the spec declaring the dependency (for error messages)

    Returns:
        The resolved asset key if a unique match is found, None otherwise

    Raises:
        DagsterInvalidDefinitionError: If multiple matches exist and cannot be disambiguated
    """
