...
class UnresolvedAssetSensorDefinitionV1(UnresolvedAssetSensorDefinition):
    ...
    def make_sensor(self, dependency_mapping):
        """
        ...
        def sensor_fn(context):
            ...
            for a in upstream.keys():
                ...
                print(f"CURSOR for {a} {a_cursor}")
                parent_asset_event_records = {}
                a_cursor_update_dict = {}
                for p in upstream[a]:
                    print(f"AFTER CURSOR for {p} {a_cursor.get(str(p))}")
                    event_records = context.instance.get_event_records(
                        EventRecordsFilter(
                            event_type=DagsterEventType.ASSET_MATERIALIZATION,
                            asset_key=p,
                            after_cursor=a_cursor.get(str(p)),
                        ),
                        ascending=False,
                        limit=1,
                    )
                    if event_records:
                        ...
                        a_cursor_update_dict[str(p)] = event_records[0].storage_id
                    else:
                        parent_asset_event_records[p] = None
                        a_cursor_update_dict[str(p)] = a_cursor.get(str(p))

                if all(parent_asset_event_records.values()):
...
def build_asset_sensor(selection: AssetSelection, name: str) -> UnresolvedAssetSensorDefinition:
    return UnresolvedAssetSensorDefinitionV1(asset_selection=selection, name=name)
