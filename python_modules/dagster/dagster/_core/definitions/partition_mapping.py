...
from dagster._record import record, record_custom
...
class MultiPartitionMapping(BaseMultiPartitionMapping, PartitionMapping):
    ...
    def get_dimension_dependencies(
        ...
        return [
            DimensionDependency(
                partition_mapping=mapping.partition_mapping,
    ...
    def __hash__(self) -> int:
        return hash(
            frozenset((k, v) for k, v in self.downstream_mappings_by_upstream_dimension.items())
        )
...
class StaticPartitionMapping(
    ...
    downstream_partition_keys_by_upstream_partition_key: Mapping[str, Union[str, Collection[str]]]

    @cached_property
    def _mapping(self) -> Dict[str, AbstractSet[str]]:
        return {
            upstream_key: (
                {downstream_key} if isinstance(downstream_key, str) else frozenset(downstream_key)
            )
            for upstream_key, downstream_key in self.downstream_partition_keys_by_upstream_partition_key.items()
        }

    @cached_property
    def _inverse_mapping(self) -> Mapping[str, AbstractSet[str]]:
        upstreams_by_downstream_key = defaultdict(set)
        for (
            upstream_key,
            downstream_keys,
        ) in self.downstream_partition_keys_by_upstream_partition_key.items():
            for downstream_key in downstream_keys:
                upstreams_by_downstream_key[downstream_key].add(upstream_key)
        return upstreams_by_downstream_key
