...
def build_last_update_freshness_checks(
    *,
    ...
    considered fresh if a record exists with an update timestamp greater than the "lower bound"
    derived from the freshness_cron and lower_bound_delta parameters. The "lower bound" is defined
    as the most recent tick of the cron minus lower_bound_delta. If no freshness_cron is provided,
    it is the current time minus lower_bound_delta.
    ...
    Args:
        ...
        lower_bound_delta (int): The check will pass if the asset was updated within
