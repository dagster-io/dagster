    ...
    def get_node_unique_id(
        ...
        *,
        ...
        return non_secure_md5_hash_str(
            f"{parent_unique_id}{index}{self.name}{self.get_label() or ''}{target_key or ''}".encode()
        )
...
class BuiltinAutomationCondition(AutomationCondition[T_EntityKey]):
    ...
    def _get_stable_unique_id(
        self, target_key: EntityKey | None, use_backcompat_child_ids: bool = False
    ) -> str:
        ...
        for i, child in enumerate(self.children):
            if use_backcompat_child_ids:
                bc_ids = child.get_backcompat_node_unique_ids(
                    parent_unique_id=None, index=i, target_key=target_key
                )
                child_ids.append(
                    bc_ids[0]
                    if bc_ids
                ...
                child_ids.append(
                    child.get_node_unique_id(parent_unique_id=None, index=i, target_key=target_key)
                )
        ...
        parts = [
            ...
            self.get_label() or "",
            *child_ids,
            str(target_key) if target_key else "",
        ]
    ...
    def get_backcompat_node_unique_ids(
        ...
        *,
        ...
        backcompat_ids = []
        # if this node has a stable unique id, check if it has a legacy stable unique id based on
        # its children's backcompat ids
        if self.get_node_unique_id(
            parent_unique_id=parent_unique_id, index=index, target_key=target_key
        ) == self._get_stable_unique_id(target_key):
            legacy_stable_id = self._get_stable_unique_id(target_key, use_backcompat_child_ids=True)
            if legacy_stable_id != self.get_node_unique_id(
                parent_unique_id=parent_unique_id, index=index, target_key=target_key
            ):
                backcompat_ids.append(legacy_stable_id)
        return backcompat_ids
