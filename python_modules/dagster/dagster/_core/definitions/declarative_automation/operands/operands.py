...
async def _get_run_records_from_materializations(
    ...
    run_records = []
    run_step = int(os.getenv("DAGSTER_ASSET_DAEMON_RUN_RECORDS_FROM_MATERIALIZATION_LIMIT", "1000"))
...
def _get_validated_partition_keys(
    unvalidated_partition_keys: Set[Optional[str]],
    context: AutomationContext,
) -> Set[Optional[str]]:
    partitions_def = context.asset_graph.get(context.key).partitions_def
    if partitions_def is None:
        return {
            partition_key for partition_key in unvalidated_partition_keys if partition_key is None
        }
    else:
        return {
            partition_key
            for partition_key in unvalidated_partition_keys
...
class AnyNewMaterializationHasRunTagsCondition(SubsetAutomationCondition):
    ...
    async def compute_subset(self, context: AutomationContext) -> EntitySubset:  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        valid_partition_keys = _get_validated_partition_keys(valid_partition_keys, context)
...
class AllNewUpdatesHaveRunTagsCondition(SubsetAutomationCondition):
    ...
    async def compute_subset(self, context: AutomationContext) -> EntitySubset:  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        valid_partition_keys = _get_validated_partition_keys(
            materialized_partition_keys - invalid_partition_keys, context
        )
