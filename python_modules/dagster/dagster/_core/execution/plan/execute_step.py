...
def _process_user_event(
    ...
    if isinstance(user_event, AssetResult):
        ...
        assets_def = _get_assets_def_for_step(step_context, user_event)
        check_specs_by_key = assets_def._check_specs_by_key  # noqa: SLF001
        spec = check.not_none(
            check_specs_by_key.get(asset_check_evaluation.asset_check_key),
        ...
        if spec.key in assets_def.check_keys:
            output_name = check.not_none(
                ...
                f"No output name found for check key {spec.key} in step {step_context.step.key}. This likely indicates that the currently executing AssetsDefinition has no check specified for the key. The provided check specs are: {set(check_specs_by_key.keys())}.",
            )
