...
from dagster._core.definitions.asset_spec import AssetResultType
...
def _get_output_asset_events(
    ...
    if result_type == "materialize":
        event_class = AssetMaterialization
        event_class = AssetMaterialization
    elif result_type == "observe":
        event_class = AssetObservation
    else:
        check.failed(f"Unexpected asset execution type {result_type}")
...
def _log_materialization_or_observation_events_for_asset(
    ...
    if asset_key:
        ...
        result_type = check.not_none(assets_def).result_type
        ...
        check.invariant(
            result_type in {"materialize", "observe"},
            f"Unexpected asset execution type {result_type}",
        )
