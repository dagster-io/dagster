...
class StepExecutionContext(PlanExecutionContext, IStepContext):
    ...
    def add_asset_metadata(
        ...
        if not self.assets_def:
            raise DagsterInvariantViolationError(
                "Attempted to log metadata for a non-asset computation. Only assets should be calling this function."
            )
        if len(self.assets_def.keys) == 0:
            raise DagsterInvariantViolationError(
                "Attempted to log metadata without providing asset_key, but no asset_keys"
        ...
        if asset_key is None and len(self.assets_def.keys) > 1:
            raise DagsterInvariantViolationError(
                "Attempted to log metadata without providing asset_key, but multiple asset_keys"
        ...
        if asset_key not in self.assets_def.keys:
            raise DagsterInvariantViolationError(
                f"Attempted to log metadata for asset key '{asset_key}' that is not being materialized."
            )
        if partition_key:
            if not self.assets_def.partitions_def:
                raise DagsterInvariantViolationError(
                    f"Attempted to log metadata for partition key '{partition_key}' without a partitions definition."
                )
            ...
            if partition_key not in targeted_partitions:
                raise DagsterInvariantViolationError(
                    f"Attempted to log metadata for partition key '{partition_key}' that is not being targeted."
                )
