...
if TYPE_CHECKING:
    from dagster._core.instance.run_launcher.run_launcher_instance_ops import RunLauncherInstanceOps
    from dagster._core.run_coordinator.base import BaseWorkspaceRequestContext
    from dagster._core.storage.dagster_run import DagsterRun


def submit_run(
    ops: "RunLauncherInstanceOps", run_id: str, workspace: "BaseWorkspaceRequestContext"
) -> "DagsterRun":
    ...
    try:
        submitted_run = ops.run_coordinator.submit_run(SubmitRunContext(run, workspace=workspace))
    except:
        from dagster._core.events import EngineEventData

        error = serializable_error_info_from_exc_info(sys.exc_info())
        ops.report_engine_event(
            error.message,
            run,
            EngineEventData.engine_error(error),
        )
        ops.report_run_failed(run)
        raise
...
def launch_run(
    ...
    ops.report_dagster_event(launch_started_event, run_id=run.run_id)
...
def resume_run(
    ...
    if run.status not in IN_PROGRESS_RUN_STATUSES:
        raise DagsterInvariantViolationError(f"Run {run_id} is not in a state that can be resumed")
...
def count_resume_run_attempts(ops: "RunLauncherInstanceOps", run_id: str) -> int:
    """Count resume run attempts."""
    from dagster._daemon.monitoring import count_resume_run_attempts

    return count_resume_run_attempts(ops.instance, run_id)


def run_will_resume(ops: "RunLauncherInstanceOps", run_id: str) -> bool:
    """Check if run will resume."""
    if not ops.run_monitoring_enabled:
        return False
    return count_resume_run_attempts(ops, run_id) < ops.run_monitoring_max_resume_run_attempts
