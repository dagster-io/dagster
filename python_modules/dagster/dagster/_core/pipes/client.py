...
class PipesClient(ABC):
    ...
    def run_non_blocking(
        ...
        **kwargs,
        ...
        """Launches an external process with the pipes protocol but doesn't wait for termination. Can be used
        to consume messages from the external process as they are reported. Derived
        clients must have `context` and `extras` arguments, but also can add arbitrary
        arguments that are appropriate for their own implementation.
        ...
        Returns:
            ...
            process.
        """
        raise NotImplementedError(f"{self.__class__.__name__}.run_non_blocking is not implemented")
...
class PipesClientRunningInvocation:
    """A wrapper for the results of a pipes client invocation, typically returned from `PipesClient.run_non_blocking`.
    ...
    def __init__(
        self,
        wait_func: Callable[["PipesClientRunningInvocation"], "PipesClientCompletedInvocation"],
        session: PipesSession,
        metadata: Optional[RawMetadataMapping] = None,
    ):
    ...
    def wait(self) -> "PipesClientCompletedInvocation":
        """Wait for the external process to complete and return a completed invocation wrapper.

        Returns:
            PipesClientCompletedInvocation: Wrapper containing results reported by the external
            process.
        """
        return self._wait_func(self)
    ...
    def stream_reported_results(
        ...
        *,
        ...
        Returns: Sequence[PipesExecutionResult]
        """
        yield from self._session.stream_reported_results(timeout=timeout, keep=keep)
    ...
    def stream_custom_messages(
        ...
        Yields: Any
        """
        yield from self._session.stream_custom_messages(timeout=timeout, keep=keep)
