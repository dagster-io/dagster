...
class AssetDaemon(DagsterDaemon):
    ...
    async def _evaluate_auto_materialize_tick(
        ...
        if is_retry:
            ...
            # Do in batches to ensure code servers don't get overloaded
            batch_size = int(os.getenv("DAGSTER_ASSET_DAEMON_CODE_SERVER_BATCH_SIZE", "10"))
            for i in range(0, len(async_code_server_tasks), batch_size):
                batch = async_code_server_tasks[i : i + batch_size]
