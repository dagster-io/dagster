...
def get_auto_materialize_paused(instance: DagsterInstance) -> bool:
    return (
        instance.daemon_cursor_storage.kvs_get({ASSET_DAEMON_PAUSED_KEY}).get(
...
def set_auto_materialize_paused(instance: DagsterInstance, paused: bool):
    instance.daemon_cursor_storage.kvs_set({ASSET_DAEMON_PAUSED_KEY: "true" if paused else "false"})
...
class AssetDaemon(IntervalDaemon):
    ...
    def run_iteration(
        ...
        persisted_info = instance.daemon_cursor_storage.kvs_get(
        ...
        instance.daemon_cursor_storage.kvs_set({CURSOR_KEY: new_cursor.serialize()})
