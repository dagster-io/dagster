...
from dagster._core.utils import InheritContextThreadPoolExecutor, make_new_backfill_id
...
class SensorLaunchContext(AbstractContextManager):
    ...
    def backfill_count(self) -> int:
        return len(self._tick.backfill_ids)
    ...
    def add_backfill_info(self, backfill_id: str) -> None:
        self._tick = self._tick.with_backfill_info(backfill_id)
...
def _evaluate_sensor(
    ...
    if not sensor_runtime_data.run_requests and not sensor_runtime_data.backfill_requests:
        ...
        if sensor_runtime_data.backfill_requests:
            _handle_backfill_requests(sensor_runtime_data.backfill_requests, instance, context)
    ...
    for backfill_request in backfill_requests:
        ...
        instance.add_backfill(
            PartitionBackfill.from_asset_graph_subset(
                ...
                asset_graph_subset=backfill_request.asset_partitions,
                tags=backfill_request.tags or {},
            )
        )
        context.add_backfill_info(backfill_id=backfill_id)
