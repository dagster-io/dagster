...
class DagsterApiServer(DagsterApiServicer):
    ...
    def ListRepositories(self, request, _context):
        if not self._serializable_load_error:
            try:
                # loadable_repository_symbols property may raise an error if there's a
                # load error while trying to fetch the list of repositories. If such an error
                # is raised, cache it for future ListRepositories calls (since subsequent
                # loads might raise a different, less informative error due to Python caching),
