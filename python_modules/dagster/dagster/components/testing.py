...
def get_original_module_name(cls):
    ...
    if not module_name:
        raise ValueError(f"Class {cls} has no module name")
...
def random_importable_name(length: int = 8) -> str:
    ...
    # Check if it's a Python keyword and regenerate if needed
    import keyword
...
class DefsPathSandbox:
    ...
    def load(
        ...
        if component_body is not None:
            ...
            old_text = (self.defs_folder_path / "defs.yaml").read_text()
            (self.defs_folder_path / "defs.yaml").write_text(yaml.safe_dump(component_body))
