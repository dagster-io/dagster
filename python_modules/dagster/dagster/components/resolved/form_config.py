"""Typed JSON Schema metadata for the UI definitions form editor.
...
are rendered in the Dagster UI definitions editor. Component-level values
serialize to ``x-ui-editable`` (a Python-side filter) and the standard JSON
Schema ``title``. Field-level values serialize to react-jsonschema-form's
...
# Python-side filter marking a component class as UI-editable. Not consumed by
# the form renderer; checked by ``fetch_component_types`` to decide which
    ...
    **Field level** — pass to ``Resolver(form_config=...)``::
    ...
    Args:
        ...
        editable: Component level only. Whether this component type can be
            created and edited from the UI. Defaults to False.
