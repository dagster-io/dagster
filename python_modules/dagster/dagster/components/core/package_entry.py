...
def discover_entry_point_package_objects() -> dict[DgRegistryKey, object]:
    ...
    for entry_point in get_plugin_entry_points():
        ...
        for name, obj in get_package_objects_in_module(root_module):
            key = DgRegistryKey(name=name, namespace=entry_point.value)
            objects[key] = obj
    return objects


def discover_package_objects(modules: Sequence[str]) -> dict[DgRegistryKey, object]:
    objects: dict[DgRegistryKey, object] = {}
    for extra_module in modules:
        for name, obj in get_package_objects_in_module(importlib.import_module(extra_module)):
            key = DgRegistryKey(name=name, namespace=extra_module)
            objects[key] = obj
    return objects
...
def load_package_object(key: DgRegistryKey) -> object:
