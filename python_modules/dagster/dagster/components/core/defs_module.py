...
def load_yaml_component_from_path(context: ComponentLoadContext, component_def_path: Path):
    ...
    for source_tree in source_trees:
        ...
        attributes = get_attributes(model_cls, source_tree, component_file_model)

        # processing_tree = source_tree.source_position_tree.children.get("post_processing", None)
        post_processors = from_post_processing_dict(
            context.resolution_context, component_file_model.post_processing
        )

        post_processors_list.append(post_processors)
...
def get_attributes(
    ...
    if model_cls is None or not source_tree:
        return None
    ...
    with enrich_validation_errors_with_source_position(attributes_position_tree, ["attributes"]):
