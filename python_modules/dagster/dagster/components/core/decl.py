...
class YamlDecl(ComponentDecl):
    ...
    def _load_component(self) -> "Component":
        ...
        obj = _get_component_class(self.context, self.component_file_model)
...
class DefsFolderDecl(ComponentDecl[DefsFolderComponent]):
    ...
    def _load_component(self) -> "DefsFolderComponent":
        ...
        if self.component_file_model:
            model_cls = DefsFolderComponent.get_model_cls()
            attributes_model = (
                _process_attributes_with_enriched_validation_err(
                    self.source_tree, self.component_file_model, model_cls
                )
                if self.source_tree
                else TypeAdapter(model_cls).validate_python(self.component_file_model.attributes)
            )
            resolved_attributes = (
                DefsFolderComponentYamlSchema.resolve_from_model(
                    self.context.resolution_context.at_path("attributes"),
                    attributes_model,
                )
                if attributes_model
                else None
