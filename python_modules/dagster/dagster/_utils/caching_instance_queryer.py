...
class CachingInstanceQueryer(DynamicPartitionsStore):
    ...
    def _is_child_planned_for_latest_run_of_parent(
        self,
        child_key: AssetKey,
        child_partition: str,
        parent_key: AssetKey,
        newly_updated_parent_partitions: PartitionsSubset,
    ) -> bool:
        """Returns a boolean representing if the given child asset partition is planned to be
        executed in the latest run of of its parent partition for the given parent key.
        """
        if (
            # if child has a different partitions def than the parent, then it must
            # have been executed in a different run
            self.asset_graph.get_partitions_def(child_key)
            != self.asset_graph.get_partitions_def(parent_key)
            ...
            # its parent
            or child_partition not in newly_updated_parent_partitions
            ...
            or child_partition not in self.get_failed_or_in_progress_subset(asset_key=child_key)
        ):
            return False
        else:
            ...
            latest_partition_record = check.not_none(
                self.get_latest_materialization_or_observation_record(
                    AssetKeyPartitionKey(parent_key, child_partition)
                )
            )
            if not self.is_asset_planned_for_run(
                latest_partition_record.run_id, AssetKeyPartitionKey(child_key, child_partition)
            ):
                return False
        return True
    ...
    def asset_partitions_with_newly_updated_parents(
        ...
        result_latest_storage_id = after_cursor
        ...
        for asset_key in target_asset_keys_and_parents:
            ...
            if partitions_def is None:
                ...
                for child in self.asset_graph.get_children_partitions(
                    ...
                    if child.asset_key in target_asset_keys and not self.is_asset_planned_for_run(
                ...
                for child in self.asset_graph.get_children(asset_key):
                    ...
                    if child not in target_asset_keys:
                        ...
                        for child_partition in child_partitions_subset.get_partition_keys():
                            # only add this to the set if the asset is not planned to be executed
                            # in the same run as its newly-updated parent
                            if not self._is_child_planned_for_latest_run_of_parent(
                                child_key=child,
                                child_partition=child_partition,
                                parent_key=asset_key,
                                newly_updated_parent_partitions=partitions_subset,
                            ):
                                result_asset_partitions.add(
                                    AssetKeyPartitionKey(child, child_partition)
                                )

            result_latest_storage_id = max(
                result_latest_storage_id or 0,
