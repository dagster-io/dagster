...
def execute_list_command(cli_args, print_fn):
    with get_instance_for_service("``dagster job list``") as instance:
        with get_external_repository_from_kwargs(
            ...
            print_fn("*" * len(title))
            first = True
            for pipeline in external_repository.get_external_jobs():
                pipeline_title = f"Job: {pipeline.name}"
                ...
                print_fn("{solid_or_op}: (Execution Order)".format(solid_or_op="Ops"))
...
def execute_print_command(instance, verbose, cli_args, print_fn):
    with get_external_pipeline_or_job_from_kwargs(
        ...
        if verbose:
            print_job(pipeline_snapshot, print_fn=print_fn)
        else:
            print_ops(pipeline_snapshot, print_fn=print_fn)
...
def print_job(pipeline_snapshot: PipelineSnapshot, print_fn: Callable[..., Any]):
    check.inst_param(pipeline_snapshot, "pipeline", PipelineSnapshot)
    check.callable_param(print_fn, "print_fn")
    printer = IndentingPrinter(indent_level=2, printer=print_fn)
    printer.line(f"Job: {pipeline_snapshot.name}")
...
@telemetry_wrapper
def execute_execute_command(instance: DagsterInstance, kwargs: Dict[str, object]):
    check.inst_param(instance, "instance", DagsterInstance)
...
@telemetry_wrapper
def execute_launch_command(instance: DagsterInstance, kwargs: Dict[str, str]):
    preset = cast(Optional[str], kwargs.get("preset"))
    mode = cast(Optional[str], kwargs.get("mode"))
    check.inst_param(instance, "instance", DagsterInstance)
