...
@click.option(
    "--schema-path",
    type=click.STRING,
    help="Path save the JSON schema.",
    default="dagster.autogenerated.schema.json",
)
def configure_vscode(loader_module: str, vscode_folder_path: str, schema_path: str):
    ...
    # No theoretical reason we can't support multiple per module, but punting on it for now
    check.invariant(len(loaders) < 2, "Multiple YamlBlueprintsLoader objects found in the module")
    loader = loaders[0]

    # Write schema
    schema_path_absolute = Path(schema_path).absolute()
    blueprint_type = loader.per_file_blueprint_type
    schema_path_absolute.write_text(json.dumps(blueprint_type.model_json_schema(), indent=2))
    click.echo(f"Wrote schema for {blueprint_type.__name__} to {schema_path_absolute}")
    ...
    add_yaml_schema_to_settings(dot_vscode_path, loader.path, schema_path_absolute)
