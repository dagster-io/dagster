...
def execute_materialize_command(instance: DagsterInstance, kwargs: Mapping[str, str]) -> None:
    ...
    if partition:
        ...
        if partitions_def is None or all(
            implicit_job_def.asset_layer.asset_graph.get(asset_key).partitions_def is None
        ...
        if any(
            implicit_job_def.asset_layer.asset_graph.get(asset_key).partitions_def is not None
            for asset_key in asset_keys
        ):
