...
def create_execution_plan_core(execution_info, execution_metadata):
    ...
    for plan_id in reversed(execution_info.plan_tracker.plan_order):
        ...
        new_plan = _create_plan(plan_id, execution_info, plan_builder)
        last_plan = new_plan
        plans_dict[new_plan.plan_id] = new_plan
    return last_plan


def _create_plan(plan_id, execution_info, plan_builder):
    check.inst_param(execution_info, 'execution_info', CreateExecutionPlanInfo)
            ...
            for output_def in solid.definition.output_defs:
                with plan_builder.push_tags(output=output_def.name):
                    ...
                    plan_builder.set_step_output_for_solid_output_handle(
                        solid_output_handle, subplan.terminal_step_output_handle
                    )

    new_plan = create_execution_plan_from_steps(plan_id, plan_builder.get_steps())
    return new_plan
...
def create_subplan(execution_plan_info, execution_plan, subset_info):
    ...
    check.invariant(len(execution_plan_info.plan_tracker.plan_order) == 1)
    plan_id = execution_plan_info.plan_tracker.plan_order[0]

    plan_builder = PlanBuilder(
        plan_id,
        execution_plan_info.plan_tracker.solid_names_by_plan_id[plan_id],
        {},
        execution_plan_info.pipeline.name,
    )
    ...
    for step in execution_plan.steps:
        ...
        with plan_builder.push_tags(**step.tags):
            if step.key not in subset_info.inputs:
                ...
                steps.extend(_create_new_steps_for_input(plan_builder, step, subset_info))

    included_keys = {step.key for step in steps}
    ...
    return create_execution_plan_from_steps(plan_id, steps)
