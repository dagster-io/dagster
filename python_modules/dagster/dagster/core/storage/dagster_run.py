...
class DagsterRunSerializer(DefaultNamedTupleSerializer):
    ...
    def value_from_storage_dict(cls, storage_dict, _klass):
        ...
        return dagster_run_from_storage(**storage_dict)
...
def dagster_run_from_storage(
    ...
    **kwargs,
    ...
    # serdes log
    # * removed pipeline_name, mode in favor of target (JobTarget and PipelineTarget)
    # * renamed solid_selection -> node_selection, solids_to_execute -> nodes_to_execute
    ...
    if selector:
        check.invariant(
            ...
            (
                "Conflicting pipeline name {pipeline_name} in arguments to DagsterRun: "
        ...
        check.invariant(
            ...
            (
                "Conflicting solids_to_execute {solids_to_execute} in arguments to DagsterRun: "
    ...
    # back compat for solid_selection => node_selection, solids_to_execute => nodes_to_execute
    if solids_to_execute is not None:
        nodes_to_execute = solids_to_execute

    if solid_selection is not None:
...
class DagsterRun(
    namedtuple(
        "_DagsterRun",
        (
            "target run_id run_config node_selection nodes_to_execute "
    ...
    def __new__(
        ...
        check.invariant(
            ...
            (
                "Must set both root_run_id and parent_run_id when creating a DagsterRun that "
        ...
        check.opt_list_param(node_selection, "node_selection", of_type=str)
    ...
    def with_mode(self, mode):
        if isinstance(self.target, JobTarget):
            raise DagsterInvariantViolationError(
                "Attempted to call with_mode when using a run created from a job."
            )
        new_target = self.target._replace(mode=mode)
    ...
    def is_finished(self):
        return (
            self.status == DagsterRunStatus.SUCCESS
            or self.status == DagsterRunStatus.FAILURE
...
@whitelist_for_serdes
class DagsterRunsFilter(
    namedtuple(
        "_DagsterRunsFilter", "run_ids target_name statuses tags snapshot_id updated_after mode"
    )
):
    def __new__(
        cls,
    ...
    def for_schedule(schedule):
        return DagsterRunsFilter(tags=DagsterRun.tags_for_schedule(schedule))

    @staticmethod
    def for_partition(partition_set, partition):
        return DagsterRunsFilter(tags=DagsterRun.tags_for_partition_set(partition_set, partition))

    @staticmethod
    def for_sensor(sensor):
        return DagsterRunsFilter(tags=DagsterRun.tags_for_sensor(sensor))

    @staticmethod
    def for_backfill(backfill_id):
        return DagsterRunsFilter(tags=DagsterRun.tags_for_backfill_id(backfill_id))
...
class ExecutionSelector(namedtuple("_ExecutionSelector", "name solid_subset")):
    """
    Kept here to maintain loading of DagsterRuns from when it was still alive.
    """
