...
class SqlRunStorage(RunStorage):  # pylint: disable=no-init
    ...
    def get_bulk_action(self, action_id: str) -> Optional[BulkAction]:
        ...
        row = self.fetchone(query)
        return deserialize_as(row[0], BulkAction) if row else None

    def add_bulk_action(self, bulk_action: BulkAction):
        check.inst_param(bulk_action, "bulk_action", BulkAction)
        check.invariant(
            bulk_action.type != BulkActionType.PARTITION_BACKFILL,
            "Backfills should use the add_backfill method",
        )
        with self.connect() as conn:
            conn.execute(
                BulkActionsTable.insert().values(  # pylint: disable=no-value-for-parameter
                    action_type=bulk_action.type.value,
    ...
    def update_bulk_action(self, bulk_action: BulkAction):
        ...
        check.invariant(
            bulk_action.type != BulkActionType.PARTITION_BACKFILL,
            "Backfills should use the update method",
        )
