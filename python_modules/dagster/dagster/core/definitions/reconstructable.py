...
def reconstructable(target):
    ...
    :py:func:`custom_reconstructable` instead, which allows you to specify your own strategy for
    reconstructing a pipeline.
    ...
    if seven.qualname_differs(target):
        raise DagsterInvariantViolationError(
            ...
            "defined at module scope instead, or use custom_reconstructable.".format(target=target)
        )
    ...
    if python_file.endswith("<stdin>"):
        raise DagsterInvariantViolationError(
            ...
            "use custom_reconstructable.".format(python_file)
        )
...
def custom_reconstructable(
    ...
    Args:
        target (Union[Callable, PipelineDefinition]): A function that returns a PipelineDefinition,
            or a function decorated with @pipeline. The pipeline to make reconstructable.
    ...
    .. code-block:: python
        ...
        from dagster import PipelineDefinition, pipeline, custom_reconstructable
        ...
        reconstructable_foo_pipeline = custom_reconstructable(
            foo_pipeline,
            'mymodule',
