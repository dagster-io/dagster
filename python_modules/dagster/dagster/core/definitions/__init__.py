...
_lazy_load_name_map = {name: mod for mod, names in LAZY_LOAD.items() for name in names}

def __getattr__(name):
    if name in _lazy_load_name_map:
        return getattr(importlib.import_module(_lazy_load_name_map[name], __name__), name)
    else:
        raise AttributeError("module '{}' has no attribute '{}'".format(__name__, name))

if TYPE_CHECKING or os.getenv("DAGSTER_NO_LAZY_LOAD") == "1":
