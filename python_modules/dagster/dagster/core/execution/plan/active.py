...
class ActiveExecution:
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if len(self._unknown_state) > 0:
            if self._interrupted:
                raise DagsterExecutionInterruptedError(
                    "Execution of job/pipeline exited with steps {step_list} in an unknown state after "
                    "being interrupted.".format(step_list=self._unknown_state)
                )
            else:
                raise DagsterUnknownStepStateError(
                    "Execution of job/pipeline exited with steps {step_list} in an unknown state to this process.\n"
