...
class ExternalRepository:
    ...
    def __init__(self, external_repository_data, repository_handle):
        ...
        self._instigation_map = OrderedDict(
            (job_data.name, job_data) for job_data in instigation_list
        )
        ...
        self._job_index_map = OrderedDict(
            (
                external_job_data.pipeline_snapshot.name,
                PipelineIndex(
                    external_job_data.pipeline_snapshot,
                    external_job_data.parent_pipeline_snapshot,
                ),
            )
            for external_job_data in external_repository_data.external_job_data
        )
    ...
    def get_external_job(self, job_name):
        ...
        return ExternalJob(
            self.external_repository_data.get_external_job_data(job_name),
            repository_handle=self.handle,
            pipeline_index=self._job_index_map[job_name],
        )
...
class ExternalJob(ExternalPipeline):
    def __init__(self, external_job_data, repository_handle, pipeline_index=None):
        check.inst_param(repository_handle, "repository_handle", RepositoryHandle)
        check.inst_param(external_job_data, "external_job_data", ExternalPipelineData)
        check.opt_inst_param(pipeline_index, "pipeline_index", PipelineIndex)
        ...
        super(ExternalJob, self).__init__(
            external_pipeline_data=external_job_data,
