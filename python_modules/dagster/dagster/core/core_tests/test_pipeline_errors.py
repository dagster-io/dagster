...
def test_pipeline_anonymous():
    assert PipelineDefinition(solids=[]).anonymous
    assert PipelineDefinition(name=None, solids=[]).anonymous
    assert PipelineDefinition(name='', solids=[]).anonymous
    assert not PipelineDefinition(name='yup', solids=[]).anonymous


def test_pipeline_anonymous_resource():
    anonymous_pipeline_def = PipelineDefinition(
        solids=[],
        context_definitions={
            'a_context':
            PipelineContextDefinition(
                resources={
                    'a_resource': ResourceDefinition(lambda *args, **kwargs: None),
                },
            ),
        },
    )
    ...
    with pytest.raises(DagsterInvariantViolationError):
        execute_pipeline(anonymous_pipeline_def)

    named_pipeline_def = PipelineDefinition(
        name='name',
        solids=[],
        context_definitions={
            'a_context':
    ...
    assert execute_pipeline(named_pipeline_def).success
