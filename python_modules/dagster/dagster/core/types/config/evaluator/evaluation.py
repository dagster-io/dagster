...
def validate_config(config_type, config_value):
    ...
    return _validate_config(context)


def _validate_config(context):
    check.inst_param(context, 'context', TraversalContext)

    if context.config_type.kind == ConfigTypeKind.NULLABLE:
        return (
            EvaluateValueResult.for_value(None)
            if context.config_value is None
            else _validate_config(context.for_nullable_inner_type())
        )

    if context.config_type.kind == ConfigTypeKind.ANY:
    ...
    if context.config_type.kind == ConfigTypeKind.SCALAR:
        if not is_config_scalar_valid(context.config_type, context.config_value):
    ...
    elif context.config_type.kind == ConfigTypeKind.SELECTOR:
        return validate_selector_config(context)
    elif context.config_type.kind == ConfigTypeKind.DICT:
        return validate_dict_config(context)
    elif context.config_type.kind == ConfigTypeKind.PERMISSIVE_DICT:
        return validate_permissive_dict_config(context)
    elif context.config_type.kind == ConfigTypeKind.LIST:
        return validate_list_config(context)
    elif context.config_type.kind == ConfigTypeKind.ENUM:
        return validate_enum_config(context)
    else:
        check.failed('Unsupported ConfigTypeKind {}'.format(context.config_type.kind))


def validate_selector_config(context):
    check.inst_param(context, 'context', TraversalContext)
...
def validate_permissive_dict_config(context):
    check.inst_param(context, 'context', TraversalContext)
    ...
    if context.config_value and not isinstance(context.config_value, dict):
        return EvaluateValueResult.for_error(create_dict_type_mismatch_error(context))

    # coercion and temp variable required because:
    # https://github.com/dagster-io/dagster/issues/1988
    config_dict_value = context.config_value or {}
    ...
    original_config_dict_value = copy.copy(config_dict_value)

    evr = _validate_fields(context, config_dict_value, errors)
    if not evr.success:
        return evr
    ...
    return EvaluateValueResult.for_value(merge_dicts(original_config_dict_value, evr.value))


def validate_dict_config(context):
    check.inst_param(context, 'context', TraversalContext)
...
def validate_list_config(context):
    check.inst_param(context, 'context', TraversalContext)
    ...
    evaluation_results = [
        _validate_config(context.for_list(index, item)) for index, item in enumerate(config_value)
    ]
...
def validate_enum_config(context):
    check.inst_param(context, 'context', TraversalContext)
