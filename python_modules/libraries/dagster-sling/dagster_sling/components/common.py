...
ResolvedSlingResource: TypeAlias = Annotated[
    SlingResource,
    Resolver(
        resolve_resource,
...
def build_sling_component_asset(
    context: ComponentLoadContext,
    replication_spec_model: SlingReplicationSpecModel,
    resource: SlingResource,
    execute_fn: Callable[
        [AssetExecutionContext, SlingResource, SlingReplicationSpecModel],
    ...
    def _asset(context: AssetExecutionContext):
        yield from execute_fn(context, resource, replication_spec_model)
...
def apply_post_processors(
    defs: Definitions, asset_post_processors: Optional[Sequence[AssetPostProcessor]]
):
    for post_processor in asset_post_processors or []:
