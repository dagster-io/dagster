...
def create_docker_task(celery_app, **task_kwargs):
    ...
    def _execute_step_docker(
        ...
        step_keys_str = ", ".join(execute_step_args.step_keys_to_execute)  # ty: ignore[no-matching-overload]

        docker_image = (
            docker_config["image"]
            if docker_config.get("image")
            else dagster_run.job_code_origin.repository_origin.container_image  # ty: ignore[unresolved-attribute]
        )
        ...
        engine_event = instance.report_engine_event(
            ...
            step_key=execute_step_args.step_keys_to_execute[0],  # ty: ignore[not-subscriptable]
        )
