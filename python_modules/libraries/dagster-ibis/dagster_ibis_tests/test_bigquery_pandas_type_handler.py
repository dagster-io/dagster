...
def test_ibis_io_manager_with_assets():
    ...
    with (
        temporary_bigquery_table(schema_name=SCHEMA) as source_table_name,
        temporary_bigquery_table(schema_name=SCHEMA) as downstream_table_name,
    ):
        # Define assets
            ...
            result = materialize(
                ...
                resources={
                    "io_manager": IbisIOManager(
                        backend="bigquery",
                        schema=SCHEMA,
                        project_id=SHARED_BUILDKITE_BQ_CONFIG["project"],
                        dataset_id=SCHEMA,
                    )
                },
            )
