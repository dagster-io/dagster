...
from typing import Dict, Generic, NamedTuple, Optional, Sequence, Type, TypeVar, Union
...
class DbTypeHandler(ABC, Generic[T]):
    @abstractmethod
    def handle_output(self, context: OutputContext, table_slice: TableSlice, obj: T) -> None:
...
class DbIOManager(IOManager):
    def __init__(self, type_handlers: Sequence[DbTypeHandler], db_client: DbClient):
        self._handlers_by_type: Dict[Type, DbTypeHandler] = {}
    ...
    def handle_output(self, context: OutputContext, obj: object) -> None:
        table_slice = self._get_table_slice(context)
