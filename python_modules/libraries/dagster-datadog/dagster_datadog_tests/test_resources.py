...
from dagster import FromResources, OpExecutionContext, build_op_context, job, op
...
def test_datadog_pythonic_resource_standalone_op(
    ...
    @op
    def datadog_op(datadog_client: FromResources[DataDogClient]):
    ...
    assert datadog_op(datadog_client=DataDogClient(api_key="NOT_USED", app_key="NOT_USED"))
    assert executed["yes"]
