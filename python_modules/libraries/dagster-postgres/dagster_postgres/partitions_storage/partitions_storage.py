...
from dagster._core.storage.partitions.sql_partitions_storage import SqlPartitionsStorage
...
class PostgresPartitionsStorage(SqlPartitionsStorage, ConfigurableClass):
    """Postgres-backed partition storage.
    ...
    def _init_db(self):
        with self.connect() as conn:
            with conn.begin():
                PartitionsStorageMetadata.create_all(conn)
    ...
    def from_config_value(inst_data, config_value):
        return PostgresPartitionsStorage(
            inst_data=inst_data,
    ...
    def create_clean_storage(postgres_url, should_autocreate_tables=True):
        ...
        try:
            PartitionsStorageMetadata.drop_all(engine)
        finally:
            engine.dispose()
        return PostgresPartitionsStorage(postgres_url, should_autocreate_tables)

    def connect(self):  # pylint: disable=arguments-differ, unused-argument
