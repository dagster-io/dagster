...
def test_end_to_end_integration(omni_workspace):
    ...
    assert len(assets) == 2  # 1 document + 1 query

    # Find the document asset
    doc_assets = [a for a in assets if "User Analysis" in str(a.key)]
    assert len(doc_assets) == 1
    doc_asset = doc_assets[0]

    # Verify document asset properties
    assert doc_asset.key == AssetKey(["analytics", "reports", "User Analysis"])
    assert doc_asset.group_name == "analytics"
    assert doc_asset.kinds == {"omni"}
    assert "dashboard" in doc_asset.tags
    assert "sales" in doc_asset.tags
    assert "omni/url" in doc_asset.metadata
    assert doc_asset.metadata["omni/document_name"] == "User Analysis"

    # Verify document has query dependency
    assert len(list(doc_asset.deps)) == 1
    assert next(iter(doc_asset.deps)).asset_key == AssetKey(["users"])
