...
import pandas as pd
from dagster import DynamicPartitionsDefinition, asset, instance_for_test, materialize
...
@asset(
    partitions_def=dynamic_regions, key_prefix=["vehicles"], metadata={"partition_expr": "region"}
)
def plant_data(context) -> pd.DataFrame:
    ...
    data = pd.read_csv(
        f"/Users/jamie/dev/dagster/python_modules/libraries/dagster-duckdb-pandas/dagster_duckdb_pandas_tests/data/{file}"
    )
...
def test_dynamic_partition(tmp_path):
    with instance_for_test() as instance:
        ...
        materialize(
            [plant_data], partition_key="northeast", resources=resource_defs, instance=instance
        )
