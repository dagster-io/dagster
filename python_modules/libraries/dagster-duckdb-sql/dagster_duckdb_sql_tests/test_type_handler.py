...
from dagster_duckdb_sql import DuckDBSqlIOManager, duckdb_sql_io_manager


@pytest.fixture
def io_managers(tmp_path):
    return [
        duckdb_sql_io_manager.configured(
            {"database": os.path.join(tmp_path, "unit_test.duckdb")}
        ),
        DuckDBSqlIOManager(database=os.path.join(tmp_path, "unit_test.duckdb")),
    ]
...
def add_one(df: SqlQuery):
    return SqlQuery("SELECT a + 1 AS a, b + 1 AS b FROM $df", df=df)
...
@asset(key_prefix=["my_schema"])
def b_plus_one(b_df: SqlQuery) -> SqlQuery:
    return SqlQuery("SELECT a + 1 AS a, b + 1 AS b FROM $df", df=b_df)
...
def test_duckdb_io_manager_with_schema(tmp_path):
    ...
    @asset
    def my_df_plus_one(my_df: SqlQuery) -> SqlQuery:
        return SqlQuery("SELECT a + 1 AS a, b + 1 AS b FROM $df", df=my_df)
...
def b_plus_one_columns(b_df: SqlQuery) -> SqlQuery:
    return SqlQuery("SELECT * FROM $df", df=b_df)
...
def daily_partitioned(context) -> SqlQuery:
...
def static_partitioned(context) -> SqlQuery:
...
def dynamic_partitioned(context) -> SqlQuery:
    partition = context.asset_partition_key_for_output()
    value = context.op_config["value"]
    return SqlQuery(
        "SELECT * FROM "
...
def test_self_dependent_asset(tmp_path, io_managers):
    ...
    def self_dependent_asset(context, self_dependent_asset: SqlQuery) -> SqlQuery:
        key = context.asset_partition_key_for_output()

        with duckdb.connect(database=os.path.join(tmp_path, "unit_test.duckdb")) as con:
            try:
