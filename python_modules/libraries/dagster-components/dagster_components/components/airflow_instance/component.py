...
class AirflowInstanceComponent(Component, ResolvedFrom[AirflowInstanceModel]):
    ...
    def build_defs(self, context: ComponentLoadContext) -> Definitions:
        from dagster_airlift.core.job_builder import migrate_runs
        ...
        for dag_info in airflow_instance.list_dags():
            if not dag_selector_fn or dag_selector_fn(dag_info):
                dag_ids_to_build.append(dag_info.dag_id)
        jobs = []
        assets = []
