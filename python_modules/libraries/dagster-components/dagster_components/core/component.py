...
def get_entry_points(group: str) -> Sequence[importlib.metadata.EntryPoint]:
...
class ComponentRegistry:
    ...
    def from_entry_point_discovery(cls) -> "ComponentRegistry":
        ...
        for entry_point in get_entry_points(COMPONENTS_ENTRY_POINT_GROUP):
            ...
            normalized_dist_name = entry_point.dist.name.replace("-", "_")
            for component in get_registered_components_in_module(root_module):
                key = f"{normalized_dist_name}.{get_component_name(component)}"
