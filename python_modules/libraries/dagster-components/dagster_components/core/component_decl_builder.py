...
from dagster_components.core.component import ComponentDeclNode, ComponentInstanceKey
...
def path_to_decl_node(path: Path, current_key: ComponentInstanceKey) -> Optional[ComponentDeclNode]:
    ...
    for subpath in path.iterdir():
        component = path_to_decl_node(subpath, current_key.child(subpath.name))
        if component:
