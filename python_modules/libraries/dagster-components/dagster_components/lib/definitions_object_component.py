...
class DefinitionsParamSchema(BaseModel):
    definitions_path: str = "definitions.py"


@component_type(name="definitions_object")
class DefinitionsObjectComponent(Component):
    """Component that allows you to load a full definitions object from a separate file.

    The definitions_path parameter should point to a file that has a `Definitiions` object assigned to the `defs` variable.
    """
    ...
    def load(cls, context: ComponentLoadContext) -> Self:
        ...
        return cls(definitions_path=Path(loaded_params.definitions_path))
