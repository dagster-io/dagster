...
def test_definitions_object_component_with_explict_file() -> None:
    defs = load_test_component_defs("definitions/definitions_object_explicit_file")
    assert {spec.key for spec in defs.get_all_asset_specs()} == {AssetKey("explicit_defs_asset")}
