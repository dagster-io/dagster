...
def test_failing_type_eval_column():
    ...
    output_data = output.event_specific_data.type_check_data  # ty: ignore[unresolved-attribute]
    output_metadata = output_data.metadata  # ty: ignore[unresolved-attribute]
...
def test_failing_type_eval_aggregate():
    ...
    column_const_data = column_const.data  # ty: ignore[unresolved-attribute]
    assert column_const_data["expected"] == {
        "bar": {"all_unique_validator": all_unique_validator.__doc__.strip()}  # ty: ignore[unresolved-attribute]
    }
...
def test_failing_type_eval_dataframe():
    ...
    output_metadata = output_data.metadata  # ty: ignore[unresolved-attribute]
    assert len(output_metadata) == 1
    column_const_data = output_metadata["dataframe-constraint-metadata"].data  # ty: ignore[unresolved-attribute]
...
def test_failing_type_eval_multi_error():
    ...
    agg_metadata = agg_data.data  # ty: ignore[unresolved-attribute]
    ...
    df_metadata = output_metadata["dataframe-constraint-metadata"].data  # ty: ignore[unresolved-attribute]
