...
from dagster._core.events import DagsterEvent, EngineEventData, StepMaterializationData
...
def extract_dagster_metadata(
    context: dg.OpExecutionContext, log: str
) -> Optional[Mapping[str, Any]]:
    context.log.info("Parsing logs...")
    # Get the string between DAGSTER_START and DAGSTER_END
    if "DAGSTER_START" in log and "DAGSTER_END" in log:
        context.log.info("Found dagster metadata in log")
        start_index = log.index("DAGSTER_START") + len("DAGSTER_START")
        end_index = log.index("DAGSTER_END")
        metadata_str = log[start_index:end_index].strip()
        try:
            return json.loads(metadata_str)
        except json.JSONDecodeError as e:
            context.log.error(f"Failed to parse dagster metadata coming from Airflow: {e}")
            return None
    return None
    ...
    def build_op(
        *,
        ...
        def my_op(context: dg.OpExecutionContext):
            ...
            context.log.info(f"Metadata accumulator id: {id(metadata_accumulator)}")
            ...
            while task_state := airflow_instance.get_task_instance(
                ...
                while True:
                    ...
                    for log in parsed_logs:
                        metadata = extract_dagster_metadata(context, log[1])
                        if metadata:
                            metadata_accumulator.update(metadata)
                        context.log.info(log[1])
                time.sleep(1)
                if task_state.state in ["success", "failed"]:
                    break
            for output_name in outs.keys():
                context.add_output_metadata(metadata_accumulator, output_name=output_name)
                        ...
                        for step in steps_to_run:
                            ...
                            for step_event in check.generator(
                                ...
                                if dagster_event.event_type_value == "ASSET_MATERIALIZATION":
                                    job_context.log.info("ALTERING MATERIALIZATION")
                                    additional_metadata = {}
                                    for (
                                        _,
                                        blob,
                                    ) in step_context._metadata_accumulator.per_output_metadata.items():
                                        additional_metadata.update(blob)
                                    job_context.log.info(
                                        f"additional_metadata: {additional_metadata}"
                                    )
                                    full_metadata = {
                                        **dagster_event.step_materialization_data.materialization.metadata,
                                        **additional_metadata,
                                    }
                                    job_context.log.info(f"full_metadata: {full_metadata}")
                                    new_step_materialization_data = StepMaterializationData(
                                        asset_lineage=dagster_event.step_materialization_data.asset_lineage,
                                        materialization=dg.AssetMaterialization(
                                            asset_key=dagster_event.step_materialization_data.materialization.asset_key,
                                            metadata=full_metadata,
                                            description=dagster_event.step_materialization_data.materialization.description,
                                            partition=dagster_event.step_materialization_data.materialization.partition,
                                            tags=dagster_event.step_materialization_data.materialization.tags,
                                        ),
                                    )
                                    dagster_event = DagsterEvent(
                                        event_type_value=dagster_event.event_type_value,
                                        job_name=dagster_event.job_name,
                                        step_handle=dagster_event.step_handle,
                                        node_handle=dagster_event.node_handle,
                                        step_kind_value=dagster_event.step_kind_value,
                                        logging_tags=dagster_event.logging_tags,
                                        event_specific_data=new_step_materialization_data,
...
def migrate_runs(af_instance: AirflowInstance):
