    ...
    for raw_line in stdout.splitlines():
        ...
        for candidate in (line, _LOG_PREFIX.sub("", line)):
            ...
            try:
                ...
                pass
        # Note: This is an aggressive, brute-force fallback to extract JSON embedded within
        # verbose or irregular log streams. It may attempt to parse invalid substrings
        # (e.g., standard log lines with arbitrary brackets), but relies on catching
        # json.JSONDecodeError to safely ignore them.
...
def discover_datasets_fn(
    ...
    if discovery_mode == "source_only":
        get_dagster_logger().warning(
            "Using 'source_only' discovery mode. This is a best-effort static parsing "
            "fallback and may miss datasets dynamically generated by complex imports or wrappers."
        )
