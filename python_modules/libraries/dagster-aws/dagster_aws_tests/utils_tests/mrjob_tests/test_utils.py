...
def test_client_error_status():
    code = 403
    ex = botocore.exceptions.ClientError({"Error": {"HTTPStatusCode": code}}, "foo")  # ty: ignore[invalid-argument-type]
...
def test_is_retriable_client_error():
    ...
    ex = botocore.exceptions.ClientError({"Error": {"HTTPStatusCode": 505}}, "foo")  # ty: ignore[invalid-argument-type]
    assert _is_retriable_client_error(ex)

    ex = botocore.exceptions.ClientError({"Error": {"HTTPStatusCode": 403}}, "foo")  # ty: ignore[invalid-argument-type]
    assert not _is_retriable_client_error(ex)
