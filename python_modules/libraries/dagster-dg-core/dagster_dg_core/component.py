...
from dagster_shared.serdes.objects import DgRegistryKey, DgRegistryObjectSnap
from dagster_shared.serdes.objects.package_entry import DgRegistryManifest, DgRegistryObjectFeature
...
class DgRegistry:
    ...
    def from_dg_context(
        ...
        return DgRegistry(plugin_manifest)

    def __init__(self, plugin_manifest: DgRegistryManifest):
    ...
    @staticmethod
    def empty() -> "DgRegistry":
        return DgRegistry(DgRegistryManifest(modules=[], objects=[]))

    @property
    def modules(self) -> Sequence[str]:
        return self._plugin_manifest.modules

    def module_entries(self, module: str) -> Set[DgRegistryKey]:
    ...
    def get_objects(
        self, package: Optional[str] = None, feature: Optional[DgRegistryObjectFeature] = None
    ) -> Sequence[DgRegistryObjectSnap]:
        return [
            entry
...
def _load_module_registry_objects(
    ...
    objects: list[DgRegistryObjectSnap] = []
    ...
    return DgRegistryManifest(modules=modules, objects=objects)


# Prior to MIN_DAGSTER_COMPONENTS_LIST_PLUGINS_VERSION, the relevant command was named
# "list library" instead of "list plugins". It also output a list[DgRegistryObjectSnap] instead of a
# DgRegistryManifest. This function handles normalizing the output across versions. We can compute a
# DgRegistryManifest from the list[DgRegistryObjectSnap], but it won't include any modules that register an
# entry point but don't expose any plugin objects.
def _fetch_plugin_manifest(entry_points: bool, extra_modules: Sequence[str]) -> DgRegistryManifest:
