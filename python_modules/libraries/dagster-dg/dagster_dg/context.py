...
_DEFAULT_PROJECT_CODE_LOCATION_LOAD_TARGET_MODULE: Final = "definitions"
...
class DgContext:
    ...
    @property
    def code_location_load_target_module_name(self) -> str:
        if not self.config.project:
            raise DgError(
                "`code_location_load_target_module_name` is only available in a Dagster project context"
            )
        return (
            self.config.project.code_location_load_target_module
            or f"{self.root_module_name}.{_DEFAULT_PROJECT_CODE_LOCATION_LOAD_TARGET_MODULE}"
        )

    @cached_property
    def code_location_load_target_path(self) -> Path:
        return self.get_path_for_local_module(self.code_location_load_target_module_name)
