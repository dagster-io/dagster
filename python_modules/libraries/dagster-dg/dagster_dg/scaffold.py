...
def scaffold_project(
    ...
    if use_editable_dagster and use_editable_components_package_only:
        exit_with_error(
            "Cannot specify both --use-editable-dagster and --use-editable-component-package-only."
        )
...
def _get_editable_dagster_from_env() -> str:
    if not os.environ.get("DAGSTER_GIT_REPO_DIR"):
        exit_with_error(
            "The `--use-editable-dagster` and `--use-editable-component-package-only` options, when used as flags,"
