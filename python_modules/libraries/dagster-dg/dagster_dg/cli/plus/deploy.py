...
DEFAULT_STATEDIR_PATH = "/tmp/dg-build-state"
...
def deploy_group(
    ...
    **global_options: object,
    ...
    Each of the stages of the deploy is also available as its own subcommand for additional
    customization:

    - `dg plus deploy start`
    - `dg plus deploy build`
    - `dg plus deploy set-build-output`
    - `dg plus deploy finish-deploy-session`
    """
...
def start_deploy_session_command(
    ...
    deployment: Optional[str],
    deployment_type_str: Optional[str],
    skip_confirmation_prompt: Optional[bool],
    **global_options: object,
) -> None:
