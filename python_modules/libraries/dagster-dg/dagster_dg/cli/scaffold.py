...
def scaffold_github_actions_command(git_root: Optional[Path], **global_options: object) -> None:
    ...
    template = (
        ...
        .replace(
            ...
            str(Path.cwd().relative_to(git_root)),
        )
    )
    ...
    dagster_cloud_yaml_file = dg_context.root_path / "dagster_cloud.yaml"

    dagster_cloud_yaml_contents = _generate_dagster_cloud_yaml_contents(
        dg_context, dg_context.root_path, cli_config, registry_urls
    )
