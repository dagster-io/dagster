...
from .errors import (
    ...
    DagsterSdfWorkspacetNotFoundError,
...
class DagsterSdfWorkspacePreparer(SdfWorkspacePreparer):
    ...
    def _prepare_workspace(self, workspace: "SdfWorkspace") -> None:
        from .sdf_cli_resource import SdfCliResource

        (
            SdfCliResource(workspace_dir=workspace)
            .cli(self._generate_cli_args, target_dir=workspace.target_dir, environment=workspace.environment)
            .wait()
        )
...
class SdfWorkspace(DagsterModel):
    ...
    Args:
        ...
        packaged_workspace_dir (Optional[Union[str, Path]]):
            A directory that will contain a copy of the sdf workspace and the target outputs
            when the workspace have been built. The prepare method will handle syncing
            the project_path to this directory.
            This is useful when the sdf workspace needs to be part of the python package data
    ...
    output_dir: Path
    environment: str
    packaged_workspace_dir: Optional[Path]
    ...
    def __init__(
        ...
        *,
        target_dir: Union[Path, str] = Path(""), # TODO: This should default to `sdftarget` but it will cause a double-up of `sdftarget` in the path
        environment: str = DEFAULT_SDF_WORKSPACE_ENVIRONMENT,
        packaged_workspace_dir: Optional[Union[Path, str]] = None,
        workspace_preparer: SdfWorkspacePreparer = DagsterSdfWorkspacePreparer(),
        sdf_executable: str = SDF_EXECUTABLE,
    ):
        workspace_dir = Path(workspace_dir)
        if not workspace_dir.exists():
            raise DagsterSdfWorkspacetNotFoundError(f"workspace_dir {workspace_dir} does not exist.")

        packaged_workspace_dir = Path(packaged_workspace_dir) if packaged_workspace_dir else None
        if not using_dagster_dev() and packaged_workspace_dir and packaged_workspace_dir.exists():
            workspace_dir = packaged_workspace_dir
        ...
        super().__init__(
            ...
            packaged_workspace_dir=packaged_workspace_dir,
