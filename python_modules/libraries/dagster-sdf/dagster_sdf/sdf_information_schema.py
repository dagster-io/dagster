...
from typing import Optional, Union, Dict, List, Tuple, Set, Mapping
import pandas as pd
import os
from .constants import DEFAULT_SDF_WORKSPACE_ENVIRONMENT, SDF_INFORMATION_SCHEMA_TABLES, SDF_TARGET_DIR
from .errors import DagsterSdfInformationSchemaNotFoundError
from dagster import AssetOut, AssetKey, Nothing, TableSchema, TableColumn
from dagster._core.definitions.metadata import TableMetadataSet, CodeReferencesMetadataSet, CodeReferencesMetadataValue, LocalFileCodeReference, TableColumnConstraints
...
class LazyDataFrameDict:
    ...
    def get(self, key: str) -> pd.DataFrame:
        ...
        if key not in self._loaded:
            self._data[key] = self._load_dataframe(key)
            self._loaded[key] = True
        return self._data[key]

    def _load_dataframe(self, key: str) -> pd.DataFrame:
        # Load the parquet file from disk
        df_list = [pd.read_parquet(f, engine='pyarrow') for f in self.information_schema_dir.joinpath(key).glob('*.parquet')]
        return pd.concat(df_list, ignore_index=True)

    def to_dict(self):
        # Convert to a serializable dictionary format
        return {k: v.to_dict() for k, v in self._data.items() if self._loaded[k]}

    @classmethod
    def from_dict(cls, dict_data):
        instance = cls()
        for key, data in dict_data.items():
            instance._data[key] = pd.DataFrame(data)
            instance._loaded[key] = True
        return instance
...
class SdfInformationSchema(BaseModel):
    ...
    def __init__(
        ...
        **data
        ...
        information_schema_dir = target_dir.joinpath(SDF_TARGET_DIR, environment, "data", "system", "information_schema::sdf")
    ...
    def get_dependencies(self) -> Dict[str, Tuple[List[str], List[str]]]:
        tables = self.information_schema.get("tables")[["table_id", "purpose", "depends_on", "depended_on_by"]]
        dependencies = {}
        for _, row in tables.iterrows():
            if row["purpose"] in ["system", "external_system"]:
                continue
            dependencies[row["table_id"]] = (row["depends_on"].tolist(), row["depended_on_by"].tolist())
        return dependencies

    def get_columns(self) -> Dict[str, List[TableColumn]]:
        columns = self.information_schema.get("columns")[["table_id", "column_id", "classifiers", "column_name", "datatype", "description"]]
    ...
    def get_outs_and_internal_deps(self, io_manager_key: Optional[str]) -> Tuple[Mapping[str, AssetOut], Dict[str, Set[AssetKey]]]:
        outs: Dict[str, AssetOut] = {}
        internal_asset_deps: Dict[str, Set[AssetKey]] = {}
        tables = self.information_schema.get("tables")[["table_id", "purpose", "dialect", "origin", "description", "depends_on", "depended_on_by", "source_locations"]]
        ...
        for _, row in tables.iterrows():
                ...
                if source_location.endswith(".sql"):
                    code_references = CodeReferencesMetadataSet(
                        code_references=CodeReferencesMetadataValue(
                            code_references=[
                                LocalFileCodeReference(file_path=os.fspath(self.workspace_dir.joinpath(source_location)))
                            ]
                        )
                    )
            ...
            internal_asset_deps[dagster_name_fn(row["table_id"])] = set([default_asset_key_fn(dep) for dep in row["depends_on"]])
