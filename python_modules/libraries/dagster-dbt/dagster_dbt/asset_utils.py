...
def get_asset_check_key_for_test(
    ...
    if attached_node_unique_id not in manifest["nodes"] and attached_node_unique_id in manifest.get(
        "disabled", {}
    ):
        ...
        for unique_id, node in manifest["nodes"].items():
            if node.get("resource_type") in ASSET_RESOURCE_TYPES and node["name"] == model_name:
                attached_node_unique_id = unique_id
                break
        else:
