...
def test_scaffold_branch_command_ai_inference_success():
    ...
    with (
        ...
        with (
            ...
            patch(
                "dagster_dg_cli.cli.scaffold.branch.get_branch_name_and_pr_title_from_prompt"
            ) as mock_ai_inference,
        ):
            mock_prompt.return_value = "Add authentication feature"

            mock_ai_inference.return_value = (
            ...
            # Verify user was prompted for their goal
            mock_prompt.assert_called_once_with("What would you like to accomplish?")

            # Verify AI inference was called with user input
            mock_ai_inference.assert_called_once()
            assert mock_ai_inference.call_args[0][1] == "Add authentication feature"
            ...
            expected_git_calls = [
                ...
                (
                    [
                        ...
                        "--allow-empty",
                        "-m",
                        "Initial commit for add-authentication-feature branch",
                    ],
                ),
                (["push", "-u", "origin", "add-authentication-feature"],),
            ]
            actual_git_calls = [call[0] for call in mock_git.call_args_list]
            assert actual_git_calls == expected_git_calls
            mock_gh.assert_called_once_with(
                [
                    ...
                    "Add authentication feature",
                    "--body",
                    "This pull request was generated by the Dagster `dg` CLI for branch 'add-authentication-feature'.",
                ]
            )

            # Check that success message includes AI-generated branch name
