...
def _validate_v3(data_context, dataset, datasource_name, suite, run_id, batch_kwargs):
    batch_kwargs = batch_kwargs or {"batch_data": dataset}
    if "datasource_name" in batch_kwargs or {}:
        data_context.log.warning(
            "`datasource_name` field of `batch_kwargs` will be ignored; use the `datasource_name` "
            "parameter of the solid factory instead."
        )
    batch_kwargs["datasource_name"] = datasource_name
    validator = data_context.get_validator(**batch_kwargs)
    return validator.validate(expectation_suite=suite, run_id=run_id, data_context=data_context)


def _validate_v2(
    data_context, dataset, datasource_name, suite, run_id, batch_kwargs, validation_operator_name
):
    batch_kwargs = batch_kwargs or {"dataset": dataset}
    if "datasource" in batch_kwargs or {}:
        data_context.log.warning(
    ...
    return convert_to_json_serializable(results.list_validation_results())[0]
...
def _get_md(result):
    ...
    rendered_document_content_list = (
        validation_results_page_renderer.render_validation_operator_result(
            validation_operator_result=result
        )
    )
...
def ge_validation_solid_factory(
    ...
    Args:
        ...
        validation_operator_name (Optional[str]): (ge_config_version == 2 only) what validation
                    operator to run  -- defaults to None, which generates an ephemeral validator.
    ...
    def ge_validation_solid(context, dataset):
        ...
        if ge_config_version == 2:
            results = _validate_v2(
                context,
                suite,
                dataset,
                datasource_name,
        ...
        elif ge_config_version == 3:
            results = _validate_v3(context, suite, dataset, datasource_name, run_id, batch_kwargs)
        else:
            check.failed("`ge_config_version` must be one of [2, 3]")
