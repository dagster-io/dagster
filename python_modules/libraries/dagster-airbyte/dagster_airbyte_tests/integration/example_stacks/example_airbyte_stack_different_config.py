...
local_json_destination = AirbyteDestination(
    ...
    destination_configuration={"destination_path": "/local/my_new_destination.json"},
)
...
reconciler_diff_secrets = AirbyteManagedElementReconciler(
    airbyte=airbyte_instance,
    connections=[
        local_json_conn,
    ],
    delete_unmentioned_resources=True,
    mark_secrets_as_changed=True,
)
