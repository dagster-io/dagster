...
class AirflowPersistentDatabase:
    """
    ...
    def from_resource_context(context: InitResourceContext) -> "AirflowPersistentDatabase":
        ...
        os.environ["AIRFLOW__DATABASE__SQL_ALCHEMY_CONN"] = airflow_home_path
        os.makedirs(airflow_home_path, exist_ok=True)
        with Locker(airflow_home_path):
            # reload airflow to ensure new db connection is used
            if is_airflow_2_loaded_in_environment():
                ...
                importlib.reload(airflow)
            # make sure connections are created
            create_airflow_connections(
