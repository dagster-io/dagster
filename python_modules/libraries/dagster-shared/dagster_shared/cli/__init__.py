...
def generate_grpc_server_options(hidden=False) -> Sequence[ClickOption]:
    return [
        click.option(
            "--grpc-port",
            type=click.INT,
...
def generate_workspace_options() -> Sequence[ClickOption]:
    return [
        ...
        *generate_python_pointer_options(allow_multiple=True),
        *generate_grpc_server_options(),
    ]


def workspace_options(f: T_Callable) -> T_Callable:
    return apply_click_params(f, *generate_workspace_options())
