...
from dagster import build_init_resource_context, op
from dagster._utils.test import wrap_op_in_graph_and_execute
from dagster_wandb.resources import WANDB_CLOUD_HOST, WandBResource, wandb_resource
...
def test_legacy_wandb_resource(wandb, api):
...
def test_wandb_resource(wandb, api):
    @op
    def wandb_op(wandb_resource: WandBResource) -> None:
        wandb_resource.login()
        assert wandb_resource.get_api() == api.return_value

    result = wrap_op_in_graph_and_execute(
        wandb_op,
        resources={"wandb_resource": WandBResource(api_key=API_KEY)},
    )
