...
from dagster._core.definitions.metadata import NotebookMetadataValue
from dagster._core.definitions.reconstruct import ReconstructablePipeline
from dagster._core.test_utils import instance_for_test
from dagster._legacy import execute_pipeline, pipeline
from dagster._utils import file_relative_path, safe_tempfile_path
from dagster import AssetKey, reconstructable
...
def get_path(materialization_event):
    for (
        ...
        if isinstance(metadata_entry.entry_data, NotebookMetadataValue):
...
@contextmanager
def exec_for_test(asset_name, env=None, raise_on_error=True, **kwargs):
    result = None
    # assets = [notebook_assets_repo._assets_defs_by_key.get(key) for key in asset_keys]
    recon_pipeline = ReconstructablePipeline.for_module("dagstermill.examples.repository", asset_name)
...
def test_hello_world():
    with exec_for_test("hello_world_asset") as result:
