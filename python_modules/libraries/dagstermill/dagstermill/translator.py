...
class DagsterTranslator(papermill.translators.PythonTranslator):
    ...
    def codify(cls, parameters):  # pylint: disable=arguments-differ
        ...
        assert "__dm_run_dict" in parameters
        ...
        plan_context_args = dict(
            executable_dict=parameters["__dm_executable_dict"],
            run_dict=parameters["__dm_run_dict"],
        ...
        for key in plan_context_args:
            plan_context_args[key] = _seven.json.dumps(plan_context_args[key])

        content = INJECTED_BOILERPLATE.format(plan_context_args=plan_context_args)
