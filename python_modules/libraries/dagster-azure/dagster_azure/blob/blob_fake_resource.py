...
def create_blob_fake_resource(account_name):
    '''Create a mock Blob client for test.'''
    return BlobFakeClient(account_name, 'fake-creds')


class BlobFakeClient(object):
    '''Stateful mock of an Blob client for testing.
    ...
    def get_container_client(self, container):
        return self._containers.setdefault(
            container, BlobFakeContainerClient(self.account_name, container)
        )
...
class BlobFakeBlobClient(object):
    ...
    def download_blob(self):
        if self.contents is None:
            raise ResourceNotFoundError("File does not exist!")
        return BlobFakeBlobDownloader(contents=self.contents)


class BlobFakeBlobDownloader(object):
