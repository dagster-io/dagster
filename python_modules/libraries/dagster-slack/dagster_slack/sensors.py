...
def _get_text_fn(using_graph_job_op: bool) -> Callable[[PipelineFailureSensorContext], str]:
    def _text_fn(context: PipelineFailureSensorContext) -> str:
        if using_graph_job_op:
            return "\n".join(
                [
...
def make_slack_on_pipeline_failure_sensor(
    channel: str,
    slack_token: str,
    text_fn: Callable[[PipelineFailureSensorContext], str] = _get_text_fn(using_graph_job_op=False),
...
def make_slack_on_run_failure_sensor(
    channel: str,
    slack_token: str,
    text_fn: Callable[[RunFailureSensorContext], str] = _get_text_fn(using_graph_job_op=False),
