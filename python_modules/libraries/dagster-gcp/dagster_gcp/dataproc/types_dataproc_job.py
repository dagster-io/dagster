...
Substate = Enum(
    ...
    enum_values=[
        ...
        EnumValue(
            'SUBMITTED', description='The Job is submitted to the agent.Applies to RUNNING state.'
        ),
        EnumValue(
            'QUEUED',
            description='The Job has been received and is awaiting execution (it may be waiting for a condition to be met). See the "details" field for the reason for the delay.Applies to RUNNING state.',
        ),
        EnumValue(
            'STALE_STATUS',
            description='The agent-reported status is out of date, which may be caused by a loss of communication between the agent and Cloud Dataproc. If the agent does not send a timely update, the job will fail.Applies to RUNNING state.',
        ),
    ],
)
