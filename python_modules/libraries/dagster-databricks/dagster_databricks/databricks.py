...
class DatabricksClient:
    ...
    def submit_run(self, *args, **kwargs):
        ...
        """
        return self.client.jobs.submit_run(*args, **kwargs)
...
class DatabricksJobRunner:
    """
    Submits jobs created by users using Dagster config to Databricks, and monitors their progress.
    """
    ...
    def submit_run(self, run_config, task):
        ...
        response = self._client.submit_run(**config)
        return response["run_id"]

    def wait_for_run_to_complete(self, log, run_id):
        """
        Wait for a Databricks run to complete.
        """
        check.int_param(run_id, 'run_id')
        log.info("Waiting for Databricks run %s to complete..." % run_id)
        start = time.time()
        while True:
            log.debug("Waiting %.1f seconds..." % self.poll_interval_sec)
            time.sleep(self.poll_interval_sec)
            run_state = self._client.get_run_state(run_id)
            if run_state.is_terminal():
                ...
                log.info("Run %s in state %s" % (run_id, run_state))
            if time.time() - start > self.max_wait_time_sec:
                raise DatabricksError(
                    "Job run {} took more than {}s to complete; failing".format(
                        run_id, self.max_wait_time_sec
                    )
                )

    def retrieve_logs_for_run_id(self, log, run_id):
        """
        Retrieve the stdout and stderr logs for a run.
        """
        api_client = self._client.client
        run = api_client.jobs.get_run(run_id)
        ...
        if log_config is None:
            log.warn(
                'Logs not configured for cluster {cluster} used for run {run}'.format(
                    cluster=cluster["cluster_id"], run=run_id
                )
            )
            return None
        if "s3" in log_config:
    ...
    def wait_for_dbfs_logs(
        ...
        while num_attempts <= waiter_max_attempts:
            try:
                logs = self._client.read_file(path)
