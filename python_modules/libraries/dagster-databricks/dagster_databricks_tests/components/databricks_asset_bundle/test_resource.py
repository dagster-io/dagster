...
def make_list_payload(
    job_ids: list[int], has_more: bool, next_token: str | None = None
) -> dict:
...
async def test_fetch_single_job_retries_on_429_then_succeeds():
    ...
    session.get = MagicMock(
        side_effect=lambda url, **kw: mock_resp(200, make_list_payload([42], has_more=False))
        if "list" in str(url)
...
async def test_fetch_single_job_exhausts_retries_and_raises():
    ...
    session = MagicMock()
    session.get = MagicMock(
        side_effect=lambda url, **kw: mock_resp(200, make_list_payload([1], has_more=False))
        if "list" in str(url)
        else rl_resp
    )
    session.__aenter__ = AsyncMock(return_value=session)
...
async def test_no_deadlock_when_all_slots_rate_limited():
    ...
    session = MagicMock()
    session.get = MagicMock(
        side_effect=lambda url, **kw: mock_resp(200, make_list_payload(job_ids, has_more=False))
        if "list" in str(url)
        else detail_side(url, **kw)
    )
    session.__aenter__ = AsyncMock(return_value=session)
...
def test_parse_raw_jobs_skips_none():
    assert DatabricksWorkspace._parse_raw_jobs([None, None]) == []  # noqa: SLF001
