...
const AssetGraphExplorerWithData = ({
  ...
  const {flagUseNewAssetOverviewPage} = useFeatureFlags();
  ...
  const panels = useSplitPanelInteractions({resetTo: 65});

  const explorer = (
    <SplitPanelContainer
      key="explorer"
      axis={flagUseNewAssetOverviewPage ? 'vertical' : 'horizontal'}
      firstInitialPercent={flagUseNewAssetOverviewPage ? 65 : 70}
      firstMinSize={flagUseNewAssetOverviewPage ? 0 : 400}
      ...
      second={
        selectedGraphNodes.length === 1 && selectedGraphNodes[0] ? (
          <RightInfoPanel
            onWheel={(e) => {
              if (!flagUseNewAssetOverviewPage) {
                return;
              }
              ...
              if (e.deltaY > 0 && !panels.isBottomExpanded) {
                panels.expandBottomPanel();
                e.preventDefault();
              ...
              if (e.deltaY < 0 && panels.isBottomExpanded && scrollerEl.scrollTop === 0) {
                e.preventDefault();
                panels.resetPanels();
              }
            }}
          >
            <ErrorBoundary region="asset sidebar" resetErrorOnChange={[selectedGraphNodes[0].id]}>
              {flagUseNewAssetOverviewPage ? (
                <AssetView
                  assetKey={selectedGraphNodes[0].assetKey}
                  trayToggleElement={
                    <ExpandCollapseButton
                      expanded={panels.isBottomExpanded}
                      onExpand={panels.expandBottomPanel}
                      onCollapse={() => {
                        panels.resetPanels();
