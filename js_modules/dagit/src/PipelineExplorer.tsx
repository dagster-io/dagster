...
> {
  ...
  public render() {
    ...
    const visibleSolids = filterSolidsByQuery(solids, visibleSolidsQuery);

    let selectedHandle = this.props.selectedHandle;
    if (
      selectedHandle &&
      !visibleSolids.some(s => s.name === selectedHandle!.solid.name)
    ) {
        ...
        <SplitPanelChildren
          ...
          left={
            <>
              ...
              {visibleSolids.length === 0 && !visibleSolidsQuery.length && (
                <LargeDAGNotice />
              )}
              <PipelineGraph
                ...
                highlightedSolids={visibleSolids.filter(s =>
