import type {SidebarsConfig} from '@docusaurus/plugin-content-docs';
const sidebars: SidebarsConfig = {
  docs: [
    {
      type: 'category',
      label: 'Getting Started',
      collapsed: false,
      items: [
        'intro',
        'getting-started/quickstart',
        'getting-started/installation',
        'getting-started/glossary',
        'getting-started/concepts',
      ],
    },
    {
      type: 'category',
      label: 'ETL pipeline tutorial',
      collapsed: false,
      link: {type: 'doc', id: 'etl-pipeline-tutorial/index'},
      // we use individual items instead of type: autogenerated
      // for this category, because autogenerated makes the next link
      // on the last page in this category point to the first page in
      // the category
      items: [
        'etl-pipeline-tutorial/create-and-materialize-assets',
        'etl-pipeline-tutorial/create-and-materialize-a-downstream-asset',
        'etl-pipeline-tutorial/ensure-data-quality-with-asset-checks',
        'etl-pipeline-tutorial/create-and-materialize-partitioned-asset',
        'etl-pipeline-tutorial/automate-your-pipeline',
        'etl-pipeline-tutorial/create-a-sensor-asset',
        'etl-pipeline-tutorial/refactor-your-project'
      ],
    },
    {
      type: 'category',
      label: 'Build',
      collapsed: false,
      link: {type: 'doc', id: 'guides/build/index'},
      items: [
        {
          type: 'autogenerated',
          dirName: 'guides/build',
        },
      ],
    },
    {
      type: 'category',
      label: 'Automate',
      collapsed: false,
      link: {type: 'doc', id: 'guides/automate/index'},
      items: [
        {
          type: 'autogenerated',
          dirName: 'guides/automate',
        },
      ],
    },
    {
      type: 'category',
      label: 'Operate',
      collapsed: false,
      link: {type: 'doc', id: 'guides/operate/index'},
      items: [
        {
          type: 'autogenerated',
          dirName: 'guides/operate',
        },
      ],
    },
    {
      type: 'category',
      label: 'Monitor & debug',
      collapsed: false,
      link: {type: 'doc', id: 'guides/monitor/index'},
      items: [
        {
          type: 'autogenerated',
          dirName: 'guides/monitor',
        },
      ],
    },
    {
      type: 'category',
      label: 'Test',
      collapsed: false,
      link: {type: 'doc', id: 'guides/test/index'},
      items: [
        {
          type: 'autogenerated',
          dirName: 'guides/test',
        },
      ],
    },
    {
      type: 'category',
      label: 'Deploy',
      link: {type: 'doc', id: 'guides/deploy/index'},
      collapsed: false,
      items: [
        {
          type: 'autogenerated',
          dirName: 'guides/deploy',
        },
      ],
    },
    {
      type: 'category',
      label: 'Labs',
      link: {type: 'doc', id: 'guides/labs/index'},
      collapsed: false,
      items: [
        {
          type: 'autogenerated',
          dirName: 'guides/labs',
        },
      ],
    },
    {
      type: 'category',
      label: 'About',
      collapsed: false,
      items: [
        {
          type: 'autogenerated',
          dirName: 'about',
        },
      ],
    },
  ],
  dagsterPlus: [
    'dagster-plus/index',
    'dagster-plus/getting-started',
    {
      type: 'category',
      label: 'Features',
      collapsible: false,
      items: [
        {
          type: 'autogenerated',
          dirName: 'dagster-plus/features',
        },
      ],
    },
    {
      type: 'category',
      label: 'Deployment',
      collapsible: false,
      items: [
        {
          type: 'autogenerated',
          dirName: 'dagster-plus/deployment',
        },
      ],
    },
  ],
  examples: [
    {
      type: 'category',
      label: 'Examples',
      collapsed: false,
      link: {type: 'doc', id: 'examples/index'},
      items: [
        {
          type: 'autogenerated',
          dirName: 'examples',
        },
      ],
    },
  ],
  integrations: [
    {
      type: 'category',
      label: 'Guides',
      collapsed: false,
      items: ['integrations/guides/multi-asset-integration'],
    },
    {
      type: 'category',
      label: 'Libraries',
      collapsible: false,
      link: {type: 'doc', id: 'integrations/libraries/index'},
      items: [
        {
          type: 'autogenerated',
          dirName: 'integrations/libraries',
        },
      ],
    },
  ],
  migration: [
    'migration',
    {
      type: 'category',
      label: 'Guides',
      collapsible: false,
      items: [
        {
          type: 'autogenerated',
          dirName: 'migration'
        }
      ]
    }
  ],
  api: [
    {
      type: 'category',
      label: 'Overview',
      collapsed: false,
      items: [
        'api/index',
        {
          type: 'autogenerated',
          dirName: 'api/api-lifecycle',
        },
      ],
    },
    {
      type: 'category',
      label: 'Dagster core',
      collapsed: false,
      link: {type: 'doc', id: 'api/dagster/index'},
      items: [
        {
          type: 'autogenerated',
          dirName: 'api/dagster',
        },
      ],
    },
    {
      type: 'category',
      label: 'Dagster libraries',
      collapsed: false,
      link: {type: 'doc', id: 'api/libraries/index'},
      items: [
        {
          type: 'autogenerated',
          dirName: 'api/libraries',
        },
      ],
    },
  ],
};

export default sidebars;
