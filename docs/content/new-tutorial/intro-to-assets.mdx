...
```python file=/intro_tutorial/basics/hackernews/hackernews.py startafter=start_topstories_asset_marker endbefore=end_topstories_asset_marker dagimage=images/asset-screenshots/intro_tutorial/basics/hackernews/hackernews.svg
import pandas as pd
import requests
from dagster import MetadataValue, OpExecutionContext, asset

@asset(group_name="hackernews", compute_kind="HackerNews API")
def hackernews_topstories(context, hackernews_topstory_ids) -> pd.DataFrame:
    results = []
...
We'll walk you through using Dagster's UI to view and materialize assets later in the tutorial. Let's consider the following DAG which shows the lineage between three assets:
