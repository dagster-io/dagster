...
Runs instigated from the Dagit UI, the scheduler, or `dagster job launch` are "launched" in Dagster. This is a distinct operation from "executing" a pipeline using the `execute_pipeline` python API or the CLI `execute` command. A 'launch' operation allocates computational resources (e.g. a process, a container, a kubernetes pod, etc) to carry out a run execution and then instigates the execution.
