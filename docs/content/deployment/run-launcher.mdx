...
The simplest run launcher is the built-in run launcher, <PyObject module="dagster._core.launcher" object="DefaultRunLauncher" />. This run launcher spawns a new process per run on the same node as the pipeline's repository location.
