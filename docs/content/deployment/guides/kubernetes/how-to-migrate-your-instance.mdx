...
- **Check the [Dagster migration guide](/migration).** Use the guide to verify any additional steps you may need to take to upgrade your Dagster version.
...
First, run [`helm upgrade`](https://helm.sh/docs/helm/helm_upgrade/) with your desired Dagster chart version and Helm values.
...
Next, run a Kubernetes Job with the `dagster instance migrate` command:

```shell
dagster instance migrate
```

Check if the migration succeeded by inspecting the resulting Pod of the Job:
...
Lastly, use the following commands to scale the Dagster webserver and daemon deployments back up:
