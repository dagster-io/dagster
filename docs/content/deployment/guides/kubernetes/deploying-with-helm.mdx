...
- Have `kuberctl` installed and configured with your desired Kubernetes cluster
- Understand [the basics of Helm](https://helm.sh/docs/)
- Have Helm 3 installed
- Have Docker installed **if creating your own user code images**
...
**Note**: If you're using a chart version **before** 0.11.13, you'll also need to update the tags of the Dagster-provided images to match the chart version. After 0.11.13, this is automatically done for you.

---

## Deployment architecture

<!-- ![Default Dagster-Kubernetes deployment architecture](/images/deploying/dagster-kubernetes-defalut-architecture.png) -->
...
A [code location server](/concepts/code-locations) runs a gRPC server and responds to Dagit's requests for information, such as:
...
Here, we can specify the configuration of the Kubernetes deployment that creates the gRPC server for Dagit and the daemon to access the user code. The gRPC server is created through the arguments passed to `dagsterApiGrpcArgs`, which expects a list of arguments for [`dagster api grpc`](/concepts/code-locations/workspace-files#running-your-own-grpc-server).
