...
- A `pod_per_op_job` job that uses the [`k8s_job_executor`](/\_apidocs/libraries/dagster-k8s#dagster_k8s.k8s_job_executor) to run each op in its own pod. NOTE: this job uses the [`s3_pickle_io_manager`](/\_apidocs/libraries/dagster-aws#dagster_aws.s3.s3\_pickle_io_manager), which requires [setting AWS credentials](/deployment/guides/aws#using-s3-for-io-management).
...
Next, try running a job. Navigate to <http://127.0.0.1:8080>, then the [**Launchpad** tab](http://127.0.0.1:8080/workspace/example_repo@k8s-example-user-code-1/jobs/example_job/launchpad), and click **Launch Run**.
