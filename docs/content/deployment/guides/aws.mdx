...
This example demonstrates how you can use `docker-compose` to manage a Dagster deployment on ECS. It includes a Dagit container for loading and launching pipelines, a `dagster-daemon` container for managing a run queue and submitting runs from schedules and sensors, a postgres container for persistent storage, and a container with user pipeline code. The Dagster instance uses DockerRunLauncher to launch each run in its own container.
