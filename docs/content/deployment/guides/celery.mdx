...
- **A running broker like [RabbitMQ](https://www.rabbitmq.com/).** This is required to run the Celery executor.
...
To run the Celery executor, you'll need a running broker like [RabbitMQ](https://www.rabbitmq.com/). With Docker, this is something like the following:
...
First, you'll ensure that appropriate persistent run and event log storage, e.g., `PostgresRunStorage` and `PostgresEventLogStorage`, are configured on your [Dagster instance](/deployment/dagster-instance) (via `dagster.yaml`). This allows the webserver and workers to communicate information about the run and events with each other.

Add a block like the following to your project's `dagster.yaml`:
...
**The same instance config must be present in the webserver's environment and in the workers' environments.** Refer to the [Dagster instance](/deployment/dagster-instance) documentation for more information.
...
<!-- At least one team is running dagster-celery using SQS as the broker, and we intend to expand the scope of broker/backend matrix testing, probably starting with the Postgres backend. -->
