...
This service must have access to your `dagster.yaml` and `workspace.yaml` files, just like the Dagit container. You can also configure your code server so that your code can be updated and deployed separately in its own container, without needing to redeploy the other Dagster services. To enable this setup, include a container exposing a code server at a port and add that port to your `workspace.yaml` file.
...
# Checkout and install dagster libraries needed to run a code server
# exposing your code to dagit and dagster-daemon, and to load
# the DagsterInstance
...
load_from:
  # Each entry here corresponds to a container that exposes a code server.
  - code_server:
