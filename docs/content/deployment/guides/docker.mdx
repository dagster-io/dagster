...
This service must have access to your `dagster.yaml` and `workspace.yaml` files, just like the Dagit container. You can also configure your [code location server](/concepts/code-locations/workspace-files) so that your code can be updated and deployed separately in its own container running a gRPC server, without needing to redeploy the other Dagster services. To enable this setup, include a container exposing a gRPC server at a port and add that port in your `workspace.yaml` file.
