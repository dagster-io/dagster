...
In the [Default Deployment](#default-deployment) section, we go over the simplest way to get started:

- [Default Quickstart →](#default-quickstart)
- [Default Architecture and Walkthrough →](#default-architecture)
- [Default Components →](#default-components)

In the [Advanced Deployment](#advanced-deployment) section, we build off of the [Default Deployment](#default-deployment)
and add more sophisticated features including solid-level isolation and queuing mechanisms.

- [Advanced Quickstart: Running Dagster in Production →](#advanced-quickstart)
- [Advanced Architecture and Walkthrough →](#advanced-architecture)
- [Advanced Components →](#advanced-components)
- [Helm Configuration Options →](#advanced-configuration)
...
In this section, we cover the simplest deployment option from [quickstart](#default-quickstart),
to [architecture](#default-architecture), to individual [components](#default-components).

### Default Quickstart

The following steps are how to get up and running with the [default architecture](#default-architecture). This example requires a Kubernetes cluster.
...
In this section, we build off of the [Default Deployment](/deployment/guides/kubernetes#default-deployment) and add more sophisticated features.

### Advanced Quickstart

The following steps set up the [advanced architecture](/deployment/guides/kubernetes#advanced-architecture). This example requires a Kubernetes cluster and a persistent file store (we use S3
in the example). Read about [GCP documentation here](/deployment/guides/gcp).

**Step 1-4:** [Follow Step 1-4 in the Default Quickstart](/deployment/guides/kubernetes#default-quickstart)
...
module="dagster_celery_k8s" object="CeleryK8sRunLauncher" /> and <PyObject module="dagster_celery_k8s" object="celery_k8s_job_executor" /> ([See Configuration](/deployment/guides/kubernetes#advanced-configuration)).

#### Daemon

The Daemon supports sensors, run queues, and schedules. See [Daemon overview](/deployment/dagster-daemon) for more information.
...
Building off the [prior description](/deployment/guides/kubernetes#dagit), but instead, it is configured with the <PyObject module="dagster_celery_k8s" object="CeleryK8sRunLauncher" />.

#### Database

Same as the [prior description](/deployment/guides/kubernetes#database).
...
Building off the [prior description](/deployment/guides/kubernetes#run-worker), the main difference in this deployment is
...
Same as the [prior description](/deployment/guides/kubernetes#user-code-deployment).
...
The [Run Launcher](/deployment/run-launcher) is responsible for allocating the computational resources for run execution.

<PyObject module="dagster_k8s" object="K8sRunLauncher" /> \[Default]:

The `K8sRunLauncher` is used in the [Default Deployment](/deployment/guides/kubernetes#default-deployment) example. It creates a new Kubernetes Job per pipeline run.

<PyObject module="dagster_celery_k8s" object="CeleryK8sRunLauncher" />:

The `CeleryK8sRunLauncher` is used in the [Advanced Deployment](/deployment/guides/kubernetes#advanced-deployment) example. It creates a new Kubernetes Job per step
...
As seen in the [Advanced Deployment](/deployment/guides/kubernetes#advanced-deployment) example, this creates a Celery task for each Step, allowing for step-level isolation and concurrency control.
...
Daemon scheduler \[Default]: Uses [dagster-native scheduler](/deployment/dagster-daemon). The daemon is also responsible for running sensors,
and managing the queue of runs. This is used in both the [Default Deployment](/deployment/guides/kubernetes#default-deployment) and
[Advanced Deployment](/deployment/guides/kubernetes#advanced-deployment) examples and is our recommended approach.
...
- `dagster-instance`: Defines the [Instance YAML](/deployment/dagster-instance) for all
