...
To start the `dagster-daemon` process locally, launch the following command in the same folder as your [`workspace.yaml`](/concepts/repositories-workspaces/workspaces#defining-a-workspace) file and keep the process running:
