...
This command launches both the [Dagster webserver](/concepts/webserver/ui) and the Dagster daemon, allowing you to start a full local deployment of Dagster from the command line. Refer to the [Running Dagster locally guide](/guides/running-dagster-locally) for more information about `dagster dev`.
