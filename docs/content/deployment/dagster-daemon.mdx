...
To start the `dagster-daemon` process locally, launch the following command in the same folder as your [`workspace.yaml`](/concepts/repositories-workspaces/workspaces#understanding-workspace-files) file and keep the process running:
