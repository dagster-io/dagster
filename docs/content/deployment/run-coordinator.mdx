...
When you submit a run from the Dagster UI or the Dagster command line, it’s first sent to the run coordinator, which applies any limits or prioritization policies before eventually sending it to the [run launcher](https://docs.dagster.io/deployment/run-launcher) to be launched.
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        Dagster UI will immediately launch the run from the Dagit process. Similarly,
        scheduled runs will immediately launch from the scheduler process.
      </td>
    </tr>
    <tr>
      <td>
