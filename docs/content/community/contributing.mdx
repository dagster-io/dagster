...
You can find the corresponding Python file at `dagster/examples/docs_snippets/docs/snippets/path/to/file.py`. The code included in each snippet is the code in the file between the `# start_marker` and `# end_marker` comments. 

If you change a code snippet, run the following from the `docs` directory:
