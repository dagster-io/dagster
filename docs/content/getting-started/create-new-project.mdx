...
If you're just starting a new Dagster project, the CLI command `dagster new-project` will generate a
...
a Dagster project called `PROJECT_NAME`:
...
can also be a repository location, like in the [Hello World](/getting-started#writing-a-pipeline) example.
...
The Dagit process automatically uses the file `workspace.yaml` to find your repository
...
Once you have created a new Dagster repository with the CLI command `dagster new-project`, you can find
tests in `PROJECT_NAME_tests`, where `PROJECT_NAME` is the name of your project. You can run all of
...
As you create Dagster solids and pipelines, add tests in `PROJECT_NAME_tests/` to check that your
