...
In this guide, we'll walk you through managing Dagster+ alert policies using the [`dagster-cloud` CLI](/dagster-plus/managing-deployments/dagster-cloud-cli). Alert policies apply to a full deployment (ex: `prod`), not Branch Deployments.
