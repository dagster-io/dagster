---
title: "Managing alert policies with the dagster-plus CLI | Dagster Docs"
description: "Create and configure Dagster+ alerts using the dagster-plus CLI."

platform_type: "cloud"
feature: "alerting"
---

# Managing alert policies with the dagster-plus CLI

<ApplicableDagsterProduct />

In this guide, we'll walk you through managing Dagster+ alert policies using the [`dagster-plus` CLI](/dagster-plus/managing-deployments/dagster-plus-cli). Alert policies apply to a full deployment (ex: `prod`), not Branch Deployments.
...
Alert policies are defined in a YAML file and then, using the `dagster-plus` CLI, set for a deployment. The YAML file should contain a single top-level key (`alert_policies`) and contain a list of alert policy objects. For example:
...
- **To install the `dagster-plus` CLI.** Refer to the [`dagster-plus` CLI reference](/dagster-plus/managing-deployments/dagster-plus-cli#installing-the-cli) for instructions.
