...
In this guide, we'll walk you through managing Dagster Cloud alert policies using the [`dagster-cloud` CLI](/dagster-plus/managing-deployments/dagster-cloud-cli). Alert policies apply to a full deployment (ex: `prod`), not Branch Deployments.

**Don't want to work in code?** You can also define and set alert policies using the [Dagster Cloud UI](/dagster-plus/managing-deployments/alerts/managing-alerts-in-ui).
...
- **To install the `dagster-cloud` CLI.** Refer to the [`dagster-cloud` CLI reference](/dagster-plus/managing-deployments/dagster-cloud-cli#installing-the-cli) for instructions.
