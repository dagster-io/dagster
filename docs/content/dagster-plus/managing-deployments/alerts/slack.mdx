...
Creating an alert policy can be done using the Dagster+ UI or the `dagster-plus` CLI and a YAML file.
...
Then, use the `dagster-plus` CLI to set the alert policies for the deployment:
...
Refer to the [Managing alerts using the `dagster-plus` CLI](/dagster-plus/managing-deployments/alerts/managing-alerts-cli) guide for more information.
