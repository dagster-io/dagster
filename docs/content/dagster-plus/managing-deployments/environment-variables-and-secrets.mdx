...
Refer to the [Setting environment variables for Dagster Cloud agents guide](/dagster-plus/managing-deployments/setting-environment-variables-dagster-cloud-agents) for more info.
...
[Dagster Cloud](/dagster-plus) provides a set of built-in, automatically populated environment variables, such as the name of a deployment or details about a branch deployment commit, that can be used to modify behavior based on environment.
