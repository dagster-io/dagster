...
Deployment settings can be configured in the Dagster+ interface or using the `dagster-plus` CLI. Refer to the [deployment settings reference](/dagster-plus/managing-deployments/deployment-settings-reference) for more info about individual settings.
...
<Note>
<strong>Note:</strong> <code>dagster-plus</code> 0.13.14 or later must be installed to run the CLI. Agent and/or job code doesn't need to be upgraded.
</Note>
...
```shell
dagster-plus deployment settings set-from-file my-settings.yaml
```
...
```shell
dagster-plus deployment settings get
```
