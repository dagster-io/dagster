...
To mask error messages in a Dagster Cloud Deployment, set the environment variable `DAGSTER_REDACT_USER_CODE_ERRORS` equal to `1` using the [**Environment variables** page](/dagster-plus/managing-deployments/environment-variables-and-secrets) in the UI:
