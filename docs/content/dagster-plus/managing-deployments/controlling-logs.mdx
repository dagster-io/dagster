---
title: "Managing Compute Logs and Error Messages in Dagster+ | Dagster Docs"
---

# Managing compute logs and error messages in Dagster+
...
By default, Dagster+ ingests [structured event logs and compute logs](/concepts/logging#log-types) from runs and surfaces error messages from [code locations](/concepts/code-locations) in the UI.
...
To mask error messages in a Dagster+ Deployment, set the environment variable `DAGSTER_REDACT_USER_CODE_ERRORS` equal to `1` using the [**Environment variables** page](/dagster-plus/managing-deployments/environment-variables-and-secrets) in the UI:
