---
title: Using Branch Deployments (CI) with the dagster-plus CLI | Dagster+
description: Develop and test in the cloud.
---

# Using Branch Deployments (CI) with the dagster-plus CLI

<Note>This guide is applicable to Dagster+.</Note>

In this guide, we'll walk you through setting up Continuous Integration (CI) [Branch Deployments](/dagster-plus/managing-deployments/branch-deployments) with a general continuous integration (CI) or git platform, using the [`dagster-plus` CLI](/dagster-plus/managing-deployments/dagster-plus-cli).
...
## Step 3: Create a branch deployment using the dagster-plus CLI

Next, you'll create a branch deployment using the `dagster-plus` CLI. When the state of a branch or merge request is updated, Dagster+ first expects these steps to occur:
...
1. Use the `dagster-plus` CLI to create a branch deployment associated with the branch, as follows:
