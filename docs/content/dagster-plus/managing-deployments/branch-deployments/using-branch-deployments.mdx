...
In this guide, we'll walk you through setting up Continuous Integration (CI) [Branch Deployments](/dagster-plus/managing-deployments/branch-deployments) with a general continuous integration (CI) or git platform, using the [`dagster-cloud` CLI](/dagster-plus/managing-deployments/dagster-cloud-cli).
