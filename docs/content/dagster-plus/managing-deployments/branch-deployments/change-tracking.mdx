...
In this example, the `returns` asset has a **Changed in branch** label indicating it has changed tags.
...
**In the pull request**, we added a new tag to `fruits_in_stock`:
...
Change Tracking can detect when an asset's metadata has changed, whether it's been added or removed.

<TabGroup>
<TabItem name="Asset in the Dagster UI">

In this example, the `returns` asset has a **Changed in branch** label indicating it has changed metadata.
...
**In the pull request**, we update the value of one of the pieces of metadata on `products`:
