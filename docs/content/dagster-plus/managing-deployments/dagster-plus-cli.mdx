---
title: "Using the dagster-plus CLI | Dagster Docs"
---

# Using the dagster-plus CLI

<Note>This guide is applicable to Dagster+.</Note>

The `dagster-plus` CLI is a command-line toolkit designed to work with Dagster+.

In this guide, we'll cover how to install and configure the `dagster-plus` CLI, get help, and use some helpful environment variables and CLI options.
...
Optionally, you can install command-line completions to make using the `dagster-plus` CLI easier.

To have the CLI install these completions to your shell, run:

```shell
dagster-plus --install-completion
```

To print out the completion for copying or manual installation:

```shell
dagster-plus --show-completion
```
...
The recommended way to set up your CLI's config for long-term use is through the configuration file, located by default at `~/.dagster_plus_cli/config`.
...
When prompted, you can specify a default deployment. If specified, a deployment won't be required in subsequent `dagster-plus` commands. The default deployment for a new Dagster+ organization is `prod`.
...
- **Organization** - Your organization name as it appears in your Dagster+ URL. For example, if your Dagster+ instance is `https://hooli.dagster.plus/`, this would be `hooli`.
- **User token** - The user token.
- **Default deployment** - **Optional**. A default deployment. If specified, a deployment won't be required in subsequent `dagster-plus` commands. The default deployment for a new Dagster+ organization is `prod`.
...
```shell
dagster-plus config set-deployment <deployment_name>
```
