...
In this guide, we'll cover the requirements for Dagster code, how to add code in Dagster+, and how to add code using the `dagster-plus` CLI.
...
- **Must run in an environment where the `dagster` and [`dagster-plus`](/dagster-plus/managing-deployments/dagster-plus-cli) 0.13.2 or later Python packages are installed.**
...
## Managing code locations using the dagster-plus CLI

You can also use the `dagster-plus workspace` CLI commands to:
...
```shell
dagster-plus workspace add-location test_location \
    --image dagster/dagster-cloud-examples:latest \
...
```shell
dagster-plus workspace add-location --help
```
...
```shell
dagster-plus workspace delete-location <LOCATION_NAME>
```

### Syncing the workspace

You can also keep the YAML configuration for your entire workspace in a [`dagster_cloud.yaml`](/dagster-plus/managing-deployments/dagster-cloud-yaml) file and use the `dagster-plus sync` command to reconcile the workspace config in Dagster+ with that local file.
...
```shell
dagster-plus workspace sync -w dagster_cloud.yaml
```
