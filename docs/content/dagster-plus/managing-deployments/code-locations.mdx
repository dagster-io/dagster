...
A [`dagster_cloud.yaml` file](/dagster-plus/managing-deployments/dagster-cloud-yaml) is recommended to configure code locations for Dagster Cloud.
...
- **Must run in an environment where the `dagster` and [`dagster-cloud`](/dagster-plus/managing-deployments/dagster-cloud-cli) 0.13.2 or later Python packages are installed.**
...
These commands perform the same underlying operations as editing your code locations in the **Deployment** tab in Dagster Cloud. Refer to the [dagster-cloud CLI guide](/dagster-plus/managing-deployments/dagster-cloud-cli) for more info and installation instructions.
...
You can also keep the YAML configuration for your entire workspace in a [`dagster_cloud.yaml`](/dagster-plus/managing-deployments/dagster-cloud-yaml) file and use the `dagster-cloud sync` command to reconcile the workspace config in Dagster Cloud with that local file.
