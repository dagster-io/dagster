...
# Integrating BigQuery + dbt with Dagster+ Insights (Experimental)

<Note>
  This feature is considered <strong>experimental</strong>.
</Note>

BigQuery costs can be integrated into the Dagster Insights UI. The [`dagster-plus`](https://pypi.org/project/dagster-plus/) package contains utilities for capturing and submitting BigQuery cost metrics about data operations to Dagster+.
...
- To install the following libraries:
  ...
  ```shell
  pip install dagster dagster-plus dagster-dbt dagster-gcp
  ```
