...
If you use dbt to materialize tables in BigQuery, use this guide to integrate BigQuery cost metrics into the Insights UI. For instructions on integrating direct BigQuery queries, see [Integrating Direct BigQuery Usage with Dagster Cloud Insights](/dagster-plus/insights/integrating-bigquery).
