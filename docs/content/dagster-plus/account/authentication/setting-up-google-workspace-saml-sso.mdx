---
title: "Setting up Google Workspace SSO for Dagster+ | Dagster Docs"
...
3. On the **Service provider details** page:
   ...
   2. Check the **Signed Response** box.
      ...
      The page should look similar to the image below. In this example, the organization's name is `hooli` and the Dagster+ domain is `https://hooli.dagster.plus`:
...
4. After you've downloaded the SAML metadata file, upload it to Dagster+ using the `dagster-plus` CLI:

   ```shell
   dagster-plus organization settings saml upload-identity-provider-metadata <the_path/to/metadata> \
      --api-token=<user_token> \
      --url https://<your_organization_name>.dagster.plus
   ```
...
In the Google Workspace portal, click on the **Dagster+ icon**. If successful, you'll be automatically signed into your Dagster+ organization.
