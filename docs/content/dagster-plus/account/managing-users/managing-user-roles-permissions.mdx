...
There are two key concepts for Dagster+ user roles. 
1. **Scope** - Some roles only apply to Organization settings (e.g., "Create Teams"). While some only apply in the context of a deployment (e.g., "Launch and Cancel Backfills").
2. **Default vs Custom** - As of {Launch Date Goes Here} we enable the creation of custom roles by defining both a Role name and set of permissions. There will still be the "default" heirarchical roles you're used to, but you you will also be able to define custom roles. 

### User role enforcement

All roles are enforced both in Dagster+ and the [GraphQL API](/concepts/webserver/graphql).
...
In order to create or edit custom roles:
1. Navigate to the **Organization Settings** page.
2. Click the Roles tab

Then to create a role:
1. Click the **Create new role** button 
2. Select a name and icon
3. Provide a brief description
4. Choose if deployment or organization type depending on where you want your role applied
5. Select the permissions you want to apply to the role. Note - you can base your role off an existing role for ease of creation

To edit a role:
1. Click the edit button next to the role you want to edit
2. Make your changes
3. Save your changes
4. You will see a confirmation dialog including the changes that have been made
---

## Deleting custom roles
Deleting custom roles is similar to editting. At the bottom of the edit dialog you'll see a button to "Delete role". Two things to note when deleting a role:
1. Deleting a role is a permanent action and cannot be undone
2. If the role is currently assigned to any users, you will need to reassign them to a different role before you can delete the role
