...
<Note>
  Once scaffolded, add <code>dagster-plus</code> as a dependency in your{" "}
  <code>setup.py</code> file.
</Note>

Next, install the [`dagster-plus` CLI](/dagster-plus/managing-deployments/dagster-plus-cli) and log in to your org. **Note**: The CLI requires a recent version of Python 3 and Docker.
...
You can also configure the `dagster-plus` tool noninteractively; see [the CLI docs](/dagster-plus/managing-deployments/dagster-plus-cli#environment-variables-and-cli-options) for more information.

Add `dagster-plus` as a dependency to `my-dagster-project/setup.py`:

```python
install_requires=[
    "dagster",
    "dagster-plus",    # add this line
    "dagster-airbyte",
```
...
```shell
dagster-plus serverless deploy-python-executable ./my-dagster-project \
...
- **With the CLI**: Add the `--python-version` CLI argument to the deploy command to specify the registry path to the desired base image:
  ...
  ```shell
  dagster-plus serverless deploy-python-executable --location-name=my_location --python-version=3.9
  ```
...
2. The `dagster-plus[serverless]` module installed in the image.
...
1. Build your Docker image using `docker build` or your usual Docker toolchain. Ensure the `dagster-plus[serverless]` dependency is included. You can do this by adding the following to your `Dockerfile`:

   ```shell
   RUN pip install "dagster-plus[serverless]"
   ```
...
2. Upload your Docker image to Dagster+ using the `upload-base-image` command. Note that this command prints out the tag used in Dagster+ to identify your image:
   ...
   ```shell
   $ dagster-plus serverless upload-base-image local-image:tag

   ...
   To use the uploaded image run: dagster-plus deploy-python-executable ... --base-image-tag=sha256_518ad2f92b078c63c60e89f0310f13f19d3a1c7ea9e1976d67d59fcb7040d0d6
   ```
...
3. To use a Docker image you have published to Dagster+, use the `--base-image-tag` tag printed out by the above command.
   ...
   - **With the CLI**: Add the `--base-image-tag` CLI argument to the deploy command:
     ...
     ```shell
     dagster-plus serverless deploy-python-executable \
       --location-name example \
       --package-name assets_modern_data_stack \
...
- **With the CLI**: Add the `--base-image` CLI argument to the deploy command to specify the registry path to the desired base image:
  ...
  ```shell
  dagster-plus serverless deploy --location-name=my_location --base-image=my_base_image:latest
  ```
