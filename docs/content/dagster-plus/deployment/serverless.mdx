...
Next, install the [dagster-cloud CLI](/dagster-plus/managing-deployments/dagster-cloud-cli) and log in to your org. **Note**: The CLI requires a recent version of Python 3 and Docker.
...
You can also configure the `dagster-cloud` tool noninteractively; see [the CLI docs](/dagster-plus/managing-deployments/dagster-cloud-cli#environment-variables-and-cli-options) for more information.
...
Often you'll need to securely access secrets from your jobs. Dagster Cloud supports several methods for adding secrets - refer to the [Dagster Cloud environment variables and secrets documentation](/dagster-plus/managing-deployments/environment-variables-and-secrets) for more info.
