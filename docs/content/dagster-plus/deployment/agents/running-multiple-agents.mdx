...
The `isolated_agents` option can be set as per-deployment configuration on the `dagster.yaml` file used by your agent. See the [ECS configuration reference](/dagster-plus/deployment/agents/amazon-ecs/configuration-reference#per-deployment-configuration) guide for more information.
...
<Note>Agent queues are a Dagster+ Pro feature.</Note>
...
First, set an agent queue for the code location in your [`dagster_cloud.yaml`](/dagster-plus/managing-deployments/dagster-cloud-yaml):
