...
This reference describes the various configuration options Dagster Cloud currently supports for [Amazon ECS agents](/dagster-plus/deployment/agents/amazon-ecs).

---

## Per-location configuration

When [adding a code location](/dagster-plus/managing-deployments/code-locations) to Dagster Cloud with an Amazon ECS agent, you can use the `container_context` key on the location configuration to add additional ECS-specific configuration that will be applied to any ECS tasks associated with that code location.
