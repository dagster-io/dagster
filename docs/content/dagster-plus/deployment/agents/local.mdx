...
- **To install the [`dagster-plus` CLI](/dagster-plus/managing-deployments/dagster-plus-cli)**. This should be in the same environment where the agent will run.
...
3. In the file, fill in the following:
   ...
   - `agent_token` - Add the agent token you created in [Step 1](#step-1-generate-a-dagster-agent-token). This specifies that the agent will launch work in local processes, reading the API URL and authentication token from environment variables.
...
```shell
dagster-plus agent run ~/dagster_home/
```
