...
<TabItem name="dagster-plus CLI">

##### dagster-plus CLI

<Note>
  <strong>Heads up!</strong> Using the <code>dagster-plus</code> CLI requires a
...
1. To install the `dagster-plus` CLI, run:
...
3. After you've successfully authenticated, run the following to deploy your code to Dagster+:
   ...
   ```shell
   dagster-plus serverless deploy \
...
- **Another CI/CD provider** - If you're not using GitHub, you can configure CI/CD using the `dagster-plus` CLI.
...
For continuous integration using a CI/CD provider other than GitHub, your system should use the `dagster-plus ci` subcommand to deploy code locations to Dagster+.
...
2. Run the configuration check:
   ...
   ```shell
   dagster-plus ci check --project-dir=.
   ```
...
3. Initialize the build session:
   ...
   ```shell
   dagster-plus ci init --project-dir=.
   ```
...
4. Build and upload Docker images for your code locations.
   ...
   The Docker image should contain a Python environment with `dagster`, `dagster-plus`, and your code. For reference, see the [example Dockerfile](https://github.com/dagster-io/dagster-cloud-hybrid-quickstart/blob/main/Dockerfile) in our quickstart repository. The example uses `pip install .` to install the code including the dependencies specified in [`setup.py`](https://github.com/dagster-io/dagster-cloud-hybrid-quickstart/blob/main/setup.py).
...
5. Update the build session with the Docker image tag. For each code location you want to deploy, run the following command passing the `IMAGE_TAG` used in the previous step:
   ...
   ```shell
   dagster-plus ci set-build-output --location-name=code-location-a --image-tag=IMAGE_TAG
   ```
