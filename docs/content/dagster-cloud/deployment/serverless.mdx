...
Dagster Cloud Serverless packages your code as [PEX](https://pex.readthedocs.io) files and deploys them on Docker images. Using PEX files significantly reduces the time to deploy since it does not require building a new Docker image and provisioning a new container for every code change. Many apps will work fine with the default Dagster Cloud Serverless setup. However, some apps may need to make changes to the runtime environment, either to include data files, use a different OS, different Python version, or install some native dependencies. You can customize the runtime environment using various methods described below.
...
```
      - name: Build and deploy Python executable
        ...
        with:
          ...
          python_version: "3.8"   # Change this to the desired Python version
        env:
...
2. Upload your Docker image to Dagster Cloud using the `upload-base-image` command. Note this command prints out the tag used in Dagster Cloud to identify your image:
