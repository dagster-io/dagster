...
<<<<<<< HEAD
- If you're using our GitHub integration, you can [create secrets in GitHub](https://docs.github.com/en/actions/security-guides/encrypted-secrets) and then pass those secrets to Dagster by modifying the serverless deploy step in your repository's GitHub action script.

  ```shell
  # .github/workflows/deploy.yml AND .github/workflows/branch_deployments.yml
  ...
  jobs:
    ...
    ...
    ...
      ...
      - name: Build and deploy to Dagster Cloud serverless
            uses: dagster-io/dagster-cloud-action/actions/serverless_prod_deploy
            with:
  ...
  ```
=======
- If you're using our GitHub integration, you can create secrets in GitHub and then pass those secrets to Dagster by modifying the serverless deploy step in your repository's GitHub action script.
  ...
  ...
    ...
    - name: Build and deploy to Dagster Cloud serverless
          ...
          with:
            ...
            # env_vars: ${{ toJson(secrets) }}
```
>>>>>>> 26bdce8b8b0a53b1cb11f429ea7c8bede1bcb8cf
...
While Python dependencies will be installed via `setup.py` or `requirements.txt`, occasionally a project will require additional dependencies. For example, dbt dependencies that are installed with `dbt deps`.

- If you're using our GitHub integration, you can create a shell script called `dagster_cloud_pre_install.sh` that will be run before the Python dependencies are installed or you can create a shell script called `dagster_cloud_post_install.sh` that will be run after the Python dependencies are installed. The scripts should be executable, e.g. run `chmod +x dagster_cloud_post_install.sh` prior to committing the file to your Git repo.

- If you're using the CLI directly, you should create a custom base image with your dependencies and include it by specifying `--base-image` in the `deploy serverless` command.
