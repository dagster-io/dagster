...
If your agent is at version 0.14.9 or higher, when [adding a code location](/guides/adding-code) to Dagster Cloud with a Kubernetes agent, you can use the `container_context` key on the location configuration to add additional Kubernetes-specific configuration. The following example illustrates the available fields:
