...
<ReferenceTable>
  ...
  How long (in seconds) to poll against ECS API endpoints. If your ECS tasks take an unusually long time to start and result in timeouts, you can increase this setting above the default.
  <ul>
  ...
  How long (in seconds) to continue polling if an ECS API endpoint fails (because the ECS API is eventually consistent).
  <ul>
