...
You can write compute logs to an AWS S3 bucket without requiring your S3 credentials by configuring the [dagster_aws.s3.compute_log_manager](https://docs.dagster.io/\_apidocs/libraries/dagster-aws#dagster_aws.s3.S3ComputeLogManager) module.
...
To show the URL to the compute log files in the Dagster Cloud UI, set the `show_url_only` parameter to `true`.
