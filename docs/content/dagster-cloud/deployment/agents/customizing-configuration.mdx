...
## Streaming compute logs to AWS S3

You can set up streaming compute logs to AWS S3 by configuring the [dagster_aws.s3.compute_log_manager](https://docs.dagster.io/\_apidocs/libraries/dagster-aws#dagster_aws.s3.S3ComputeLogManager) module. Note that by setting the `show_url_only` parameter to `true`, only the URL of the log file will be shown in the Dagster Cloud UI instead of fetching and displaying the full content.
