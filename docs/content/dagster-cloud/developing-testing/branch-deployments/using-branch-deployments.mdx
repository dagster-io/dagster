...
In this guide, we'll walk you through setting up [Branch Deployments](/dagster-cloud/developing-testing/branch-deployments) with a general continuous integration (CI) or git platform, using the [`dagster-cloud` CLI](/dagster-cloud/developing-testing/dagster-cloud-cli).
...
**If you use GitHub for version control or want Dagster to automate branch deployments**, check out the dedicated [Branch deployments with GitHub guide](/dagster-cloud/developing-testing/branch-deployments/using-branch-deployments-with-github).
...
- The ability to run a new agent in your infrastructure, if using [Hybrid Deployment](/dagster-cloud/deployment/hybrid)
...
<Note>
  ...
  <a href="#step-3-create-a-branch-deployment">Skip to the next step</a>.
</Note>
...
## Step 3: Create a branch deployment

Next, you'll create a branch deployment using the `dagster-cloud` CLI.

When the state of a branch or merge request is updated, Dagster Cloud first expects these steps to occur:

1. A new image containing your code and requirements is built on the branch. Refer to the [Dagster code requirements guide](/dagster-cloud/developing-testing/code-locations).
...
**Note**: If using [Serverless Deployments](/dagster-cloud/deployment/serverless), Dagster Cloud handles the above on your behalf.
