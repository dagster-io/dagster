...
>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Environment variables are defined in the agent's configuration files (
        <code>dagster.yaml</code>, <code>dagster_cloud.yaml</code>). Values are
        pulled from your user cluster, meaning they don't pass through Dagster
        Cloud.
      </td>
    </tr>
    <tr>
...
- **Local** - Variables with this scope will be included when [downloading variables to a local `.env` file](TODO)
...
4. In the window that displays, fill in the following:
   ...
   - In **Deployment Scope**, select the deployment(s) where the variable should be accessible:
     ...
     - **Local** - If selected, the variable will be included when [exporting environment variables to a local `.env` file](#exporting-variables-to-an-env-file)
...
**Note**: The methods described below will make the environment variables available for all code locations in your Dagster Cloud full deployment. To configure environment variables on a code location basis, refer to the [configuration reference for your agent type](/dagster-cloud/deployment/agents).
...
In `env_vars`, specify the environment variables as keys (`SNOWFLAKE_PASSWORD`) or key-value pairs (`SNOWFLAKE_USERNAME=dev`). If only `KEY` is provided, the value will be pulled from the local (agent's) environment.
