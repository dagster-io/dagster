  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        pulled from storage and decrypted at run time.
      </td>
      <td>
    ...
    <tr>
      ...
      <td>
        ...
        >
          ...
          <li>If using Hybrid, agent uses Dagster version 1.0.17 or later</li>
          <li>
            <a href="/dagster-cloud/account/managing-users">
              Editor, Admin, or Organization Admin user permissions
            </a>{" "}
            in Dagster Cloud
          </li>
        </ul>
      ...
      <td>
        ...
        >
          ...
          >
            Ability to configure the agent
          </li>
        </ul>
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Scoped by deployment (full and Branch Deployments) and code location
      </td>
      <td>Scoped by code location</td>
    </tr>
  </tbody>
</table>
...
When creating a new environment variable in the Dagster Cloud UI, its scope will default to all deployments (full and [Branch Deployments](/dagster-cloud/developing-testing/branch-deployments)) and all [code locations](/dagster-cloud/developing-testing/code-locations)

When assigning scope to an environment variable, keep the following in mind:

- **For Branch Deployments**, variables will be read-only. Environment variables must be managed in the Branch Deployment's parent full deployment, which will usually be `prod`.

- **You can create multiple instances of the same environment variable and assign different values and scopes.** Each instance of the variable can then be scoped to a deployment and code location (or locations). This approach can be useful for parameterizing behavior by environment without needing to modify your application code.
  ...
  In this example, `SNOWFLAKE_PASSWORD=production_password` when in the `prod` deployment, while `SNOWFLAKE_PASSWORD=testing_password` in a Branch Deployment

**Note**: If an environment variable isn't accessible at run time, [a `Variable not set` error will surface in Dagster Cloud](/guides/dagster/using-environment-variables-and-secrets).
...
[Built-in (system) Dagster Cloud environment variables](#built-in-environment-variables) are reserved and therefore unavailable for use. [An error will surface in Dagster Cloud](#reserved-name) if a built-in variable name is used.

---

## Managing environment variables

The simplest way to manage environment variables is to use Dagster Cloud's built-in secrets manager, which allows you to create and manage environment variables right in the UI.

<TabGroup>
  <TabItem name="Using the Dagster Cloud UI">
...
4. In the window that displays, fill in the following:
   ...
   - In **Deployment Scope**, select the deployment(s) where the variable should be accessible. Refer to the [Scope](#scope) section for more info.
...
<TabItem name="Using agent configuration (Hybrid)">
...
**Note**: After the agent's configuration is updated, make sure to [re-deploy the code location](/dagster-cloud/developing-testing/code-locations) to ensure the changes take effect.
...
- [Maximum size exceeded](#maximum-size-exceeded)
- [Maximum characters exceeded](#maximum-characters-exceeded)
- [Invalid name](#invalid-name)
- [Maximum number of variables reached](#maximum-number-of-variables-reached)

### Reserved name

```shell
[variable] is a reserved environment variable name.
```
...
>
  <tbody>
    <tr>
      ...
      <td>
        The name of the environment variable is is reserved. Dagster reserves
        the names of <a href="#built-in-variables">built-in (system)</a>{" "}
        variables.
      </td>
    </tr>
    <tr>
      ...
      <td>
        Change the variable's name to a name that isn't currently{" "}
...
>
  <tbody>
    <tr>
      ...
      <td>
        The value of the environment variable is greater than 4KB in size.
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>Reduce the size of the value to less than the maximum of 4KB.</td>
    </tr>
  </tbody>
</table>
...
>
  <tbody>
    <tr>
      ...
      <td>
        The name of the environment variable exceeds the maximum length of 512
        characters.
      </td>
    </tr>
    <tr>
      ...
      <td>Reduce the number of characters in the variable's name.</td>
    </tr>
  </tbody>
</table>
...
```shell
Deployment [deployment_name] has reached the maximum of 1,000 environment variables.
```
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        been reached. New variables cannot be added.
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        Remove any unneeded variables to reduce the total below the maximum,
