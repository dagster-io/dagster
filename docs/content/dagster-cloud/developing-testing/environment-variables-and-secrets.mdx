...
- **While viewing a Branch Deployment in Dagster Cloud**, variables will be read-only. Environment variables must be managed in the Branch Deployment's parent full deployment, which will usually be `prod`.
...
The simplest way to manage environment variables is to use Dagster Cloud's built-in environment variables manager which allows you to create and manage environment variables right in the UI.
