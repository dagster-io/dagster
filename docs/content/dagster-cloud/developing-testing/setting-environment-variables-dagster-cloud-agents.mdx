...
- **On a per-code location basis**, which involves modifying the `dagster.yaml` file. **Note**: This approach is functionally the same as [setting environment variables using the Dagster Cloud UI](/dagster-cloud/developing-testing/environment-variables-and-secrets). Values will pass through Dagster Cloud.
...
<Note>
  <strong>Editor, Admin, or Organization Admin</strong> permissions are required
  to modify and redeploy code locations in Dagster Cloud.
</Note>
...
**Note**: This approach is functionally the same as [setting environment variables using the Dagster Cloud UI](/dagster-cloud/developing-testing/environment-variables-and-secrets).
