...
- **Import an existing project** - Import an existing Dagster project using our GitHub app or the [dagster-cloud CLI](/dagster-cloud/developing-testing/dagster-cloud-cli). **Note**: If using the GitHub app, Dagster Cloud will automatically set up CI/CD for you.
...
<TabGroup>
<TabItem name="GitHub app">

**Using the GitHub app**

Using the GitHub app to import an existing project also sets up CI/CD for you.

Before you get started, note that your project must have a `dagster_cloud.yaml` file in order for Dagster Cloud to deploy it. If your project doesn't contain this file, you'll need to add it before proceeding.

In the root of your project, create a `dagster_cloud.yaml` with the following code:
...
To complete this step using the CLI, you can use your own Dagster code or the [Dagster Cloud Serverless Quickstart](https://github.com/dagster-io/dagster-cloud-serverless-quickstart). The quickstart is a template with everything you need to get started using Serverless in Dagster Cloud, including CI/CD configuration and the required `dagster_cloud.yaml` file.

- **If using the template**, click [**Use this template** in the repository](https://github.com/dagster-io/dagster-cloud-serverless-quickstart). Finish setting up the repository on GitHub and clone it to your local environment.

- **If not using the template**, add `dagster-cloud` as a dependency in `setup.py`. [Click here for an example](https://github.com/dagster-io/dagster-cloud/blob/main/dagster-cloud-examples/setup.py). This is already done for you if using the Hybrid quickstart template.
...
3. Add the code location in Dagster Cloud
...
Once Dagster Cloud receives an agent heartbeat, click **Continue**.
...
1. Copy the workflow files (`.github/workflows`) from the [Hybrid quickstart repository](https://github.com/dagster-io/dagster-cloud-hybrid-quickstart/tree/main/.github/workflows) and add them to your repository.
...
3. In the repository, set the necessary GitHub action secrets:

   - `DAGSTER_CLOUD_API_TOKEN` - A Dagster Cloud agent token. Refer to the [agent tokens documentation](/dagster-cloud/account/managing-user-agent-tokens#managing-agent-tokens) for more info.
...
**Step 3: Add a Dagster Cloud code location**
...
**Note**: Creating Branch Deployments using the CLI requires some additional steps. Refer to the [Branch Deployments with the dagster-cloud CLI guide](/dagster-cloud/developing-testing/branch-deployments/using-branch-deployments) for more info.
