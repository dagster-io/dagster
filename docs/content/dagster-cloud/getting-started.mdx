...
1. Initializate - check out your code and validate `dagster_cloud.yaml`.
...
Ensure that you have created a `dagster_cloud.yaml` file as described in [the quickstart]. For detailed documentation about this file see the [dagster_cloud.yaml reference](/dagster-cloud/managing-deployments/dagster-cloud-yaml). The following steps are typically implemented in the CI/CD workflow:

1. Set the build environment variables:

   - `DAGSTER_CLOUD_ORGANIZATION` - [required] the name of your organization in Dagster Cloud.
   - `DAGSTER_CLOUD_API_TOKEN` - [required] a token created in Dagster Cloud. This is a sensitive value and should be stored as a CI/CD secret, if possible.
   - `DAGSTER_BUILD_STATEDIR` - [required] path to a blank or non-existent temporary directory on the build machine. This directory is used to store local state during the build.
...
4. Build and upload Docker images for your code locations.
   ...
   The upload step is specific to your Docker container registry. The only requirement is that the registry you upload to must match the registry specified in `dagster_cloud.yaml`. It is a good idea to use a unique image tag for each build. You can build one image per code location or a shared image for multiple code locations. As an example image tag, you can use the git commit SHA:
...
6. Deploy to Dagster Cloud:
   ...
   This command updates the code locations in Dagster Cloud.
