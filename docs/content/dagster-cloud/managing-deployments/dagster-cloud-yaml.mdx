...
The `location_name` key that specifies the name of the code location and a **mandatory** section in the `dagster_cloud.yaml`. The location name will always be paired with a [code source](#code-source)
...
This is a **mandatory** section in the `dagster_cloud.yaml` file. This specifies the code sources that should be deployed to Dagster Cloud.
...
The build registry is applicable to hybrid deployments only. Setting a build directory is useful if your `setup.py` or `requirements.txt` is in a subdirectory instead of the project root. This is common if you have multiple Python packages within a single Dagster project.
...
<ReferenceTable>
  ...
  <ReferenceTableItem propertyName="build.registry">
    ...
    push your code location to in the registry
    <ul>
      <li>
...
For Dagster Cloud Serverless deployments, you can specify a different Python version by [following these instructions](https://docs.dagster.io/dagster-cloud/deployment/serverless#using-a-different-python-version).
