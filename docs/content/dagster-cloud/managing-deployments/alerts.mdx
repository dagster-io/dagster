---
title: "Alerts in Dagster Cloud | Dagster Docs"
description: "Create and configure alerts in Dagster Cloud."
---

# Alerts in Dagster Cloud
...
Additionally, alert policies are configured on a **per-deployment basis**. For example, asset alerts configured in a `prod` deployment are only applicable to assets in that deployment.
...
7. In the **Notification service** section, select how the alert will be delivered. Refer to the [Supported notification services](#supported-notification-services) for information about setting up these services.
