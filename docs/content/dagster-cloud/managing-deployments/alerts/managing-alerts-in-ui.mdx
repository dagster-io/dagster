---
title: "Managing alert policies in Dagster Cloud | Dagster Docs"
...
In this guide, we'll walk you through managing alert policies in Dagster Cloud.

**Prefer to work in code?** You can also define and set alert policies using the [`dagster-cloud` CLI](/dagster-cloud/managing-deployments/alerts/managing-alerts-cli).
...
5. In the **Create alert policy** window, select an [alert type](/dagster-cloud/managing-deployments/alerts#alert-policy-types) and enter a name and description for the alert.
...
7. In the **Notification service** section, select how the alert will be delivered. Refer to the [Supported notification services](/dagster-cloud/managing-deployments/alerts#supported-notification-services) section for information about setting up these services.
...
1. Click **⌄ (More actions) > Edit policies**, which is next to the **Create alert policy** button:
   ...
   <!-- ![Highlighted More Actions menu in the Dagster Cloud UI](/images/dagster-cloud/alerts/more-actions-menu.png) -->

   <Image
   alt="Highlighted More Actions menu in the Dagster Cloud UI"
