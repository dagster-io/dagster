...
A [dagster_cloud.yaml](/dagster-cloud/managing-deployments/dagster-cloud-yaml) is recommended to configure code locations for Dagster Cloud.
...
You can also keep the YAML configuration for your entire workspace in a [`dagster_cloud.yaml`](docs/content/dagster-cloud/managing-deployments/dagster-cloud-yaml) file and use the `dagster-cloud sync` command to reconcile the workspace config in Dagster Cloud with that local file.
