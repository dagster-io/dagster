...
6. In the **Create alert policy** window, fill in the following:
   ...
   - **Notification service** - Select the service for the alert policy:
     ...
     To disconnect your Slack workspace: the Dagster Cloud Slack app will need to be removed from your Slack workspace. A Slack workspace admin can do that by going to the Dagster Cloud app's `Configuration` settings and selecting the **Remove App** button. After doing this, refresh the Alerts page and the button will show the **Connect to Slack** option again.
