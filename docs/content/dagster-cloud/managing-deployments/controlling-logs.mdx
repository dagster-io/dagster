...
Dagster's compute logs are handled by the configured [`ComputeLogManager`](/\_apidocs/internals#dagster.\_core.storage.compute_log_manager.ComputeLogManager). By default, Dagster Cloud utilizes the `CloudComputeLogManager` which stores logs in a Dagster Cloud-managed Amazon S3 bucket, but you can customize this behavior to store logs in a destination of your choice.
...
You can configure the `NoOpComputeLogManager` in your [`dagster.yaml` file](/dagster-cloud/deployment/agents/customizing-configuration):
...
By default, Dagster Cloud surfaces error messages from your code locations in the UI, including when runs fail, sensors or schedules throw an exception, or code locations fail to load. You can mask these error messages in the case that their contents are sensitive.

To mask error messages in a Dagster Cloud Deployment, set the environment variable `DAGSTER_REDACT_USER_CODE_ERRORS` equal to `1` using the [**Environment variables** page](/dagster-cloud/managing-deployments/environment-variables-and-secrets) in the UI:
