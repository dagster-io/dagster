...
By default, Dagster Cloud ingests [structured event logs and compute logs](/concepts/logging#log-types) from runs and surfaces error messages from [code locations](/concepts/code-locations#code-locations) in the Dagster Cloud UI.
...
You can configure the `S3ComputeLogManager` in your [`dagster.yaml` file](https://docs.dagster.io/dagster-cloud/deployment/agents/customizing-configuration):
...
If you are using Helm to deploy thew Kubernetes agent, you can provide the following configuration in your `values.yaml` file:
...
Once set, error messages from your code locations will be masked in the Dagster Cloud UI, and will provide a unique error ID that you can use to look up the error message in your own logs.
