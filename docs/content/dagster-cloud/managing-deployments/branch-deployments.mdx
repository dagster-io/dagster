...
Note that if an asset is marked as having changed dependencies, it means that either an upstream asset has been added or removed as a dependency. It does **not** mean that the upstream dependency has changed or that the upstream dependency has new data.
...
And the pull request updates the `WeeklyPartitionsDefinition` to start one year earlier:
