...
- Branch deployments can show which assets have been changed in the associated pull request
- For each branch deployment, the assets will be compared to the assets in the main deployment and changes will be indicated in the asset graph
- Filtering - can filter to show which assets have changed, or the assets with a particular type of change
- What changes are detected?
  - section for each change type - code version, dependencies, partitions definition, new, (tags, metadata once those branches land)
  - covers what the change means, how to use it, anything else relevant
- Tips and tricks - using the global asset graph to see all assets affected by a change

Branch deployments have a feature called Change Tracking. Change Tracking makes it easy to see which assets in your pull request have changed by marking them in the UI.

#### New assets

Change Tracking will indicate if an asset is new in the Branch Deployment. INSERT IMAGE
...
The Branch Deployment will mark `customers` as changed. INSERT IMAGE
...
The Branch Deployment will mark `returns` as having changed dependencies. Note that if an asset is marked as having changed dependencies, it means that either an upstream asset has been added or removed as a dependency. It does not mean that the upstream dependency has changed or that the upstream dependency has new data. TODO wording
...
The Branch Deployment will mark `weekly_orders` as having a changed partitions definition. INSERT IMAGE
