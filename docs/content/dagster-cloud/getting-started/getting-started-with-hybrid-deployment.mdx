...
There are two options for deploying your code. For most Hybrid deployments, with the exception of those using a local agent, you will need to create a Docker image containing your code and then add a code location to Dagster Cloud that references the image. Typically, this build and deploy is done through CI/CD. It is also possible to deploy a code location manually, and this manual process would be used with the local agent.

- For continunous integration with GitHub, begin by copying the GitHub action deploy workflow in [this repository](https://github.com/dagster-io/dagster-cloud-hybrid-quickstart). You will need to set a variety of secrets as outlined in the action, and should uncomment the step that is relevant to your Docker container registry. You will also want to copy the example Dockerfile. Finally, you should copy the `dagster_cloud.yaml` file and update it accordingly.  If you are not using GitHub but are using a different CI/CD tool, you will want to follow the same steps outlined in the GitHub action, but use the `dagster-cloud` CLI to deploy your code location. Refer to the [Adding a code location guide](/dagster-cloud/developing-testing/code-locations) for more info.
...
## Step 5: Set up Branch Deployments

In Step 3, you most likely setup CI/CD to automatically deploy code changes in your main branch to Dagster Cloud. Dagster Cloud also supports continuous integration testing through Branch Deployments. For example, you may wish to create an isolated deployment in Dagster Cloud when a pull request is created in your Git repository.
