...
- For continunous integration with GitHub, begin by copying the GitHub action deploy workflow in [this repository](https://github.com/dagster-io/dagster-cloud-hybrid-quickstart). You will need to set a variety of secrets as outlined in the action, and should uncomment the step that is relevant to your Docker container registry. If you are not using GitHub but are using a different CI/CD tool, you will want to follow the same steps outlined in the GitHub action, but use the `dagster-cloud` CLI to deploy your code location. Refer to the [Adding a code location guide](/dagster-cloud/developing-testing/code-locations) for more info.
