...
- **Serverless** - Serverless Deployment is a fully-managed version of Dagster Cloud that allows you to run your Dagster jobs without spinning up any infrastructure. Refer to the [Serverless deployment documentation](/dagster-cloud/deployment/serverless) for more info.

  **Note**: This deployment type works best with jobs that aren't computationally expensive.

- **Hybrid** - With a Hybrid Deployment, your Dagster code is executed in your infrastructure. Refer to the [Hybrid deployment documentation](/dagster-cloud/deployment/hybrid) for more info.

  This deployment type requires **the ability to create and configure an agent in your infrastructure**. The agent runs in your infrastructure, executes your code, and streams metadata about code execution over HTTPS back to Dagster Cloud.
