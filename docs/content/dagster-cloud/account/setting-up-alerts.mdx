...
In this example, we'll configure a Slack notification to trigger whenever a run of a job succeeds or fails. This job, named `sales_job`, has a tag of `{"team", "sales"}`:
...
In the alert policies YAML file, we'll define a policy that listens for jobs with the `{"team", "sales"}` tag to succeed or fail. When this occurs, a notification will be sent to the `sales-notification` channel in the `hooli` workspace:
...
In this example, we'll configure an email alert when a job fails. This job, named `important_job`, has a tag of `{"level": "critical"}`:
...
In the alert policies YAML file, we'll define a policy that listens for jobs with the `{"level": "critical"}` tag to fail. When this occurs, an email notification will be sent to `richard.hendricks@hooli.com` and `nelson.bighetti@hooli.com`:
