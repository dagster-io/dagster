  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <strong>User role permissions</strong> tab for more info.
      </td>
    </tr>
    <tr>
      <td>
...
With the exception of the **Organization Admin** role, user roles are set on a per-deployment basis and enforced both in Dagster Cloud and the GraphQL API. Organization Admins have access to the entire organization, including all [full deployments](/dagster-cloud/developing-testing/managing-deployments), [code locations](/dagster-cloud/developing-testing/code-locations), and [Branch Deployments](/dagster-cloud/developing-testing/branch-deployments).
...
7. In the window that displays, select the appropriate [user role](#understanding-user-roles) for each deployment. **Note**: For Admins, you can only set permissions for deployments where you're an Admin:
   ...
   4. **Enterprise only**: To set permissions for individual [code locations](#code-location-permissions) in a deployment:
      ...
      4. Click **Save**.
8. Repeat step 6 for each deployment.
9. **Optional**: To change the user's permissions for branch deployments:
