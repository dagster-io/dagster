...
- **If using a SAML-based solution like Okta**, users must be assigned to the Dagster app in the Identity Provider (IdP) to be able to log in to Dagster Cloud. Refer to the [SSO setup guides](/dagster-cloud/account/authentication) for setup instructions for each of our supported IdP solutions.
...
**Note**: When determining a user's level of access, Dagster Cloud will use the **most permissive** role assigned to the user between all of their team memberships and any individual role grants, whether at the deployment or code location level. Refer to the [Managing user roles and permissions](/dagster-cloud/account/managing-users/managing-user-roles-permissions#applying-role-overrides) guide for more info.
