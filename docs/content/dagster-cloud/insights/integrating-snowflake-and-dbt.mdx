...
This allows you to add a comment to every query recorded in Snowflake's `query_history` table. The comments will contain the dbt invocation ID and unique ID. This allows Insights to attribute cost metrics in Snowflake to the corresponding Dagster concepts of jobs and assets.
