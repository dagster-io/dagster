...
This allows you to add a comment to every query recorded in BigQuery's `INFORMATION_SCHEMA.JOBS` table. The comments will contain the dbt invocation ID and unique ID.

Once the pipeline runs, BigQuery credits will be visible in the **Insights** tab in the Dagster UI:
