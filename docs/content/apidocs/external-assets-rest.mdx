...
- Report an asset check evaluation for an external asset to Dagster
...
Authentication is required only if requests are being made against a Dagster+ instance. To authenticate, provide a valid [Dagster+ user token](/dagster-plus/account/managing-user-agent-tokens#managing-user-tokens) using the `Dagster-Cloud-Api-Token` header, for example:
