...
- **For materializable assets**, which are assets materialized by Dagster, Dagster can infer the asset's last update time using the asset's latest materialization timestamp. In this guide, we'll TODO
...
This model uses its training on past data to determine if data is arriving later than expected. In the following example, we used `build_anomaly_detection_freshness_checks` to accomplish this:
