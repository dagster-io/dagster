...
- **Op concurrency** controls the number of ops that can execute in parallel **in a single run**. **Note**: This type of concurrency applies to both op and asset-based jobs, as each asset includes an op that's responsible for computing the contents of the asset.
- **Global op concurrency <Experimental />** controls the number of ops that can execute in parallel **across runs**. **Note**: This type of concurrency applies to both op and asset-based jobs, as each asset includes an op that's responsible for computing the contents of the asset.
...
### Limiting op concurrency across runs <Experimental />
...
For example, to globally limit the number of running ops touching redshift to 2, the op must be first tagged with the global concurrency key:
...
To specify limits on the number of ops/assets tagged with a particular concurrency key, you can either use the `dagster` CLI command or use Dagit:

- To specify a global concurrency limit using the CLI, use `dagster instance concurrency set <concurrency_key> <limit>`
- To specify a global concurrency limit using Dagit, navigate to the **Concurrency limits** tab on the Deployment page in Dagit, after first enabling the **Experimental instance-level concurrency limits** option in **User Settings**.

The concurrency key should match the name that the op is tagged with. For example, if the op is tagged with `dagster/concurrency_key: redshift`, then the concurrency key should be `redshift`.
