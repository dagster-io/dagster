...
Asset jobs are defined using <PyObject object="define_asset_job" />. In this example, the job will execute up to three assets at once:
...
Op-based jobs are defined using the <PyObject object="job" decorator /> decorator. In this example, the job will execute up to four ops at once:
...
The possible causes for runs remaining in `QUEUED` status depends on whether you're using Dagster Cloud or Dagster Open Source.
