...
- **An existing boto3 client that can authenticate to AWS.** 
...
Note: There are other ways to do this, but we're going to use the UI for simplicity.
...
4. On the **Create function** page, fill in the following in **Basic information**:
   ...
   - **Runtime** - Select **Python 3.11** (TODO: Is this correct, or should it be a different version?)
...
2. Copy and paste the following URL into a new browser tab:
...
### Step 1.3: TODO: Add code to execute

In this step, you'll add the code you want to execute to the function. Create another file in the AWS UI and paste in the following code:
...
- **Initializes the Dagster Pipes context (<PyObject object="open_dagster_pipes" module="dagster_pipes" />), which yields an instance of <PyObject object="PipesContext" module="dagster_pipes" /> called `pipes`.**
  ...
  <!-- TODO: Looks like this paragraph will need to be updated a bit, based on the fact that we're using PipesMappingParamsLoader -->
...
In this step, you’ll create a Dagster asset that, when materialized, opens a Dagster pipes session and invokes the Lambda function you created in [Step 1](TODO).
...
def lambda_pipes_asset(context: AssetExecutionContext, lambda_pipes_client: PipesLambdaClient):
  return lambda_pipes_client.run(
      context: context,
      client: "TODO", 
      function_name: "function_name_or_arn",
      event: "TODO",
...
def lambda_pipes_asset(context: AssetExecutionContext, lambda_pipes_client: PipesLambdaClient):
  return lambda_pipes_client.run(
      # TODO - Update when code in previous section is complete,
  ).get_materialize_result()
