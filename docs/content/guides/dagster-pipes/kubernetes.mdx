...
3. Next, you’ll define the asset. Copy and paste the following below the imports in `dagster_k8s_pipes.py`:
   ...
   - Specified a resource for the asset to use, <PyObject module="dagster_k8s" object="PipesK8sClient" />, which is a pre-built Dagster resource that allows you to quickly get Pipes working with your Kubernetes workspace.
     ...
     - `context` - Using the `context` argument, we've passed in the asset's `context` data as well as the Kubernetes image we want to use.
   ...
   Additionally, the Kubernetes resource used by the asset exposes a `run` method. This method synchronously spins up a Kubernetes pod, runs the specified image, and returns any asset materialization events.
...
In this example, we're using an existing file named `existing_kubernetes_code.py`:

```python
# existing_kubernetes_code.py

from dagster_pipes import PipesContextLoader, PipesMessageWriter, open_dagster_pipes
...
- **Imports <PyObject object="PipesContextLoader" module="dagster_pipes" />, <PyObject object="PipesMessageWriter" module="dagster_pipes" />, and <PyObject object="open_dagster_pipes" module="dagster_pipes" /> from `dagster_pipes`.** The <PyObject object="PipesContextLoader" module="dagster_pipes" /> and <PyObject object="PipesMessageWriter" module="dagster_pipes" /> objects write temporary files for communication between the orchestration and external process. The <PyObject object="PipesContextLoader" module="dagster_pipes" /> and <PyObject object="PipesMessageWriter" module="dagster_pipes" /> match a corresponding `PipesContextInjector` and `PipesMessageReader` on the orchestration end, which are instantiated inside the <PyObject module="dagster_k8s" object="PipesK8sClient" />.
...
- **Passes the context loader and message writer to the <PyObject object="open_dagster_pipes" module="dagster_pipes" /> context manager**, which yields an instance of <PyObject object="PipesContext" module="dagster_pipes" /> called `pipes`.
  ...
  Inside the body of the context manager are various calls against `pipes` to retrieve a log and report an asset materialization. All of these calls will use the temporary file-based communications channels established by <PyObject object="PipesContextLoader" module="dagster_pipes" /> and <PyObject object="PipesMessageWriter" module="dagster_pipes" />. To see the full range of what you can do with the <PyObject object="PipesContext" module="dagster_pipes" />, see the API docs or the general [Pipes documentation](/guides/dagster-pipes).
