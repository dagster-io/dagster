...
When launching the subprocess, you may want to make environment variables or additoional parameters available in the external process. Extras can be arbitrary, user-defined parameters made available on the context object in the external process.
...
```python file=/guides/dagster/dagster_pipes/subprocess/extras_env_dagster_code.py
import os
import shutil
...
```python file=/guides/dagster/dagster_pipes/subprocess/extras_env_external_code.py lines=2-
import os
...
On the Dagster's side, the `PipesClientCompletedInvocation` object returned from `PipesSubprocessClient` includes a `get_asset_check_result` method, which you can use to access the <PyObject object="AssetCheckResult" /> event reported by the subprocess.

```python file=/guides/dagster/dagster_pipes/subprocess/asset_check_dagster_code.py
import shutil
...
From the external code, you can report to Dagster that an asset check has been performed via <PyObject module="dagster_pipes" object="PipesContext" method="report_asset_check" />. Note that `asset_key` in this case is required, and must match the asset key defined in `@asset_check`:

```python file=/guides/dagster/dagster_pipes/subprocess/asset_check_external_code.py
import pandas as pd
from dagster_pipes import PipesContext, open_dagster_pipes
...
Sometimes, you may invoke a single call to an API that results in multiple tables being updated, or you may have a single script that computes multiple assets. In these cases, you can use Dagster Pipes to report back to multiple asset at once.

<TabGroup>
<TabItem name="Define in the Dagster code">

In the Dagster code, you can use `@multi_asset` to define a single asset that reprensents multiple assets. The `PipesClientCompletedInvocation` object returned from `PipesSubprocessClient` includes a `get_results` method, which you can use to access all the events, such as multiple <PyObject object="AssetMaterialization" />s and <PyObject object="AssetCheckResult" />s, reported by the subprocess:

```python file=/guides/dagster/dagster_pipes/subprocess/multi_asset_dagster_code.py
import shutil
...
Note: when working with multi_asset, `PipesContext.report_asset_materialization` may only be called once per unique asset key. If called more than once, an error similar to the following will surface:
...
```python file=/guides/dagster/dagster_pipes/subprocess/multi_asset_external_code.py
import pandas as pd
from dagster_pipes import PipesContext, open_dagster_pipes
