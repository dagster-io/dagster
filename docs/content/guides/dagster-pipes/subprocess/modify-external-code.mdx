...
- An instance of the Dagster Pipes context via <PyObject module="dagster_pipes" object="Pipes" method="get"/>. You can access information like `partition_key` and `asset_key` via this context object. Refer to the [the API documentation](/\_apidocs/libraries/dagster-pipes#dagster_pipes.Pipes) for more information.
...
import pandas as pd
from dagster_pipes import Pipes, open_dagster_pipes
...
In this example, we’re passing a piece of metadata named `total_orders` to the `metadata` parameter of the <PyObject module="dagster_pipes" object="Pipes" method="report_asset_materialization" />. This payload will be sent from the external process back to Dagster:
...
If your asset has data quality checks defined, you can report to Dagster that an asset check has been performed via <PyObject module="dagster_pipes" object="Pipes" method="report_asset_check" />:
