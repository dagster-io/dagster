---
title: Assets without I/O | Dagster
...
Finally, let's define an asset that joins the cereal ratings data from `stopwords` with the cereals in our `topstories_word_cloud` asset:
...
In general, doing the former is preferred - to use return values and arguments in the <PyObject module="dagster" object="asset" displayText="@asset"/>-decorated function and to keep any custom I/O code inside an [I/O manager](/concepts/io-management/io-managers). A couple of the reasons are:
