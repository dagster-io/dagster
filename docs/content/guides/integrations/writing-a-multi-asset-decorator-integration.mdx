...
In this guide, you'll develop a new multi-asset integration that takes a YAML file as input and produces a multi-asset. This allows your end users to customize the translation of the definition spec and understand how it maps to Dagster concepts.
...
For this guide, let's imagine a hypothetical tool that replicates data between two databases. It's configured using a `replication.yaml` configuration file, in which a user is able to define source and destination databases, along with the tables that they would like to replicate between these systems.
