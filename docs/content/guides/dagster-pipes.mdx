...
At a high level, Dagster Pipes allows you to send Dagster context data into an external process which is then sent back to Dagster.

For example, let’s say you want to incorporate some business logic in a Dagster pipeline. However, it needs to deal with pre-existing data pipelines and Python environments, it’s written in a language other than Python, or you wish to complete separate the orchestration and business logic execution environment.,

Using Dagster Pipes, you can write a few lines of code in your business logic in an external environment and send metadata and event logs back to Dagster.
...
- Run code in external environments (with minimal dependencies) and stream log and structured metadata back to Dagster
...
2. The external process sends information about the its execution, and any specified metadata, to Dagster.
