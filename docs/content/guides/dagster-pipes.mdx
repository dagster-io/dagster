...
- **Step launchers are currently incompatible.**
- **I/O managers aren’t currently supported.** Dagster Pipes isn't built for processing data in memory, which is what I/O managers are for. For example, to process data in memory in Pyspark, you can use an I/O manager [as demonstrated in the Pyspark integration guide](/integrations/spark#running-pyspark-code-in-assets). Otherwise, in a remote case, you can use Pipes.
