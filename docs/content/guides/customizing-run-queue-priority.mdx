...
Using the `QueuedRunCoordinator` or a custom run coordinator, you can define custom prioritization rules for your Dagster instance’s run queue.
...
Used by the Dagster daemon, the run queue is a sequence of Dagster runs waiting to be executed. The daemon pulls runs from the queue and calls `launch_run` on submitted runs. It operates as a first-in, first-out priority queue.
...
Using the launchpad in the Dagster UI, you can also override priority tag values. For example:
