...
**[As of Dagster 1.1.11](/changelog#1111-core--01711-libraries)**, you can use the `dagster dev` command to start a full deployment of Dagster from your command line. This command launches [Dagit](/concepts/dagit/dagit) and the [Dagster daemon](/deployment/dagster-daemon).
...
**Note**: If using an example Dagster project, or if you used the [`dagster` CLI to create a project](/getting-started/create-new-project#bootstrapping-a-new-project), you can run the `dagster dev` command in the same folder as the project to load the project code.
...
- [Run and asset storage](#run-and-asset-storage)
...
You can optionally use a `dagster.yaml` file to configure your Dagster instance - for example, to configure [run concurrency limits](/deployment/run-coordinator#limiting-run-concurrency) or specify that runs should be stored in a [Postgres database](/deployment/dagster-instance#postgres-storage) instead of on the filesystem.
