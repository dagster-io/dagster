...
When starting with Dagster, assets are often straightforward and can be Python objects in memory - e.g. `nabisco_cereals` is created by constructing a list of row objects and then storing that list in a file.
...
Let's assume that we're working with an asset graph that takes a CSV of cereals and does some filtering and analytics on them. We'd like to augment these cereal assets with a dataset of cereal ratings from a consumer reporting organization. We've found a separate data source with this information, and it lives in a zip file that we can download from the internet. We'd like to use it to compute three assets:
...
Let's start with `cereal_ratings_zip`. Make an asset that downloads a zip file from the internet:
...
The implementation downloads a zip file from the internet to our local filesystem. It looks a lot like the assets we [defined in the tutorial](/tutorial), with two differences:
...
The implementation extracts our zip file to a CSV file in the same directory. It looks a lot like the assets you defined in the tutorial, with one main difference.

In the assets you've seen in the tutorial, we indicate the asset dependencies by including function arguments with the name of the upstream assets. Dagster then invokes an I/O manager to load the asset as a Python object and supply it to our function.

However, in this case, we don't want a Python object for the upstream asset, we just want to work with the file directly. So we instead supply `"cereal_ratings_zip"` to the `non_argument_deps` parameter of the <PyObject object="asset" method="asset" display_text="@asset"/> decorator. This lets us tell Dagster that the `cereal_ratings_csv` asset depends on the `cereal_ratings_zip` asset, without telling it to load `cereal_ratings_zip` for us.

### Nabisco cereal ratings

Finally, let's define an asset that joins the cereal ratings data from `cereal_ratings_csv` with the cereals in an asset called `nabisco_cereals`:
