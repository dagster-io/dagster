...
Users who are not using composite solid can skip this step. Composite solids are not compatible with the new APIs, and so they will need to be switched over before anything else.
2. A pipeline with n modes becomes n different jobs.
If you only have one mode on your pipeline, the `@job` decorator may be the most convenient way to do this. If using more than one mode, then the most natural way is to first create a graph, and then create multiple graphs from that.
3. (Optional) Convert each solid into an op.
While this step is not required to start using the job API, `solid` is now a deprecated API, and may have feature gaps with `op` in the future. We recommend switching each solid to use the new op API.
4. (Optional) Reorganize repositories based upon new job splits.
Splitting a pipeline into n different jobs provides a good opportunity to re-organize repositories around different execution environments.
...
The config document looks like so:
```yaml
...
graph:
  nests_solids:
    requires_config:
      config: "foo"
```
In addition to changing the top-level key to `graph`, we remove the layer of indirection, the additional `solids` key, between the composite and its constituent solids.
