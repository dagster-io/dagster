...
Migrating a pipeline to jobs does not require migrating all your other pipelines to jobs. Graphs, jobs, and pipelines can co-exist peacefully in Python, and pipelines will continue to be recognized in Dagit.

# Step-by-step migration

Using this doc and the dagster documentation at large as a guide, this is the order we recommend migrating your dagster code to the new APIs:

1. Migrate all composite solids to graphs, starting with composite solids that don't contain other composite solids.

Users who are not using composite solid can skip this step. Composite solids are not compatible with the new APIs, and so they will need to be switched over before anything else. 2. A pipeline with n modes becomes n different jobs. If you only have one mode on your pipeline, the `@job` decorator may be the most convenient way to do this. If using more than one mode, then the most natural way is to first create a graph, and then create multiple graphs from that. 3. (Optional) Convert each solid into an op. While this step is not required to start using the job API, `solid` is now a deprecated API, and may have feature gaps with `op` in the future. We recommend switching each solid to use the new op API. 4. (Optional) Reorganize repositories based upon new job splits. Splitting a pipeline into n different jobs provides a good opportunity to re-organize repositories around different execution environments.
...
### The solids keyword becomes ops

When using the job API, the top-level keyword for configuring your solids/ops changes from `solids` to `ops`. For backcompatibility purposes, and those who may still be using solids within their jobs, the `solids` keyword will still work, but the dagit playground will autosuggest in terms of `ops` for jobs.
...
```python file=/guides/dagster/graph_job_op/job_with_config_mapping.py startafter=start endbefore=end
...
```python file=/guides/dagster/graph_job_op/job_with_config_mapping.py startafter=start_execute endbefore=end_execute
...
Note that graphs no longer require information about the inputs they map to, but they still need information about the outputs they map to. The inputs and outputs of a graph now respect the types specified on the underlying `Op` instead of specifying those types directly on the graph. That is, `arg1` in the above graph will expect the same type, `Int` as the input `arg1` on `do_something_else`.
...
### A composite solid with config

Consider the following composite solid whose nested solid requires config.

```python file=/guides/dagster/graph_job_op/composite_solid_requires_config.py
...
In order to provide config for the nested solid, your config document would look like this:

```yaml file=/guides/dagster/graph_job_op/composite_solid_requires_config.yaml
solids:
  nests_solids:
    solids:
      requires_config:
...
Consider an equivalent graph whose nested solid requires config:

```python file=/guides/dagster/graph_job_op/graph_requires_config.py
...
The config document looks like:

```yaml file=/guides/dagster/graph_job_op/graph_requires_config.yaml
ops:
  nests_solids:
    requires_config:
      config: "foo"
...
In addition to changing the top-level key to `ops`, we remove the layer of indirection, the additional `solids` key, between the composite and its constituent solids.

### A composite solid with input and/or output config

Composite solids can specify their inputs via config. Consider the following composite solid:

```python file=/guides/dagster/graph_job_op/composite_solid_input_config.py
...
You can provide input values in config like so:

```yaml file=/guides/dagster/graph_job_op/composite_solid_input_config.yaml
solids:
  my_composite:
    inputs:
      feeds_x:
...
With the graph API, input values provided via config must be pushed down into the nested solids/ops.

The following is the converted graph version of `my_composite`:

```python file=/guides/dagster/graph_job_op/graph_input_config.py
...
I would provide the following config to provide input values:

```yaml file=/guides/dagster/graph_job_op/graph_input_config.yaml
ops:
  my_composite:
    my_solid:
      inputs:
...
The config must be repeated for each solid which is accepting the input.

### A composite solid with config mapping

If your composite solid utilized config mapping, that can be carried over to the equivalent graph. There are slight differences in the config schema between the two.

Consider the following composite solid:

```python file=/guides/dagster/graph_job_op/composite_solid_with_config_mapping.py
from dagster import composite_solid, solid
...
Which you would configure like so:

```yaml file=/guides/dagster/graph_job_op/composite_solid_with_config_mapping.yaml
solids:
  nests:
    config: foo
```

In order to create the equivalent graph, you would first construct a config mapping object, and then pass that to the graph's definition.

```python file=/guides/dagster/graph_job_op/graph_with_config_mapping.py
...
When configuring this graph, the config key is not necessary:

```yaml file=/guides/dagster/graph_job_op/graph_with_config_mapping.yaml
ops:
  nests: foo
```
