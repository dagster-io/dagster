...
- An [**op**](/concepts/ops-jobs-graphs/ops) is the core unit of computation in Dagster. For example, an op might accept tabular data as input and produce transformed tabular data as output.
...
Without software-defined assets, you're often forced to choose between:

- **Easy dependency tracking** by containing everything in a single mega-job, which requires maintaining a huge, unwieldy object.

- **Manageable maintenance** by splitting up your pipeline into smaller jobs, which makes tracking cross-job dependencies difficult.

This isn't the case with software-defined assets:

- **Assets track their dependencies,** so you don't need to define mega-jobs just to keep track. This allows you to avoid interruptions in dependency graphs and eliminate the need for root input managers.

- **Using software-defined assets usually means writing less code**. Specifying the inputs to a software-defined asset defines the assets it depends on. This means you don't need to use `@graph` and `@job` to wire dependencies between ops.

  This approach reduces the number of times an asset's name appears in your codebase by half, making improving your codebase's scalability. Refer to the [IO manager-based example](#materialize-two-interdependent-tables-without-an-io-manager) below to see this in action.
...
Not sure how software-defined assets fit into your current Dagster usage? In this section, we'll touch on how software-defined assets work with some of Dagster's core concepts:
...
## Upgrading jobs to use software-defined assets
