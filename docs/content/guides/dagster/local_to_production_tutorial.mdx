...
If you are using \[Dagster Cloud]\(link to dagster cloud page explaining this?), this environment variable will be set for you. Additionally, if you are using \[Branch Deployments]\(link to branch deployments page explaining this? claire's doc?) `DAGSTER_DEPLOYMENT` will be automatically set to `VAR`.
