...
- [DuckDBIOManager](/\_apidocs/libraries/dagster-duckdb#dagster_duckdb): Dagster provided DuckDB I/O manager that can store and load data as Pandas or PySpark DataFrames. Useful for local development since DuckDB runs locally and requires minimal setup.
- [`SnowflakeIOManager`](/integrations/snowflake): Dagster provided Snowflake I/O manager that can store and load data as Pandas or PySpark DataFrames.
