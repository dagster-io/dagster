...
Before proceeding, we recommend reading the following guides:

- [Automating your data pipelines](/guides/dagster/automating-pipelines)
- [Building machine learning pipelines with Dagster](/guides/dagster/ml-pipeline)

---

## Overview

This guide will review ways to manage and maintain your machine learning (ML) models in Dagster.
...
We can also use a more traditional schedule on our machine learning assets, causing them to be re-materialized or retrained on the latest data. For example, setting up a [cron schedule on a daily basis](/concepts/partitions-schedules-sensors/schedules).
...
We can also monitor the score or other information of a model through alerts, such as email or Slack. In the example below, we set up a Slack message to post the score of `ml_model` upon success. The message can be customized to include information such as which features were most influential in model training, the volume of data that the model was trained on, or in this case, the score of the model.
...
In this example, we’re using a dictionary called metadata to store the Markdown plots and the score value in Dagster.
...
If we look at the `xgboost_comments_model` asset in the Dagster UI, we see the metadata rendered. Numerical values, such as the `score (mean_absolute_error)` will be logged and plotted for each materialization, which can be useful to understand the score over time for machine learning models.
...
Version control of your machine learning model is as critical as version control of your data and code. Using Dagster will enable you to understand - what the data was that was used to train the model, when it was refreshed, what code version and finally what version of the ML model was used to generate whatever predictions it is used for the predicted values.
...
In the screenshot below, we can see each materialization of `xgboost_comments_model` and the path for where each iteration of the model is stored.
...
We can also view any plots that were generated through the asset's metadata. In this example, the plots of `score (mean_absolute_error)`
