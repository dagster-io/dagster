...
Before proceeding, the following guide is recommended:
...
Viewing previous versions of a machine learning model can be useful to understand the evaluation history or referencing a model that was used for inference. Using Dagster will enable you to understand - what the data was that was used to train the model, when it was refreshed, what code version and finally what version of the ML model was used to generate whatever predictions it is used for the predicted values.
