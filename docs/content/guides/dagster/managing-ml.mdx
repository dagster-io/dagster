...
We can use [auto-materializing assets](/concepts/assets/asset-auto-execution#auto-materializing-assets-) to update a machine learning model when the upstream data is updated. This can be done by setting the `AutoMaterializePolicy` to `eager`, which means that our machine learning model asset will be refreshed anytime our data asset is updated.
...
@asset(auto_materialize_policy=AutoMaterializePolicy.eager(max_materializations_per_minute=60))
