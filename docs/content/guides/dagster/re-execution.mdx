...
```python file=/guides/dagster/reexecution/pipeline/unreliable_pipeline.py
...
Upon launching the execution in Dagit, you can find the re-execution option on the top right. You will see multiple options for different types of re-execution.

\*\*\*\* INSERT NEW SCREENSHOT OF UNREABLE PIPELINE \*\*\*\*
...
There are several options for launching a re-execution. No matter which one you choose, the re-executed run is linked to the original run.
...
<iframe
  width="560"
  height="315"
  src="https://www.youtube.com/embed/LWc8MxSN22c"
  frameBorder="0"
...
```python file=/guides/dagster/reexecution/reexecution_api.py startafter=end_initial_execution_marker
