...
<Note>
  At the end of this guide, you'll find an optional section that demonstrates writing a resource to fetch data from Hacker News and a
...
def items(context) -> pd.DataFrame:
    ...
    result = pd.DataFrame(rows, columns=ITEM_FIELD_NAMES).drop_duplicates(
        subset=["id"]
    )
