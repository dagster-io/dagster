...
- write three assets: all of the Hacker News, data about comments, and data about stories
...
- use Dagster's Snowflake IO manager to store the datasets in [Snowflake](https://www.snowflake.com/)
  - for local development, we will use a Snowflake database called `SANDBOX` with a schema for each user
  - for staging, we will use a Snowflake database called `STAGING` with a `HACKER_NEWS` schema
  - for production, we will use a Snowflake database called `PRODUCTION` with a `HACKER_NEWS` schema
...
Let’s start by writing the asset that will contain all of the Hacker News data. We'll interact with the dataset as a Pandas DataFrame.

```python file=/guides/dagster/local_to_prod_tutorial/assets.py startafter=start_items endbefore=end_items
# assets.py
...
In this asset we use a resource (`hn_client`) that hasn't been written yet. We'll write it in a moment. First, let's write the assets to create the `comments` and `stories` datasets:

```python file=/guides/dagster/local_to_prod_tutorial/assets.py startafter=start_comments_stories endbefore=end_comments_stories
# assets.py
...
When you’re in the beginning development stages, you may find it easier to write the code to fetch data directly in your assets or ops. This is great for initial development, but eventually you may want to refactor your code to use resources. In this case we’re going to jump right into defining a resource because we're going to use it again in the optional section at the end of this guide.
...
<Note>
  ...
  in the [full code example](https://github.com/dagster-io/dagster/tree/master/examples/local_to_production) on GitHub.
</Note>
...
While we define our assets as Pandas DataFrames, the Snowflake IO manager automatically translates the asset data to and from Snowflake tables. The Python asset name determines the Snowflake table name. In this case three tables will be created: <code>ITEMS</code>, <code>COMMENTS</code>, and <code>STORIES</code>.
...
Here we run into an issue. The next time a developer wants to work on the assets, they will have to remember to change the configuration for the `snowflake_io_manager`. This leaves room for a developer to accidentally overwrite the production asset during local development.
...
<Note>
If you're using [Dagster Cloud](https://dagster.io/cloud), this environment variable will be set for you. Additionally, if you're using [Branch Deployments](https://docs.dagster.cloud/guides/branch-deployments) <code>DAGSTER_DEPLOYMENT</code> will be automatically set to <code>VAR</code>.
...
Since all of the Snowflake IO manager configuration is being set through environment variables, we no longer need to select the resource configuration based on deployment! If we decide to add more configuration that varies between local and production, we can go back to the previous pattern of selecting config based on the deployment name.
...
If you are a Dagster Cloud user, we recommend using [Branch Deployments](https://docs.dagster.cloud/guides/branch-deployments) as your staging step. Check out our \[comprehensive guide to Branch Deployments]\(link to claire’s doc) to learn how to use Branch Deployments to verify your data pipelines before deploying them to production.
...
<Note>
  ...
  For more information and recommendations on testing, refer to our [testing documentation](/concepts/testing).
</Note>
...
<Note>
  ...
  We’ll skip the implementation in this guide, but you can find it in the [full
  code](https://github.com/dagster-io/dagster/tree/master/examples/local_to_production)
</Note>
