...
- we need to be confident that data won't be accidentally overwritten because a user forgot to change a configuration value
...
- write three assets: one to represent the Hacker News data and two for the datasets containing data for comments and stories
...
If you want to follow along with this guide:

#### Clone the example code (this will give you the final version of the code):
...
When you’re in the beginning development stages, you may find it easier to write the code to fetch data directly in your assets or ops. This is great for initial development, but eventually you may want to refactor your code to use resources to support things like testing. In this case we’re going to jump right into defining a resource because we're going to use it again in the optional section at the end of this guide.
...
<Note>
  ...
  in the [full code example](link to code example) on GitHub.
</Note>
...
<Note>
If you are using [Dagster Cloud](link to dagster cloud page explaining this?), this environment variable will be set for you. Additionally, if you are using [Branch Deployments](link to branch deployments page explaining this?) <code>DAGSTER_DEPLOYMENT</code> will be automatically set to <code>VAR</code>.
...
<Note>
  ...
  For more information and recommendations on testing, refer to the [testing documentation](/concepts/testing).
</Note>
