...
The Run Coordinator is invoked when runs are submitted on the [instance](/deployment/dagster-instance) (e.g. via the [GraphQL API](/concepts/dagit/graphql#launch-a-run), and as a result it can be used to dynamically attach <Link href="/concepts/solids-pipelines/pipelines#pipeline-tags">tags</Link> to submitted runs.
