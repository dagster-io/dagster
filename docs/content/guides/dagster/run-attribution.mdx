...
The Run Coordinator is invoked when runs are submitted on the [instance](/deployment/dagster-instance) (e.g. via the [GraphQL API](/concepts/webserver/graphql#launch-a-run), and as a result it can be used to dynamically attach <Link href="/concepts/ops-jobs-graphs/metadata-tags#tags">tags</Link> to submitted runs.
