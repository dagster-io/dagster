...
However we do enable the use of strong typing to launch runs, which you can opt-in to if you so choose. For that you can use the <PyObject object="RunConfig"/> object.
...
The first step is to convert to a Pythonic resource. We will convert `FancyDbResource` to a Pythonic resource by making it inherit from <PyObject object="ConfigurableResource"/> . Instead of `config_schema={"conn_string": str}`, we can instead declare attributes on a class using vanilla Python typing.
