...
## Step Three: Computing your own logical versions

A logical version is like a fingerprint for the value that an asset represents, i.e. the output of its materialization function. We therefore want our logical versions to correspond 1-1 to the possible return values of a materialization function. Dagster auto-generates logical versions by hashing the code version together with input logical versions. This satisfies the above criterion in many cases, but sometimes a different approach is necessary.

For example, when a materialization function contains an element of randomness, then multiple materializations of the asset with the same code over the same inputs will produce the same logical version for different outputs. On the flip side, if we are generating code versions with an automated approach like source-hashing, then materializing an asset after a cosmetic refactor will produce a different logical version (which is derived from the code version) but the same output.

Dagster accommodates these and similar scenarios by allowing user code to supply its own logical versions. We simply include the logical version alongside the returned asset value in an <PyObject object="Output" /> object. Let's update `versioned_number` to do this. For simplicity, we'll use the stringified return value as the logical version:

```python file=/guides/dagster/asset_versioning_and_caching/manual_logical_versions_1.py
...
Because we updated the code version of `versioned_number`, both assets are again stale. Let's materialize them both again to bring them back to fresh. Notice the `LogicalVersion` of `versioned_number` is now `20`:

<Image
alt="Manual logical versions 1"
src="/images/guides/asset-versioning-and-caching/manual-logical-versions-1.png"
width={2662}
height={1748}
/>
...
```python file=/guides/dagster/asset_versioning_and_caching/manual_logical_versions_2.py
from dagster import DataVersion, Output, asset
...
Once again, both assets are now marked stale. Dagster doesn't know that `v5` of versioned number will return the same value as `v4`-- it only knows about code versions and logical versions. But let's see what happens if we now materialize _only_ `versioned_number`. Select it in the asset graph and click "Materialize". The sidebar shows the latest materialization now has a code_version of `v5`, and the logical version is again `20`:

<Image
alt="Manual logical versions 2"
src="/images/guides/asset-versioning-and-caching/manual-logical-versions-2.png"
width={2662}
height={1748}
/>

Notice that `multiplied_number` is no longer stale, despite the fact that we did not materialize it! Here's what happened-- `versioned_number` is no longer stale, so `multiplied_number` is no longer automatically stale. Dagster then compared the logical version of `versioned_number` last used to materializate `multiplied_number` to the current logical version of `versioned_number`. Since this comparison shows that the logical version of `versioned_number` has not changed, Dagster knows that `multiplied_number` is not stale.

Note that if `versioned_number` had used a Dagster-generated logical versions, `multiplied_number` would be stale. The logical version of `versioned_number` would have changed due to its updated code version, despite the fact that the returned value did not change. `multiplied_number` would then register as stale based off of the updated logical version of `versioned_number`.
