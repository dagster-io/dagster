...
In this example, we'll demonstrate how to use different I/O manager configurations for `local` and `production` environments using [configuration](/concepts/configuration/run-configuraiton) and [resources](/concepts/resources).
