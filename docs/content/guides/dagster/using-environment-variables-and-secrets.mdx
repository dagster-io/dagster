...
If using the UI, you can also [export locally-scoped variables to a `.env` file](/dagster-cloud/developing-testing/environment-variables-and-secrets#exporting-variables-to-a-env-file), which you can then use to develop locally.

Refer to the [Dagster Cloud environment variables guide](/dagster-cloud/developing-testing/environment-variables-and-secrets) for more info.
...
- We're adding that configured resource to our <PyObject object="Definitions" /> object so it's available for our assets.
...
```python file=/guides/dagster/using_environment_variables_and_secrets/repository_v2.py startafter=start endbefore=end
# __init__.py
...
- For both `local` and `production`, we used the <PyObject object="configured" /> method on `snowflake_pandas_io_manager` to provide environment-specific run configuration. Note the differences in configuration between `local` and `production`, specifically where environment variables were used.
