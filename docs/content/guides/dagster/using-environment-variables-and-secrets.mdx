...
To access an environment variable as part of a Dagster configuration in Python code, use the following syntax:
...
- For both `local` and `production`, we consturcted the IO manager using environment-specific run configuration. Note the differences in configuration between `local` and `production`, specifically where environment variables were used.
