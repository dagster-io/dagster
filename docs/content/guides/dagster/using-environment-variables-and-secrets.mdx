...
If Dagster detects a `.env` file in the same folder where Dagit or the `dagster-daemon` is started, it will automatically load the environment variables in the file.
