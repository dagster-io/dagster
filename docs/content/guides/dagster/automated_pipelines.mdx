---
title: Automating Your Data Pipelines | Dagster
description: This guide illustrates how to 
---
...
This guide assumes you have some familiarity with several Dagster concepts, including [software-defined assets,](https://docs.dagster.io/concepts/assets/software-defined-assets) [ops](https://docs.dagster.io/concepts/ops-jobs-graphs/ops) and [graphs](https://docs.dagster.io/concepts/ops-jobs-graphs/graphs), and [jobs](https://docs.dagster.io/concepts/ops-jobs-graphs/jobs), and [when to use each](https://docs.dagster.io/guides/dagster/how-assets-relate-to-ops-and-graphs).

### Things to think through

When thinking about automating your data pipelines, it’s helpful to think through when your data needs to be refreshed. A few helpful things to think through:
...
- How predictable is my pipeline?
- What are internal and external points of failure and how can i mitigate them?

---
## My data should be updated every morning

You want some data to be updated every day by a certain time to refresh a dashboard, have updated data go to to a system or you want your database to have the most recent information for the team to work with. 
This is one of the most traditional cases when building a pipeline. Lets go into how to do this with Dagster!
### I want a job to start at a specific time

In Dagster, we have [basic](https://www.notion.so/The-dagster-airflow-concept-mapping-and-package-use-86652814c9ca4ccf85117e444e44e72e) [scheduling](https://docs.dagster.io/concepts/partitions-schedules-sensors/schedules#basic-schedules) [](https://www.notion.so/The-dagster-airflow-concept-mapping-and-package-use-86652814c9ca4ccf85117e444e44e72e)capabilities, which means that you say how often and when you want a job to run, and it will run. This can mean daily, weekly, or hourly. Dagster also supports any type of [cron scheduling](https://en.wikipedia.org/wiki/Cron).
```python 
...
Let’s say the CFO wants a report updated by 9:00AM so they can drink their coffee and review the latest numbers. The data comes from a number of systems and are all part of your DAG. 
...
Dagster’s [declarative scheduling](https://docs.dagster.io/guides/dagster/scheduling-assets) allows you to set a freshness policy. The decision to use declarative scheduling and freshness policies is not a purely practical decision but also a mindset change compared to imperative workflow processes. 
...
defs = Definitions(assets=[update_revenue, update_sales], sensors=[update_sensor])
```
---
...
If every time you materialize Airbyte assets you want to execute a dbt asset, [asset sensors](https://docs.dagster.io/concepts/partitions-schedules-sensors/asset-sensors#asset-sensors-1) can be used to automate that process. Asset sensors can be used across [code locations](https://docs.dagster.io/concepts/code-locations#code-locations), so if you are managing some Airbyte assets in one code location and DBT assets in another, you can use asset sensors to trigger jobs to run. 

Dagster provides [reconciliation sensors](https://docs.dagster.io/concepts/partitions-schedules-sensors/asset-sensors#asset-reconciliation-sensors) that automatically materialize assets if they are ‘unreconciled’. One of the ways assets are considered unreconciled if any of the asset’s upstream dependencies have been materialized more recently than it has.  

In the example below, the sensor will materialize the dbt assets when the parent Airbtye assets materialize.
```python
from dagster import build_asset_reconciliation_sensor
...
Partitions can be used with both schedules and sensors. You can use schedules to kick off a partitioned job to [update slices of data](https://docs.dagster.io/concepts/partitions-schedules-sensors/partitions#creating-schedules-from-partitioned-jobs). You can also trigger runs once a specific partition has been updated using asset sensors based on [partitioned materializations](https://docs.dagster.io/concepts/partitions-schedules-sensors/asset-sensors#triggering-runs-upon-partitioned-materializations).
--- 
...
*Asset reconciliation sensors and freshness policy sensors can be used independently or be used for declarative scheduling.