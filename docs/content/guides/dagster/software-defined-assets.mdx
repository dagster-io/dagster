...
sfo_q2_weather_sample = AssetsDefinition.single(
...
`sfo_q2_weather_sample` represents our base temperature table. Other assets rely on it, but it's not generated inside our pipeline. We define it without specifying any function that computes its contents.
