---
title: Dagster Cloud Branch Deployments with Snowflake | Dagster
...
# Dagster Cloud Branch Deployments with Snowflake
...
- [Ops](/concepts/ops-jobs-graphs/ops) - We'll define ops that query Snowflake to drop and clone a database.
- [Graphs](/concepts/ops-jobs-graphs/graphs) - We'll define graphs that define the order our ops should run.
...
- An existing [Branch Deployments setup that uses GitHub actions](https://docs.dagster.cloud/guides/branch-deployments). After following this guide, you should have:
  - A running agent to serve your branch deployments
  - A Dagster project set up for branch deployments containing:
...
3. Clone our production database upon each branch deployment launch, naming each clone `PRODUCTION_CLONE_<ID>`, where `<ID>` is the pull request ID of the branch. Then we'll create a branch deployment and test our Hacker News assets against our newly cloned database.
...
As you can see, our assets use a [I/O manager](/concepts/io-management/io-managers) named `snowflake_io_manager` (source code available in the [Transitioning Data Pipelines from Development to Production](/guides/dagster/local-to-production-tutorial#part-one-local-development) guide). Using I/O managers and other resources allow us to swap out implementations per environment without modifying our business logic.
...
```python file=/guides/dagster/local_to_production/branch_depl_tutorial/repository_v1.py startafter=start_repository endbefore=end_repository
@repository
def repo():
    snowflake_config = {
...
The `branch_deployments.yml` workflow file from the [quickstart template](https://github.com/dagster-io/dagster-cloud-branch-deployments-quickstart) defines a `dagster_cloud_build_push` job that builds each branch deployment. We can modify this workflow file to add a custom step that launches a job within our repository that will clone the production database upon deployment.
...
```python file=/guides/dagster/local_to_production/branch_depl_tutorial/clone_and_drop_db.py startafter=start_clone_db endbefore=end_clone_db
...
```python file=/guides/dagster/local_to_production/branch_depl_tutorial/repository_v2.py startafter=start_resources endbefore=end_resources
resource_defs = {
    "branch": {
        "snowflake_io_manager": snowflake_io_manager.configured(
            {
...
```python file=/guides/dagster/local_to_production/branch_depl_tutorial/repository_v2.py startafter=start_repository endbefore=end_repository
@repository
def repo():
    ...
    return [
...
In our `branch_deployments.yml` file, we’ll add an additional step to the existing `dagster_cloud_build_push` job that queues a run of `clone_prod` within each branch deployment upon launch. The `dagster_cloud_build_push` job is triggered on multiple pull request events: `opened`, `synchronize`, `reopen`, and `closed`. This means that upon future pushes to the branch, we'll trigger a run of `clone_prod`. The `if` condition below ensures that `clone_prod` will not be be run if the pull request is closed:

```yaml file=/guides/dagster/local_to_production/branch_depl_tutorial/clone_prod.yaml
...
```python file=/guides/dagster/local_to_production/branch_depl_tutorial/repository_v3.py startafter=start_drop_db endbefore=end_drop_db
@graph
def drop_prod_clone():
    drop_database_clone()
...
```yaml file=/guides/dagster/local_to_production/branch_depl_tutorial/drop_db_clone.yaml
name: Dagster Branch Deployments
on:
  pull_request:
    types: [opened, synchronize, reopened, closed]
