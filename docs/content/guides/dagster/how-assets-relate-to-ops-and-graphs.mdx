...
- When you should assets or ops and graphs
- How assets, ops, and graphs relate to each other
- Why Dagster offers all three abstractions
...
A single Dagster deployment can contain software-defined assets and op/graph-based jobs side-by-side, which means that you’re not bound to one particular choice. If your workflow reads from software-defined assets, you might wish to model that explicitly in Dagster, which is discussed in a [a later section](#op-graphs-that-read-from-an-asset).

### Flavor 2: You want to break up one of your software-defined assets into multiple steps

If computing a particular asset requires multiple steps, and some of those steps don’t produce assets of their own, and you want to be able to restart from the middle, you might want to use a _graph-backed asset_. This is discussed more [later in this guide](#graph-backed-assets).
