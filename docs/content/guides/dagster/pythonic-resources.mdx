...
```python file=/guides/dagster/pythonic_resources/pythonic_resources.py startafter=start_new_resource_runtime_launch endbefore=end_new_resource_runtime_launch
...
```python file=/guides/dagster/pythonic_resources/pythonic_resources.py startafter=start_io_adapter endbefore=end_io_adapter
from dagster import (
    Definitions,
    IOManagerDefinition,
    io_manager,
...
class OldFileIOManager(IOManager):
    ...
    def load_input(self, context: InputContext):
        with open(
            os.path.join(
                self.base_path, context.upstream_output.step_key, context.upstream_output.name
            ),
            "r",
        ) as fd:
            return fd.read()
