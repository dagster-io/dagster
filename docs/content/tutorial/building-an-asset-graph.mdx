...
Reload your definitions and rematerialize your assets. The word cloud image will be visible with the rest of your materialization metadata for the `topstories_word_cloud` asset. The `path` key in the metadata will contain a link that says **`[Show Markdown]`**. Clicking on the link will open the preview in the Dagster UI. The word cloud will change throughout the day as the top stories change. Here is a screenshot of the word cloud:
