...
# Dagster tutorial, part four: Building an asset graph
...
## Step 1: Adding the data frame asset

By the end of the previous section, you ingested the top Hacker News story IDs into Dagster. Using this data, you'll now look up each story by its ID, ingest that data, and make a data frame out of it. You'll connect your current asset with this new asset to establish dependencies and make an _asset graph_.
...
Dependencies between assets are defined by adding the asset name as a parameter to the dependent asset's function. In this case, `topstory_ids` (the list of IDs) is a dependency of `topstories` (the data frame).
...
Along with structured data like tables, Dagster's assets can also be unstructured data, such as images. Your next and final asset will take the data frame of stories and create a word cloud with the titles.
...
The data frame was embedded into the asset's metadata with Markdown. Any valid Markdown snippet can be stored and rendered in the Dagster UI, including images. By embedding the word cloud image as metadata, the word cloud asset is also visible to your team from within the UI.
...
@asset
def topstories_word_cloud(context, topstories):
    stopwords = set(STOPWORDS)
    stopwords.update(["Ask", "Show", "HN", "S"])
    titles_text = " ".join([str(item) for item in topstories["title"]])
    titles_cloud = WordCloud(stopwords=stopwords, background_color="white").generate(titles_text)
...
For more information about metadata, refer to the [metadata documentation](/concepts/metadata).
...
Move on to find out what to do now that you've completed the tutorial in the [next steps](/tutorial/next-steps) section.
