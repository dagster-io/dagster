...
- To install Dagster, Dagit, and the packages you'll be using during this tutorial, run the follow command from your terminal:

  ```bash
  pip install dagster dagit requests pandas matplotlib wordcloud dagster_duckdb dagster_duckdb_panda
  ```
  ...
  - `dagster_duckdb_pandas` allows loading data from DuckDB into Pandas data frames
