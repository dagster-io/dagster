...
I/O managers are a powerful tool that can greatly simplify your code and allow you to easily modify where your assets are stored. However, some assets do not need I/O managers.

For example, I/O managers are useful in the following cases:
* When you need to access the contents of an upstream asset in memory.
* When your assets can be stored in a consistent way (i.e. in the same schema of a database).

Using I/O managers is not required, and may not be the right tool for some assets. For example:
* Your asset executes a SQL queries in Snowflake to join two tables to create a third table.
* Your asset launches a Databricks job that creates a table.
