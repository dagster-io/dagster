...
You've written a data pipeline using assets and had it rebuilt every hour by now. Currently, your assets are stored in a temporary folder that is deleted after closing the `dagster` process. **Note**: If you set up Dagster without the tutorial, you may have set a `DAGSTER_HOME` environment variable. If so, then your assets will be stored in that directory.
...
<Note>
  In-memory processing means the data is transformed on a computer's RAM (Random
  access memory). This is different from disk storage, which is slower but
  likely you have more of. In-memory processing is faster, but it can only be
  done on a computer's RAM and you likely have less RAM on your machines.
</Note>
...
Dagster includes out-of-the-box IO managers for common databases, such as DuckDB. When a DataFrame is returned in an asset’s definition, it is translated into a database-compatible format. The I/O manager then runs a SQL query to write the data into the database.
