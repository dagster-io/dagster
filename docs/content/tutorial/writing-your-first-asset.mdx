---
title: Writing your first asset | Dagster Docs
...
Now, add the <PyObject module="dagster" object="asset" displayText="@asset"/> decorator from the `dagster` library to the function:
...
To launch the UI, set your current directory to `/tutorial-project` in your terminal and run the following command:

```bash
dagster dev
```

If `localhost:3000` is not already being used, it will use that port. Navigate to `localhost:3000` in your browser to see the Dagster UI.
