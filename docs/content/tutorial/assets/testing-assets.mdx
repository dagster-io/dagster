...
First, create a new file and name it `test_complex_asset_graph.py`.

At the top of the file, import the `cereals` and `nabisco_cereals` assets:
...
To run the function that derives an asset from its upstream dependencies, we can invoke it directly as if it's a regular Python function. Paste the following code at the bottom of the file:
...
In `test_complex_asset_graph.py`, add `cereal_protein_fractions` and `highest_protein_nabisco_cereal` to the list of imported assets:
...
Add a new line below the imported assets to import `materialize` from `dagster`:
...
Now you can use pytest or your test runner of choice to run the unit test:
