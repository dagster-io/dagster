...
Let's enter the config we need in order to execute our pipeline (note: the screenshot below is outdated; your config will look like `solids`->`read_csv`->`config`->`csv_name: "cereal.csv"`).
