...
To tell an asset not to load a dependency into memory when materializing, you can use the <PyObject object="asset" decorator /> decorator’s `upstream_assets` parameter to establish the dependency instead of including it as an argument of the asset’s function definition.
