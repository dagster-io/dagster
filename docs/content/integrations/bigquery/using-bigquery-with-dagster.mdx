...
In this example, you're creating an asset definition for a pre-existing table called `iris_harvest_data`.
...
If you already have existing tables in BigQuery and other assets defined in Dagster depend on those tables, you may want Dagster to be aware of those upstream dependencies. Making Dagster aware of these tables will allow you to track the full data lineage in Dagster. You can create [non-materializable asset definitions](/concepts/io-management/io-managers#using-io-managers-to-load-source-data) for these tables. When using an I/O manager, creating a non-materializable asset for an existing table also allows you to tell Dagster how to find the table so it can be fetched for downstream assets.
