...
For larger projects, the overhead involved in recompiling your entire project whenever your repository needs to be loaded may become a concern. In these cases, it's possible to load
...
This means that there's a range of ways that we might want to load a dbt model as input to a python function. For example, you might want to load the contents of the table as a Pandas dataframe, or you might want to load the contents into a PySpark session.

To define exactly how you want models to be loaded, you can specify an input manager on the downstream asset.
...
Dagster automatically generates AssetKeys for each dbt model in the project, as well as any of the sources used by these models.
