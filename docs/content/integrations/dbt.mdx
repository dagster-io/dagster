...
`dbt` models translate well into Dagster's software-defined asset paradigm, and if you're just getting started, this is the

It's also possible to load

## dbt models and Software-Defined Assets

Models defined in a `dbt` project are conceptually very similar to Dagster's Software-Defined Assets.
...
There are two ways to load `dbt` models as software-defined assets.
...
Note: you'll need to recompile

## Running dbt commands as Ops in a Graph
