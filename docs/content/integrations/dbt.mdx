...
**Example:** The solid `dbt_cli_run` is configured to run specific models similar to above except after another solid has executed. When use the `start_after` argument, which has the `InputDefinitions` of `dagster_type=Nothing`, you can assign the output from another solid so that you can still model the dependencies with another solid. See the [Order Based Dependencies](/concepts/solids-pipelines/pipelines#order-based-dependencies-nothing-dependencies) for more info.
