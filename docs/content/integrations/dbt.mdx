...
**Example:** The solid `dbt_cli_run` is configured to run specific models similar to above except after another solid has executed. Use can use the `start_after` argument, which has the `InputDefinitions` of `dagster_type=Nothing` to assign the output from the solid if you do not care about the output.
