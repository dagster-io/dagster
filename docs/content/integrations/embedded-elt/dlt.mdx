...
1. Create a location for the dlt source code and initialize the GitHub source:
...
Ensure that these variables are defined in your environment; either in your `.env` file when running locally, or in the Dagster deployment environment variables.

---

## Step 4: Define a DagsterDltResource

The <PyObject module="dagster_embedded_elt.dlt" object="DagsterDltResource" /> provides a wrapper of a dlt pipeline runner. This resource is shared across dlt pipelines; create an instance of this resource in the file in which you define resources. We will register the resource to our <PyObject module="dagster" object="Definitions" /> in a later step.
...
The `@dlt_assets` decorator takes a `dlt_source` and `dlt_pipeline` parameter. In this example, we used the `github_reactions` source and create a `dlt_pipeline` to ingest data from Github to Snowflake. In the file in which you define your assets, you can create create an instance of your `@dlt_assets` like so:
...
In the file where you register your definitions, include the `dagster_github_assets` along with the `dlt_resource`.
