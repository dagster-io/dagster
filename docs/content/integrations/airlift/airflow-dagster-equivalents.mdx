...
An Airflow Task is a single computation within a DAG, which executes an `execute` method of its given Operator. Here's an example of an Airflow Task using the `PythonOperator` to upload a file to S3:
...
Dagster in comparison uses software-defined assets to represent the _outputs_ of a task. You define the data asset you want to exist, and then define a computation for _how_ to create that asset. Here's an example of a Dagster Asset that uploads a file to S3:
...
You can use a Dagster sensor to similarly kick off compute of assets once a condition is met. Here's an example of a Dagster sensor that will kick off computation of an asset whenever a new file is added to a directory:
...
Key differences between sensors in Airflow and Dagster:
...
Airflow allows templating variables into your DAGs using Jina. Dagster doesn't have a direct jinja templating feature, instead you're encouraged to use Python functions and interact with Dagster's richly typed API to pass information. The context information available to each asset's execution is on the <PyObject object="AssetExecutionContext" /> object.
