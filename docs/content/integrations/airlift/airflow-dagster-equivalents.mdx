...
Dagster in comparison uses software-defined assets to represent what a task produces. You define the data asset you want to exist, and then define a computation for _how_ to create that asset. If you're familiar with Airflow's Datasets, you can think of it as defining a task and a dataset at once. Here's an example of a Dagster Asset that uploads a file to S3:
