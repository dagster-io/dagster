...
You'll also want to have a dbt Cloud instance with an existing project that is deployed with a dbt Cloud job. If you don't have one already, you can select and follow one of dbt's quickstart guides to [set up dbt Cloud with a sample project](https://docs.getdbt.com/guides).
