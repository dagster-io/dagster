...
You may want to have different assets stored in different Snowflake schemas. The Snowflake I/O manager allows you to specify the schema in several ways. You can specify the schema as configuration to the I/O manager:
...
@asset(
    io_manager_key="blob_io_manager"
)
...
The Snowflake I/O manager requires some configuration to connect to your Snowflake instance. Configuration can be supplied to the I/O manager when you attach it as a resource to your assets, like this:

For documentation for each of the configuration values, see the **Config schema** dropdown in the [Snowflake API docs](#dagster_snowflake.build_snowflake_io_manager).
