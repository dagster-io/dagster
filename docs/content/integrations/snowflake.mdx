...
The configuration values for the Snowflake I/O manager can be stored in [environment variables](/guides/dagster/using-environment-variables-and-secrets) and referenced in the configuration dictionary. In this example, we're storing the username and password in environment variables and telling the Snowflake I/O manager to find the values in the `SNOWFLAKE_USER` and `SNOWFLAKE_PASSWORD` variables, respectively.

For example, with this configuration, if you materialized an asset called `iris_dataset`, Dagster would store the data in the `FLOWERS.IRIS.IRIS_DATASET` table.
