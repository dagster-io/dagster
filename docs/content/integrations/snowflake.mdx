...
### Concepts
This guide assumes you are familiar with Dagster's software-defined assets. For more information on software-defined assets, see the [Assets tutorial](/tutorial/defining-an-asset) or the [Assets concept page](/concepts/assets/software-defined-assets).
...
- The same logic for reading and writing data is used for all assets using the same I/O manager, meaning your assets will be written and read consistently.
...
When an op or asset needs the value from an upstream op or asset as input, the I/O manager can also fetch the value from the location where it was stored. Dagster calls the `load_input` method of the I/O manager with the information required to find where the value was stored and the I/O manager returns the correct value. The `handle_output` and `load_input` methods are symmetric, meaning that `handle_input(handle_output(X)) == X`.

For the Snowflake I/O manager, the `handle_output` and `load_input` methods take care of creating the SQL queries required to store and fetch the data in Snowflake tables.

To learn more about I/O managers, check out the [I/O manager concept page](/concepts/io-management/io-managers).

## Using the Snowflake IO manager

In the remainder of this guide, we will go over how to use the Snowflake I/O manager with your Dagster assets. Each of the following examples will go over how to achieve a particular task, but all of the concepts covered can be combined.

* [Storing data in Snowflake](#storing-data-in-snowflake)
* [Making existing tables available in Dagster](#making-existing-tables-available-in-dagster)
* [Loading Snowflake tables in downstream assets](#loading-snowflake-tables-in-downstream-assets)
* [Selecting specific columns from a Snowflake table in downstream assets](#selecting-specific-columns-from-a-snowflake-table-in-downstream-assets)
...
* [Configuring the Snowflake I/O manager](#configuring-the-snowflake-io-manager)
...
### Making existing tables available in Dagster
...
### Loading Snowflake tables in downstream assets
...
### Selecting specific columns from a Snowflake table in downstream assets
...
In this example, we only need the columns with sepal data in our `sepal_data` asset. Fetching the entire table would be unnecessarily costly. To select specific columns, we need to add metadata to the input asset. We do this by explicitly specifying which asset should be loaded into our `sepal_data` asset.

In the `ins` parameter to `sepal_data` we create a dictionary where the keys are the parameters for the asset function and the values are `AssetIn`s that tell Dagster what assets to load. In this example, we tell Dagster that the `iris_sepal` parameter should be the `iris_dataset` asset. In the `metadata` of the `AssetIn` we supply the key `columns` with a list of the column names we want to fetch.

When Dagster materializes the `sepal_data` asset, when it loads the `iris_dataset` asset using the Snowflake I/O manager, it will only fetch the `Sepal length (cm)` and `Sepal width (cm)` columns of the `FLOWERS.IRIS.IRIS_DATASET` table as a Pandas DataFrame and pass them to `sepal_data`.
...
```python file=/integrations/snowflake/schema.py startafter=start_configuration endbefore=end_configuration
from dagster_snowflake_pandas import snowflake_pandas_io_manager
...
In this example, all assets will be stored in the `IRIS` schema.
...
You can also provide configuration to the Snowflake I/O manager that will tell it where to store your data. Specifically, you can tell the Snowflake I/O manager the `warehouse`, `database`, and `schema` to store your data in:
...
TODO add these sections as they are supported
