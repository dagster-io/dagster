...
| <PyObject module="dagster_dbt" object="load_assets_from_dbt_manifest" />                       | A convenience function that defines Dagster assets for dbt models with either `dbt build` or `dbt run`. This function uses <PyObject module="dagster_dbt" object="dbt_assets" decorator /> in its implementation. |
