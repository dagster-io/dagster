...
You can create a Dagster project that wraps your dbt project by using the [`dagster-dbt project scaffold`](/\_apidocs/libraries/dagster-dbt#dagster-dbt-project-scaffold) command line interface. It is recommended to use the `--use-dbt-project` flag to use [DbtProject](/\_apidocs/libraries/dagster-dbt#dagster_dbt.DbtProject).
...
- [**Option 2:**](#option-2-not-using-dbtproject) Not using `DbtProject`

</TabItem>
<TabItem name="Option 1: Using DbtProject">

### Option 1: Using `DbtProject`

The easiest way to handle the creation of your manifest file is to use [DbtProject](/\_apidocs/libraries/dagster-dbt#dagster_dbt.DbtProject).

In the Dagster project created by the [`dagster-dbt project scaffold`](/\_apidocs/libraries/dagster-dbt#dagster-dbt-project-scaffold) command line interface, when using the [--use-dbt-project flag](/\_apidocs/libraries/dagster-dbt#cmdoption-dagster-dbt-project-scaffold-project-name), the creation of your manifest is handled during development:
...
In production, a precompiled manifest should be used. Using `DbtProject`, the manifest can be created at build time by running the [dagster-dbt project prepare-and-package](/\_apidocs/libraries/dagster-dbt#dagster-dbt-project-prepare-and-package) command line interface in your CI/CD workflow. For more information, see the [Deploying a Dagster project with a dbt project](#deploying-a-dagster-project-with-a-dbt-project) section.

</TabItem>
<TabItem name="Option 2: Not using DbtProject">

### Option 2: Not using `DbtProject`

Creating the manifest can be done by running `dbt parse` using the command line interface (CLI) with [DbtCliResource](/\_apidocs/libraries/dagster-dbt#dagster_dbt.DbtCliResource).
...
3. Depending on whether [DbtProject](/\_apidocs/libraries/dagster-dbt#dagster_dbt.DbtProject) is used, this step will vary:
   - **For projects using `DbtProject`**, run `dagster-dbt project prepare-and-package --file path/to/project.py` to
      ...
      - create a dbt manifest for your Dagster project,
      - and package your dbt project.
   - **For projects not using `DbtProject`**:
...
Dagster allows you to emit column-level metadata, like column schema and column lineage, as [materialization metadata](/concepts/metadata-tags/asset-metadata).
