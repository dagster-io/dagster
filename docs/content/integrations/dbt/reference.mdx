...
An AutoMaterializePolicy may be added to a dbt node by defining [meta config](https://docs.getdbt.com/reference/resource-configs/meta) on your dbt node. This policy may be one of two types, `"eager"` or `"lazy"`.

The following example applies an eager AutoMaterializePolicy to the "customers" dbt model:
...
A FreshnessPolicy may be added to a dbt node by defining [meta config](https://docs.getdbt.com/reference/resource-configs/meta) on your dbt node. This config accepts identical arguments to the <PyObject object="FreshnessPolicy"/> class.

The following example applies a FreshnessPolicy to the "customers" dbt model:
