...
Then the asset created for that model will be given an _upstream_ asset key of `jaffle_shop/orders`. In many cases, this upstream asset might also be managed by Dagster. If you add an asset definition to your repository which produces `jaffle_shop/orders`, then this asset will be upstream of your dbt model.
