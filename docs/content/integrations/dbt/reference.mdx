...
**Note**: The `key_prefix` argument only applies to assets loaded from this command. If you want to apply a prefix to the input keys that Dagster generates, pass in a `source_key_prefix` argument:
...
<Note>
  Check out parts{" "}
  <a href="/integrations/dbt/using-dbt-with-dagster/part-two">two</a> and{" "}
  <a href="/integrations/dbt/using-dbt-with-dagster/part-three">
