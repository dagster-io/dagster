...
In the Dagster project created by the [`dagster-dbt project scaffold`](/\_apidocs/libraries/dagster-dbt#dagster-dbt-project-scaffold) command line interface, we use <PyObject module="dagster_dbt" object="DbtArtifacts" /> to handle this for us:
...
As part of deploying to production, this code can be run as a script to handle compiling the manifest.
