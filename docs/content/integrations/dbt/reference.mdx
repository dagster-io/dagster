...
You can create a Dagster project that wraps your dbt project by using the [`dagster-dbt project scaffold`](/\_apidocs/libraries/dagster-dbt#dagster-dbt-project-scaffold) command line interface. We recommend using the `--use-dbt-project` flag to use <PyObject object="DbtProject" module="dagster_dbt" />.
...
In the Dagster project created by the [`dagster-dbt project scaffold`](/\_apidocs/libraries/dagster-dbt#dagster-dbt-project-scaffold) command, when using the [`--use-dbt-project` flag](/\_apidocs/libraries/dagster-dbt#cmdoption-dagster-dbt-project-scaffold-project-name), the creation of your manifest is handled during development:
