...
1. In `/tutorial_template/dbt_dagster_sda_tutorial/assets/__init__.py`, replace the imports section with the following:
...
- Finally, the chart is saved as `order_count_chart.html` in `/tutorial_template/dbt_dagster_sda_tutorial` and automatically opened in the browser upon successful materialization.

At this point, `/tutorial_template/dbt_dagster_sda_tutorial/assets/__init__.py` should look like this:
...
Replace the contents of `/tutorial_template/dbt_dagster_sda_tutorial/repository.py` with the following:
