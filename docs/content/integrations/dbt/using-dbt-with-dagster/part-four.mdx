...
You've added upstream assets to your data pipeline, but nothing downstream - until now. Here's a quick refresher of what our graph of assets currently looks like:
...
In this step, you'll create a Dagster asset - called `order_count_chart` - that's downstream of the `customers` asset produced by the `stg_orders` and `stg_customers` dbt models. The `order_count_chart` asset computes a plotly chart using the data from its upstream asset dependency.

1. In `/assets/__init__.py`, replace the imports section with the following:
...
2. After the `customers_raw` and `orders_raw` assets, add the following:
...
- Finally, the chart is saved as `order_count_chart.html` in `/tutorial_template/dbt_dagster_sda_tutorial` and automatically opened in the browser.
...
from dbt_dagster_sda_tutorial.assets import (
    DBT_PROFILES,
    DBT_PROJECT_PATH,
    customers_raw,
    dbt_assets,
...
1. Back in Dagit on the asset graph, click **Reload definitions**. This will ensure that Dagit picks up the changes you made in steps one and two.
...
4. Click the **Materialize all** button to kick off a run that materializes the assets.
