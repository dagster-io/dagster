...
src="/images/integrations/dbt/using-dbt-with-dagster/part-three-materialized-assets.png"
width={998}
height={808}
/>
...
- We've also used <PyObject object="AssetIn" /> to explicitly define an upstream dependency. Just like in [part two of this tutorial](/integrations/dbt/using-dbt-with-dagster/part-two#step-2-create-the-dagster-assets), we've defined these as `jaffle_shop` and `staging`, respectively.

- Finally, the chart is saved as `order_count_chart.html` in `/dbt_dagster_tutorial/dbt_dagster_tutorial` and automatically opened in the browser.
...
2. Click the **Materialize all** button to kick off a run that materializes the assets.

That's it! When the run finishes successfully, the plotly chart will automatically open in your browser:
