...
- [Materialize the assets using the Dagster UI](#step-3-materialize-the-assets-using-the-dagster-ui)
...
1. In the UI, click the **Reload definitions** button. This ensures that the UI picks up the changes you made in the previous steps.
   ...
   <Image
   alt="Asset graph in the Dagster UI, showing dbt models and unmaterialized assets"
...
2. Click the **Materialize all** button near the top right corner of the page, which will launch a run to materialize the assets. When finished, the **Materialized** and **Latest Run** attributes in the asset will be populated:
   ...
   <Image
   alt="Asset graph in the Dagster UI, showing materialized assets"
...
- Click the ID of the **Latest Run** - in the above image, that's `651489a2` - in an asset to view the [**Run details**](/concepts/webserver/ui#run-details) page. This page contains detailed info about the run, including timing information, errors, and logs.
