...
To retrieve the `manifest.json`, `assets.py` imports from `project.py`, which manages resolving the `manifest.json` file:

```python file=integrations/dbt/tutorial/load_dbt_models/project.py startafter=start_load_manifest endbefore=end_load_manifest
import os
