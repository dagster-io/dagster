...
2. To start Dagster's UI, run the following:
   ...
   <Note>
    `DAGSTER_DBT_PARSE_PROJECT_ON_LOAD` is an environment variable. If using [Microsoft PowerShell](https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_environment_variables?view=powershell-7.4#using-the-variable-syntax), set it before running `dagster dev` using this syntax:
