...
As the comment explains, calling `dbt_project.prepare_if_dev()` prepares your dbt project during development, i.e. when `dagster dev` is used, which ensures that the dbt project is ready to be used by Dagster. This process creates the `manifest.json` file.

To disable the project preparation during development, `dbt_project.prepare_if_dev()` can be removed.
