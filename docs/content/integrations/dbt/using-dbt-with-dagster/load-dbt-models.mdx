...
To retrieve the `manifest.json`, `assets.py` imports from `artifacts.py`, which manages resolving the `manifest.json` file:

```python file=integrations/dbt/tutorial/load_dbt_models/artifacts.py startafter=start_load_manifest endbefore=end_load_manifest
import os
...
As the comment explains, <PyObject module="dagster_dbt" object="DbtArtifacts" /> helps us achieve a nice workflow regarding our `manifest.json` file. When using `dagster dev`, the `manifest.json` is compiled on load which ensures that we pick up any changes while developing. Outside of development, it is expected that `manifest.json` was already compiled as part of the deployment process. This avoids the time-consuming overhead of compiling the manifest in the Dagster processes that need to load the Definitions.
