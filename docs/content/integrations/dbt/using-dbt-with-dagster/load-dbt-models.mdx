...
- [Install the dagster and the dagster-dbt libraries](#step-1-install-the-dagster-and-the-dagster-dbt-libraries)
- [Create a Dagster project that wraps your dbt project](#step-2-create-a-dagster-project-that-wraps-your-dbt-project)
- [Inspect your Dagster project in Dagster's UI](#step-3-inspect-your-dagster-project-in-dagsters-ui)
- [Build your dbt models in Dagster](#step-4-build-your-dbt-models-in-dagster)
- [Understand the Python code in your Dagster project](#step-5-understand-the-python-code-in-your-dagster-project)

---

## Step 1: Install the dagster and the dagster-dbt libraries
...
## Step 2: Create a Dagster project that wraps your dbt project

You can create a Dagster project that wraps your dbt project by using the `dagster-dbt` command line interface. Make sure you're in the directory where your `dbt_project.yaml` is. If you're continuing from the previous section, then you'll already be in this directory. Then, run:
...
**Note**: The `dagster-dbt project scaffold` command creates the Dagster project in whatever directory you run it from. If that's a different directory from where your `dbt_project.yaml` lives, then you'll need to provide a value for the `--dbt-project-dir` option so that Dagster knows where to look for your dbt project.

---

## Step 3: Inspect your Dagster project in Dagster's UI
...
## Step 4: Build your dbt models in Dagster
...
src="/images/integrations/dbt/using-dbt-with-dagster/load-dbt-models/dbt-asset-graph-materialized.png"
width={2330}
height={1708}
/>
...
## Step 5: Understand the Python code in your Dagster project
...
Once you've got a path to a `manifest.json`, it's time to define your Dagster assets using it. The following code, also already in your project's `definitions.py`, does this:

```python file=integrations/dbt/tutorial/load_dbt_models/definitions.py startafter=start_dbt_assets endbefore=end_dbt_assets
...
If you later want to customize how your dbt models are translated into Dagster assets, you'll do so by editing this file.
