...
In `/tutorial_template/dbt_dagster_sda_tutorial/assets/__init__.py`:
...
At this point, the `/tutorial_template/dbt_dagster_sda_tutorial/assets__init__.py` file should look like this:
...
In this step, we'll update the Dagster repository to supply the `duckdb_io_manager` to the assets in the repository. This resource contains an [I/O manager](/concepts/io-management/io-managers) that, when assets are materialized, allows:
...
In `/tutorial_template/dbt_dagster_sda_tutorial/repository.py`, replace its contents with the following:
...
- Click the ID of the **Latest Run** - in the following image, that's `651489a2` - in an asset to view the [**Run details**](/concepts/dagit/dagit#viewing-a-single-run) page. This page contains detailed info about the run, including timing information, errors, and logs.
