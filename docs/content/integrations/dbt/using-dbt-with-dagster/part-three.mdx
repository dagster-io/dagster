...
- [Add an I/O manager to the Dagster code location](#step-2-add-an-io-manager-to-the-dagster-code-location)
...
## Step 2: Add an I/O manager to the Dagster code location
...
In this step, we'll update the Dagster code location to supply the `duckdb_io_manager` to our assets. This resource contains an I/O manager that, when assets are materialized, allows:
...
Now that you've created assets, resources, and defined a code location, it's time to materialize the assets! Materializing an asset runs the op it contains and saves the results to persistent storage. In this tutorial, we're saving asset outputs to DuckDB.
