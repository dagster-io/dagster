...
- We've also used <PyObject object="AssetIn" /> to explicitly define an upstream dependency. Just like in [part three of this tutorial](/integrations/dbt/using-dbt-with-dagster/upstream-assets#step-1-create-upstream-dagster-assets), we've defined these as `jaffle_shop` and `staging`, respectively.
