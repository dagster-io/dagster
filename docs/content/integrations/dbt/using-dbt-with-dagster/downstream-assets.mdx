...
1. Back in the UI on the asset graph, click **Reload definitions**. This ensures that the UI picks up the changes you made in the previous steps.

   At this point, the `order_count_chart` asset should display below `customers` as a downstream dependency:

   <Image
   alt="order_count_chart asset displaying in the Dagster UI's asset graph"
...
- Learn how to [build jobs that materialize dbt assets](/integrations/dbt/reference#scheduling-dbt-assets)
