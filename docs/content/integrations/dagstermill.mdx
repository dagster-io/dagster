...
Developing in notebooks is an important part of many data science workflows. However, when developing a notebook, it is often considered a stand alone artifact. The notebook is responsible for fetching any data it needs, running the analysis, and presenting results. The notebook may be executed manually or on a schedule (potentially using a tool like `cron`). This presents a number of problems, the main two being:
