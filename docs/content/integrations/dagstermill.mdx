...
This series of commands installs Dagster, then downloads the code for the `tutorial_notebook_assets` example project, and installs the dependencies required for the example project.
...
Then navigate to the Asset Group named `template_tutorial`. You can find the list of Asset Groups by clicking the hamburger menu icon in the top left corner of Dagit, and then opening the dropdown of the `template_tutorial` repository to open the Asset Graph page. If you click on your notebook asset, you will see `View Source Notebook`, which allows you to view your notebook directly in Dagit. The button will render the notebook that will be executed when you materialize the notebook (ie the notebook you referenced in the `notebook_path` parameter).
...
Click the `Materialize` button. An alert will pop up with a `View` button. Clicking the `View` button will open a new tab where you can watch the execution of your notebook and see the Dagster logs. Once your notebook has executed successfully, you can close the tab and return the Asset Graph page. If you click on your notebook asset again, you will see an additional `View Notebook` button in the top section of the right side panel. Clicking this button will render the _executed_ notebook (ie the notebook that the materialization executed and wrote to a persistent location).
...
1. Creating an iris dataset asset allows us to use the same asset as input to more notebooks. This means that all notebooks analyzing the iris dataset will use the same source data.
...
We've successfully integrated our `iris-kmeans` notebook with Dagster! But soon we may want to do more analysis of the iris dataset, and we decide to write a new notebook. Instead of fetching the iris dataset in the notebook for our initial development, we should use the `iris_dataset` asset! In our Jupyter notebook we can import our Dagster repository and use the <PyObject object="RepositoryDefinition" method="load_asset_value" /> function to load the data for the `iris_dataset` asset.
...
Now we have successfully created an asset from a Jupyter notebook and integrated it with our Dagster project! To learn about additional dagstermill features, the the remainder of this page.
...
No match for endBefore value "end_py_file"
```
...
Creating multiple assets within a Jupyter notebook is not currently supported. If you'd like to see this feature prioritized, upvote this [GitHub issue](https://github.com/dagster-io/dagster/issues/10557).
