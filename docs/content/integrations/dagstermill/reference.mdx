...
<Note>
  This functionality only works for notebooks run with
...
For example, if you'd like to yield a custom <PyObject module="dagster" object="AssetMaterialization" /> object (for instance, to tell Dagit where you've saved a plot), you can do the following:
