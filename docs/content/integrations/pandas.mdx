...
<Warning>
  <code>dagster-pandas</code> is not yet officially deprecated, but the
  recommended approach for dealing with Pandas DataFrame types is to use{" "}
  <Link href="/integrations/pandera">dagster-pandera</Link> instead.
</Warning>

- [Creating Dagster DataFrame Types](#creating-dagster-dataframe-types) - [Dagster DataFrame Level Validation](#dagster-dataframe-level-validation) - [Dagster DataFrame Summary Statistics](#dagster-dataframe-summary-statistics)
