...
<Warning>
`dagster-pandas` is not yet officially deprecated, but the recommended approach for dealing with pandas dataframe types is to use [`dagster-pandera`](https://docs.dagster.io/integrations/pandera) instead.
</Warning>
