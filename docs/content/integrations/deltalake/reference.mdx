...
- [Executing custom SQL commands with the Delta Lake resource](#executing-custom-sql-commands-with-the-delta-lake-resource)
...
You can use the `DeltaLakePyArrowIOManager` in your <PyObject object="Definitions" /> as in [Step 1](/integrations/deltalake/using-deltalake-with-dagster#step-1-configure-the-delta-lake-io-manager) of the [Using Dagster with Delta Lake tutorial](/integrations/deltalake/using-deltalake-with-dagster).
...
To do this, inherit from the <PyObject module="dagster_deltalake" object="DeltaLakeIOManager" /> base class and implement the `type_handlers` and `default_load_type` methods. The resulting I/O manager will inherit the configuration fields of the base `DeltaLakePandasIOManager`.
