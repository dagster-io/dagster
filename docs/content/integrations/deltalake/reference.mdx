...
<Note>
  The delta-rs community is actively working on extending the awailable options for locking
  backends. This includes locking backends compatible with Databricks to allow concurrent
  writes from Databrticks and external environments.
</Note>
