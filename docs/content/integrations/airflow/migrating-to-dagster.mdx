...
Migrations are complicated as there are lots of moving pieces. It's important for you to go in knowing exactly how much scope and complexity you'll be tackling.
...
- **To know the following about your Airflow setup**:
  ...
  If you're unsure where to find these things, you're likely not the right person to make the migration. We recommend looping a teammate in before continuing.
...
4. Using `stdout/stderr`, identify and address the cause of the failure.
...
2. In the root of your project, create a `dagster_cloud.yaml` file with the following code:
...
3. Run the following to add the project to your Dagster Cloud deployment as a code location:

   ```bash
   dagster-cloud workspace add-location --from ./dagster_cloud.yaml
   ```
