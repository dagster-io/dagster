...
If you're running on Dagster Cloud and any of your connection fields are sensitive you can also pass them from [Environment Variables](/dagster-cloud/developing-testing/environment-variables-and-secrets) instead of hard coding them.

---

## Step 3: Run your migrated DAGs locally

Once we have a Repository defined we can launch dagit, view your newly migrated Jobs and launch runs.
...
Your airflow instance likely had specific IAM or Kubernetes permissions that allowed it to successfully run your airflow dags, in order to run the migrated dagster jobs you'll need to duplicate these permissions dagster.

In general if you can lean on [Airflow Connections](https://airflow.apache.org/docs/apache-airflow/stable/howto/connection.html) or [Environment Variables](/dagster-cloud/developing-testing/environment-variables-and-secrets) for defining these permissions it will be easier than modifying the underlying dagster infrastructure to assume roles that are permissioned correctly.

### KubernetesPodOperators

If your airflow dags were using [KubernetesPodOperators](https://airflow.apache.org/docs/apache-airflow-providers-cncf-kubernetes/stable/operators.html) its possible that you were using the `in_cluster` config or loading a kube_config file. When migrating switching to using a [Connection with a kube_config JSON blob](https://airflow.apache.org/docs/apache-airflow-providers-cncf-kubernetes/stable/connections/kubernetes.html) will be easier.
