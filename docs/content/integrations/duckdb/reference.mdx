...
If you want to store assets in different schemas, you can specify the schema as part of the asset's asset key. The schema will be the second-to-last value in the asset key. For `SourceAsset`s, the full asset key is specified using the `key` parameter. For Software-defined Assets, the full asset key is the `key_prefix` prepended to the asset name.
