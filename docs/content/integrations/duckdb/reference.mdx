...
- [Using the DuckDB resource in assets](#using-the-duckdb-resource-in-assets)
- [Using the DuckDB resource in ops](#using-the-duckdb-resource-in-ops)
...
For further information on the DuckDB resource, see the [DuckDB resource API docs](/\_apidocs/libraries/dagster-duckdb#dagster_duckdb.DuckDBResource).

### Using the DuckDB resource in assets
...
In this example, we attach the DuckDB resource to the `export_iris_data` op. In the body of the op function, we use the `get_connection` context manager on the resource to get a [`duckdb.DuckDBPyConnection`](https://duckdb.org/docs/api/python/reference/#duckdb.DuckDBPyConnection). We can then use this connection to execute a custom SQL query. In this case, we export the entire database to a new directory, `iris_copy` in CSV format.

---

## DuckDB I/O manager

The DuckDB I/O manager provides several ways to customize how your data is stored and loaded in DuckDB. However, if you find that these do not provide enough customization for your use case, we recommend using the DuckDB resource to save and load your data. By using the resource, you will have more fine grained control over how your data in handled, since you have full control over the SQL queries that are executed.
...
```python file=/integrations/duckdb/reference/schema.py startafter=start_asset_key endbefore=end_asset_key
import pandas as pd
