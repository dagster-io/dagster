...
The Snowflake IO manager supports storing a loading partitioned data.
...
from dagster import asset, StaticPartitionDefinition
import pandas as pd
...
Dagster uses the `partition_expr` metadata to craft the `SELECT` statement when loading the correct partition in a downstream asset. When loading a static partition, the statement `SELECT * WHERE <partition_expr> = <selected partition>` is used, so when materializing the **Iris-setosa** partition of the above asset, the query is `SELECT * WHERE SPECIES = 'Iris-setosa'`.

### Storing time partitioned assets

Like static partitioned assets, you can must specify `partition_expr` metadata on the asset to tell the Snowflake I/O manager which column contains the partition data:
...
Dagster uses the `partition_expr` metadata to craft the `SELECT` statement when loading the correct partition in a downstream asset. When loading a dynamic partition, the statement `SELECT * WHERE <partition_expr> >= <partition_start> AND <partition_expr> < <partition_end>` is used. The `<partition_start>` and `<partition_end>` bounds are of the form `YYYY-MM-DD HH:MM:SS`. So when materializing the **2023-01-02** partition the bound query is `SELECT * WHERE DATE >= '2023-01-02 00:00:00' AND DATE < '2023-01-03 00:00:00'`. If the data in your partition column cannot be directly compared to the time bounds generated by the Snowflake I/O manager, you can include a conversion in your `partition_expr` to convert the data in your column to the expected format.
...
from dagster import asset, StaticPartitionDefinition, MultiPartitionsDefinition, DailyPartitionsDefinition
import pandas as pd
    ...
    partitions_def=MultiPartitionsDefinition(
                {
                    ...
                    "species": StaticPartitionDefinition(["Iris-setosa", "Iris-virginica", "Iris-versicolor"]),
                }
            ),
...
Dagster uses the `partition_expr` metadata to craft the `SELECT` statement when loading the correct partition in a downstream asset. For multi partitions, Dagster concatenates the `WHERE` statements described in the above sections to craft the correct `SELECT` statement. For example, when materializing the **2023-01-02|Iris-setosa** partition, the query `SELECT * WHERE SPECIES = 'Iris-setosa' AND DATE >= '2023-01-02 00:00:00' AND DATE < '2023-01-03 00:00:00'` will be used.
