...
- Used the connection to execute a custom SQL query against the `IRIS_DATASET` table created in [Step 2](/integrations/snowflake/using-snowflake-with-dagster#store-a-dagster-asset-as-a-table-in-snowflake) of the [Snowflake resource tutorial](/integrations/snowflake/using-snowflake-with-dagster).
...
```python file=/integrations/snowflake/static_partition_io.py startafter=start_example endbefore=end_example
