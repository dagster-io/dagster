...
- **To install the `dagster-snowflake` and `dagster-snowflake-pandas` libraries**:
  ...
  Dagster also supports using PySpark DataFrames with the Snowflake I/O manager. The Snowflake-PySpark I/O manager works in the same way as the Snowflake-Pandas I/O manager, 
