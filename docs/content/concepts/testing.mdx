...
| <PyObject object="materialize_in_process"  />                      | Ephemerally materializes a provided list of software defined assets for testing.                                     |
...
An asset with input dependencies:
...
Resources provided on <PyObject object="build_op_context" /> will override any resources provided directly on the asset. This means you can avoid using production resources when testing.

```python file=/concepts/ops_jobs_graphs/unit_tests.py startafter=start_test_resource_override_asset endbefore=end_test_resource_override_asset
...
You may want to test multiple assets together, to more closely mirror actual materialization. This can be done using the <PyObject object="materialize_in_process" /> method, which loads the materialized results of assets into memory:
...
Mock resources can be provided directly using `materialize_in_process`:
