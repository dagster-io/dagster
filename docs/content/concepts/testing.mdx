...
| Name                                     | Description                                                                                                                                                                                                      |
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <PyObject object="execute_pipeline"  />  | A method to execute a pipeline synchronously, typically for testing pipeline execution or running standalone scripts. For more info, see the [Executing Pipelines](/concepts/solids-pipelines/executing) section |
| <PyObject object="execute_solid" />      | A method to execute a single solid in an ephemeral pipeline, intended to support unit tests.                                                                                                                     |
| <PyObject object="ExpectationResult"  /> | Dagster event corresponding to a data quality test.                                                                                                                                                              |

## Overview

Computations and pipelines in data applications are often relatively untested before hitting production because testing them is notoriously challenging.
...
2. Data applications usually do not control their inputs. They must take the data given and compute over it, which means that the computations encode numerous implicit, unexpressed assumptions about the incoming data.
...
- It also provides a mechanism to do [Data Quality Tests on Unexpected Inputs](#data-quality-tests-on-unexpected-inputs).
...
For more information, you can check out the [Modes](/concepts/modes-resources/modes) and [Resources](/concepts/modes-resources/resources) sections.

## Data Quality Tests on Unexpected Inputs

Dagster provides a mechanism for data quality tests, called **Expectations**. Expectations are an important concept in data applications because, typically, a data author does not control the inputs to that computation.

Traditionally, if a user presents an invalid input to an application, you can simply refuse to do the computation and report an error back. However, this is nearly impossible if in a data application. You would often depend on inputs or upstream steps that you do not control. If unexpected inputs happen to break the pipeline, you will likely have to update your code to adjust to the unexpected dependencies. As a result, you will bake many unexpressed, implicit assumptions about the inputs into the code.

To ensure a data pipeline works and reliable, testing the code and monitoring the pipeline are necessary but not sufficient. It is also critical to make those implicit assumptions explicit, and therefore test the data quality.

The **Expectations** mechanism lets you make those _implicit_ assumptions _explicit_. Expectations can be arbitrary computations -- from a simple type check to a file's existence, all the way to a distributional analysis of an entire data set. The semantics are up to you, as these expectations are user-defined.

We use <PyObject object="ExpectationResult" /> to indicate that an expectation has been executed. You can find more information about this mechanism in [Expectation Results](/concepts/solids-pipelines/solid-events#expectation-results).
