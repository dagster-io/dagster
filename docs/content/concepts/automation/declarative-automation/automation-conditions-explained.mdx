...
This condition translates to **Any upstream dependencies are part of an in-progress run or failed during the latest run**.
...
AutomationConditions generally depend on the status of their dependent assets. For example, `AutomationCondition.eager()` executes after a dependency updates, and `AutomationCondition.on_cron()` only executes after all dependencies have updated since a given cron schedule tick.
