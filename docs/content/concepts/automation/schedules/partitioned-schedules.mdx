...
If the current date is `2024-05-20`, this schedule would submit a run for the `2024-05-19` partition at `2025-05-20-01:30:00`.

### Customizing the ending partition in a set

<Note>
<strong>Heads up!</strong> The examples in this section use daily partitions, but the same logic also applies to other time-based partitions, such as hourly, weekly, and monthly partitions.
</Note>
...
After `2024-05-20 23:59:59` passes, the time window is complete and the `2024-05-20` partition will be included in the partition set.
...
daily_partition = DailyPartitionsDefinition(start_date="2024-05-10-00:00", end_offset=1)
```
