...
## Step 4: Verify that the UI is using the latest version of the schedule

The next step is to verify that the UI is using the latest version of the schedule code. Use the tabs to view instructions for the version of Dagster you're using.
...
## Step 4: Verify dagster-daemon setup
...
3. In the **Last tick** column, check for a **Failure** badge. If the schedule failed to execute, you can click the badge to display the error and stack trace describing the failure.
...
<ArticleList>
  ...
  ></ArticleListItem>
  <ArticleListItem title="OSS deployment" href="/deployment"></ArticleListItem>
</ArticleList>
