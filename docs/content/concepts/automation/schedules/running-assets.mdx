...
In this guide, we'll show you one method of accomplishing this by using schedules and jobs. If you're looking to do this for ops, refer to the [Defining schedules for ops guide](/concepts/automation/schedules/running-ops).
...
Next, we'll use <PyObject object="ScheduleDefinition" /> to construct the schedule and attach it to the job we created in [Step 1](#step-1-create-a-job).
...
Schedules need to be turned on before they can be used. In our case, we already turned the schedule on by using the `default_status` parameter in its <PyObject object="ScheduleDefinition" />, but there are a few other ways to do this:
...
Refer to the [Troubleshooting schedules](/concepts/automation/schedules/troubleshooting) guide if the schedule has been started but isn't submitting runs.
...
<ArticleList>
  <ArticleListItem
    ...
    href="/concepts/automation/schedules/running-ops"
  ></ArticleListItem>
  <ArticleListItem
