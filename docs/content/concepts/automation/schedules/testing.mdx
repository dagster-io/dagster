...
In this guide, we'll show you how to use the Dagster UI and Python to test your [schedules](/concepts/partitions-schedules-sensors/schedules).
...
A window containing the result of the evaluation will display after the test completes. If the evaluation was successful and a run request was produced, you can open the Launchpad with the pre-scaffolded config corresponding to that run request.
