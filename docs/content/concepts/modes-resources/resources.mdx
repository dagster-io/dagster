...
Resources are pipeline-scoped, and are typically used to expose features of the execution environment (like EMR, Redshift, Spark, BigQuery, etc.) to solids during pipeline execution. Resources bound to pipeline execution through [Modes](/concepts/modes-resources/modes), and can also clean up after execution resolves. They are typically defined using the <PyObject object="resource" decorator /> decorator or using the <PyObject object="ResourceDefinition" /> class directly.

The resource system enables:

- **Lightweight testing**: A resource is An intuitive and logical seam where application designers can provide where you can mock and fake implementations testing.
- **Business logic and environment separation** You can use this same seam to execute the same business logic in different environments and clouds.
- **Easy onfiguration** Resources declare their configuration in a self-describing way. You can configure them in the typeahead in Dagit and with high-quality error messages.
