...
**Note**: If a code location is re-named or its configuration in a workspace file is modified, you'll need to stop and re-start any running schedules or sensors in that code location. You can do this in Dagit by navigating to the [**Deployment overview** page](/concepts/dagit/dagit#deployment-overview) and using the **Schedules** and **Sensors** tabs.
...
## Running your own code server

By default, Dagster tools automatically create a process on your local machine for each of your code locations. However, it's also possible to run your own code server that's responsible for serving information about your code locations. This can be useful in more complex system architectures that deploy user code separately from Dagit.
...
To initialize the Dagster code server, run the `dagster code-server start` command and include:

- A target file or module. Similar to a workspace file, the target can either be a Python file or module.
- Host address
- Port or socket

The following tabs demonstrate some common ways to initialize a code server:
...
```shell
dagster code-server start --python-file /path/to/file.py --host 0.0.0.0 --port 4266
```

Running on a socket, using a Python file:

```shell
dagster code-server start --python-file /path/to/file.py --host 0.0.0.0 --socket /path/to/socket
```
...
```shell
dagster code-server start --module-name my_module_name --host 0.0.0.0 --port 4266
```
...
```shell
dagster code-server start --python-file /path/to/file.py --attribute my_repository --host 0.0.0.0 --port 4266
```
...
```shell
dagster code-server start --python-file /path/to/file.py --working-directory /var/my_working_dir --host 0.0.0.0 --port 4266
```
...
Refer to the [API docs](/\_apidocs/cli#dagster-code-server-start) for the full list of options that can be set when running a code server.

Then, in your workspace file, configure a new code server code location to load:
...
load_from:
  - code_server:
      host: localhost
      port: 4266
      location_name: "my_code_server"
```

### Specifying a Docker image

When running your own code server within a container, you can tell Dagit that any runs launched from a code location should be launched in a container with that same image.
...
If you're using the built-in [Helm chart](/deployment/guides/kubernetes/deploying-with-helm), this environment variable is automatically set on each of your code servers.
