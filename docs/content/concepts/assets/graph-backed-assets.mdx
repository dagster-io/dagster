...
<Note>
  This page assumes you have some familiarity with <code>ops</code> and{" "}
  <code>graphs</code>. To learn more about <code>ops</code> and{" "}
  <code>graphs</code>, see the documentation for <code>ops</code>
  <a href="https://docs.dagster.io/concepts/ops-jobs-graphs/ops">here</a> and <code>
    graphs
  </code>
  <a href="https://docs.dagster.io/concepts/ops-jobs-graphs/graphs">here</a> .
</Note>
...
If you want to write a graph-backed asset that results in the materialization of multiple assets, you can use the <PyObject object="graph_multi_asset" decorator /> decorator.

In the below example, `two_assets` depends on the asset `ten`. `two_assets` is a graph-backed multi-asset that is composed of two ops, `multiply_by_two` and `divide_by_three`:
