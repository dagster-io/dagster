...
One way of recording materialization events is to explicitly log <PyObject module="dagster" object="AssetMaterialization" /> events at runtime. These events should be co-located with your materialization logic, meaning if you store your object within your op body, then you should log from within that op, using the <PyObject module="dagster" object="OpExecutionContext" method="log_event"/> method. If you store your object using an <PyObject module="dagster" object="IOManager" />, then you should log the event using <PyObject module="dagster" object="OutputContext" method="log_event"/> in the <PyObject module="dagster" object="IOManager" method="handle_output"/> method.

### Logging an AssetMaterialization from a Op

To make Dagster aware that we materialized an asset in our op, we can log a <PyObject module="dagster" object="AssetMaterialization" /> event. This would involve changing the following op:
...
To record that an <PyObject object ="IOManager"/> has mutated or created an asset, we can log an <PyObject module="dagster" object="AssetMaterialization" /> event from its `handle_output` method.
...
Just as there are two places in which you can log runtime <PyObject object="AssetMaterialization" /> events (within an op body and within an IOManager), we provide two different interfaces for linking an op output to to an asset. Regardless of which you choose, every time the op runs and returns that output, an <PyObject object="AssetMaterialization" /> event will automatically be created to record this information.

If you used <PyObject object="OpExecutionContext" method="add_output_metadata" /> to specify metadata for an output (see: [Op Event Docs](/concepts/ops-jobs-graphs/op-events#attaching-metadata-to-outputs)), these entries will automatically be attached to the materialization event for this asset.
...
When an output is linked to an asset in this way, the generated <PyObject object="AssetMaterialization" /> event will contain any <PyObject object="EventMetadataEntry" /> information added from the `handle_output` function (in addition to all of the output `metadata` specified within the op.

See the [IOManager docs](/concepts/io-management/io-managers#adding-output-metadata-from-an-iomanager) for more information on adding output metadata from an IOManager.
...
### Legacy syntax for AssetMaterializations

We also support yielding <PyObject object="AssetMaterialization" /> objects from within the body of an op, although it is preferred to use the <PyObject object="OpExecutionContext" method="log_event" /> method. When using the yield syntax, it is also necessary to yield all Outputs.

```python file=/concepts/assets/materialization_ops.py startafter=start_materialization_yield endbefore=end_materialization_yield
from dagster import op, AssetMaterialization, Output
