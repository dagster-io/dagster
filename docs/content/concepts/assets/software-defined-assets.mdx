...
Since a software-defined asset contains an op, all the typical functionality of an op - like the use of [resources](/concepts/resources) - is available to an asset. Supplying the `context` parameter provides access to system information for the op, for example:
...
Like ops, configuration is also supported for assets. Configuration is accesible through the asset context at runtime and can be used to specify behavior. Note that asset configuration behaves the same as configuration for ops.

For example, we can define an asset where the API endpoint it queries is defined through its configuration:
...
We recommend providing a `config_schema` when utilizing configuration for assets. Refer to the [Config schema documentation](/concepts/configuration/config-schema) for more info and examples.
