...
## Loading asset values dynamically

### Outside of a Dagster run
...
It is valuable to be able to load the contents of an asset inside of a job, such as while loading a dataset for validation or testing of a pre-trained ML model.

For this, we can still use <PyObject object="Definitions" method="load_asset_value" />:

```python file=/concepts/assets/load_asset_values.py startafter=partitioned_asset_dynamically_start_marker endbefore=partitioned_asset_dynamically_end_marker dedent=4
@op
