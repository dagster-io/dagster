...
- **Dagit can show asset lineage that includes the source assets**. If a different asset definition in a different repository in the same [workspace](/concepts/code-locations/workspace-files) has the same asset key as a <PyObject object="SourceAsset" />, Dagit can represent the asset lineage across those repositories.
