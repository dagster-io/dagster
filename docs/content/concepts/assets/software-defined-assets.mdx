...
Dagster supports attaching arbitrary metadata to assets. To attach arbitrary metadata, supply a `metadata` dictionary to the asset:

```python file=/concepts/assets/asset_metadata.py startafter=start_simple_metadata endbefore=end_simple_metadata
from dagster import asset
...
To specify data types for metadata values, use the <PyObject object="MetadataValue"/> class:

```python file=/concepts/assets/asset_metadata.py startafter=start_typed_metadata endbefore=end_typed_metadata
...
If a type is provided using <PyObject object="MetadataValue"/>, Dagster will use it to validate the metadata value.

If a type isn't provided - such as `cereal_name` in the above example - Dagster will attempt validation using each supported <PyObject object="MetadataValue"/> type. If validation fails, the value will be typed as a string.

Asset metadata can be viewed in Dagit on the \[TODO: PAGE]
