...
- [Graph-backed assets and multi-assets](#graph-backed-assets-and-multi-assets) # TODO replace with eventual title rename
...
### Writing more computationally complex assets

If you'd like to define more complex assets, Dagster offers augmented software-defined asset abstractions:

- [Multi-assets](/concepts/assets/multi-assets) allow you to update multiple data assets using one function
- [Graph-backed assets](/concepts/assets/graph-backed-assets) allow you to split the computation for an asset into multiple functions. This allows you to enable parallelization and launch re-executions of individual functions.

### Asset configuration

Assets in Dagster can specify a config schema. This allows you to provide values to the asset at run time. The configuration system is explained in detail in the [Config schema documentation](/concepts/configuration/config-schema).
...
When writing an asset, users can optionally include a first parameter, `context`. When this parameter is included, Dagster will supply an <PyObject object="AssetExecutionContext"/> object to the body of the asset which provides access to system information like loggers and the current run id.
...
When an asset with a code version is materialized, it is tagged with the version. The UI will indicate when an asset has a different code version than the code version used for its most recent materialization.
...
If you want the UI to show both assets and [jobs](/concepts/ops-jobs-graphs/jobs) that target the assets, you can place the assets and jobs together inside a <PyObject object="Definitions" /> object. For example:
...
<!-- ![Asset catalog page in the UI](/images/concepts/webserver/all-assets.png) -->
...
<!-- ![Asset details page in the UI](/images/concepts/webserver/asset.png) -->
...
Dagster supports more complex metadata types as well. For example, you can render Markdown by using <PyObject object="MetadataValue" method="md" /> or clickable URLs using <PyObject object="MetadataValue" method="url" />:

```python file=/concepts/assets/asset_materialization_metadata_with_metadata_value.py startafter=start endbefore=end
...
For a full list of supported metadata types, see <PyObject object="MetadataValue" />.
