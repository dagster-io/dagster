...
Asset functions can specify an annotated `config` parameter for the assets's configuration. The config class, which subclasses <PyObject object="Config"/> (which wraps [`pydantic.BaseModel`](https://docs.pydantic.dev/usage/models/#basic-model-usage)) specifies the configuration schema for the asset.
...
def test_uses_resource() -> None:
    result = uses_config_and_resource(
        config=MyConfig(api_url=...), my_api=MyAPIResource()
    )
    assert result == "bar"
```
