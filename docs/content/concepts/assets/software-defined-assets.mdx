...
In some cases, an asset may not need to be updated in storage each time the decorated function is executed. In this case you can use the `always_materialize` parameter along with `yield` syntax to invoke this behavior. If the `always_materialize` parameter is set to `False`, you may indicate to the Dagster framework that no data should be persisted to storage by not yielding an output from your computation function. If an output is not emitted during computation, no asset materialization event will be created, the I/O manager will not be invoked, downstream assets will not be materialized, and asset sensors monitoring the asset will not trigger.
