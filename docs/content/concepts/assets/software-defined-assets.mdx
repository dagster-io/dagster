...
Since a function decorated with <PyObject object="asset" /> acts as the op for a software defined asset, you can write unit tests for this function in the same way you would write unit tests for a standard op. See this section on [op testing](/concepts/testing#testing-ops) for some background information.

Similar to the `@op` decorator, the `@asset` decorator can slightly modify the call signiture of the decorated function. However, the new signiture closely mirrors the signature of the decorated function, and in some cases, they match exactly.

For instance, if you have a simple asset that does not need a context object, a unit test can directly invoke the asset using the same call signature as the decorated function.

```python file=/concepts/assets/asset_testing.py startafter=start_no_context endbefore=end_no_context
@asset
def my_simple_asset():
    return [1, 2, 3]
...
Notice how you can pass any value to `more_complex_asset` when you directly invoke it in the unit test. This allows you to test assets in isolation from one another.

If you use a context object in your function, `@asset` will provide the correct context during execution. But when writing a unit test, you can mock it with <PyObject object="build_op_context" />. We can use <PyObject object="build_op_context" /> because under the hood the function decorated by `@asset` is an op.

```python file=/concepts/assets/asset_testing.py startafter=start_with_context endbefore=end_with_context
@asset
