...
Assets may be assigned a `code_version`:
...
When an asset with a code version is materialized, the generated `AssetMaterialization` is tagged with the version. The code version together with the <PyObject object="LogicalVersion" /> values of upstream assets are hashed to create a `LogicalVersion` for the asset, also stored on the `AssetMaterialization`, which is used to track asset staleness. Updating the `code_version` will change the projected `LogicalVersion` of the asset for a new materialization, thus rendering it stale.
...
Just as with regular assets, these versions be attached to the `AssetMaterialization` objects for each of the constituent assets and accounted for in calculation of asset staleness.
