...
- [Dependency graph](#asset-dependency-graph)
...
- `upstream` is upstream of `downstream`
- `downstream` depends on `upstream`
- `downstream` was last materialized on **June 1**
- `upstream` was last materialized on **June 2**

In this case, the contents of `downstream` may have outdated, as the most recent data from `upstream` wasn't used to compute them.
...
<Note>
    Currently, the <strong>upstream changed</strong> indicator won't display in the following scenarios:
    <ul>
