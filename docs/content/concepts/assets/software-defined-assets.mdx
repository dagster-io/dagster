...
- [By using the `load_assets_from_package_module` function](#from-assets-in-a-sub-module) (**recommended**)
...
Assets can also be grouped on an individual basis by specifying an argument when creating the asset:
...
To view your asset groups in Dagit, open the left navigation ([todo-menu path]). As asset groups are grouped in repositories, you may need to open a repository to view its asset groups.

For example, in the following image, the `hacker_news_assets_prod` repository is open and the `default` asset group is visible:
...
Click the asset group to open a dependency graph. This graph includes all assets in the group:
