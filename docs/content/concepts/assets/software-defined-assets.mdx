...
In some cases, an asset may not need to be updated in storage each time the decorated function is executed. In this case you can use the `always_materialize` parameter along with `yield` syntax to only materialize an asset to storage in some cases. If an asset is not materialized to storage, downstream assets will not be materialized, and asset sensors monitoring the asset will not trigger
