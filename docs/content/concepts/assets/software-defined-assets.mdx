...
[Basic software-defined assets](#a-basic-software-defined-asset) can only produce one data artifact. If your use case requires an asset that produces more than one data artifact, you can use graph-backed assets.
...
If creating a job using a graph-backed asset, select all graph outputs using `asset_selection`:

<TODO />

**Note**: All graph outputs must be selected when using a graph-backed asset to create a job.
