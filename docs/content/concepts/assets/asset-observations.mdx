...
<PyObject object="SourceAsset" /> objects may have a user-defined <em>
observation function </em> that returns a <PyObject object="LogicalVersion" />. Whenever the
observation function is run, an <PyObject object="AssetObservation" /> will be
generated for the source asset and tagged with the returned logical version. The
logical version is used in staleness calculations for downstream assets.
