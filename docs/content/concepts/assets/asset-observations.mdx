...
<PyObject object="SourceAsset" /> objects may have a user-defiend _observation function_
...
The <PyObject object="observable_source_asset" /> decorator provides a conveninet way to define source assets with observation functions. The below observable source asset takes a file hash and returns it as the logical version. Every time you run the observation function, a new observation will be generated with this hash set as logical verison.

```python file=/concepts/assets/observable_source_assets.py startafter=start endbefore=end
