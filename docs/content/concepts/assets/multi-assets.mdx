...
Like regular assets, multi-assets may assign different code versions for each of their outputs. Versions let you help Dagster track what assets haven't been re-materialized since their code has changed, and avoid performing redundant computation.
...
Just as with regular assets, these versions are attached to the materializations for each of the constituent assets and represented in the UI. The UI will indicate when an asset has a different code version than the code version used for its most recent materialization.

To learn more about code versions, see the [Asset versioning and caching](/guides/dagster/asset-versioning-and-caching) guide.
