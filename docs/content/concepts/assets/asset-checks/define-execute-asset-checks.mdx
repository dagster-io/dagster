...
Depending on your needs, there are a few ways you can define asset checks:

| I want to...                                              | Which will...                                              | You should use...                                                   |
| --------------------------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------- |
| Define checks separately from targeted assets             | Execute asset materializations and checks in their own ops | [`@asset_check` decorator](#defining-checks-separately-from-assets) |
| Define checks together with targeted assets               | Execute asset materializations and checks in the same op   | [`AssetCheckSpec`](#defining-checks-and-assets-together)            |
| Define multiple, similar checks                           | Execute checks in their own ops                            | [A factory pattern](#using-a-factory)                               |
| Mitigate performance issues or execute a subset of checks | Execute multiple checks in an op without an asset          | [`@multi_asset_check` decorator](#TODO)                             |
| Load my dbt tests as checks                               | TODO                                                       | [`DagsterDbtTranslator`](#loading-dbt-tests-as-asset-checks)        |
...
### TODO: multi_asset_check

Using the <PyObject object="multi_asset_check" decorator />, you can define multiple checks that execute in an op without an asset. This approach can help mitigate performance issues for expensive computations.

For example, you're using Dagster to perform some statistical analysis and you want to execute checks against a few of the computed metrics. If you have a metric that's expensive to compute and shared across multiple checks, you can use <PyObject object="multi_asset_check" decorator /> to avoid having to re-compute the metric each time a check is executed.

```python
TODO - EXAMPLE
```

You can also use the <PyObject object="multi_asset_check" decorator /> to execute subsets of asset checks. Refer to the [Subsetting asset checks](/concepts/assets/asset-checks/subsetting-asset-checks) guide for more information.
