...
# Defining and executing asset checks (Experimental)
...
There are a few methods you can use to define an asset check:

- **Define separate functions for assets and checks** - In this approach, asset materialization and asset checks are executed in their own separate ops. If using the <PyObject object="multiprocess_executor" />, this allows you to launch runs that will use separate processes to materialize the asset and execute its check.
- **Define assets and checks in the same function** - In this approach, checks execute in the same op that materializes the asset.
- **Define an asset check factory** - Useful for defining multiple, similar asset checks at once
...
The severity determines how the check result will display in the Dagster UI. For example, if a check fails with `ERROR` severity, the asset will appear red in the lineage graph.
