...
In this section, we'll show various modifications to asset checks.
...
To block downstream assets from executing when checks fail, set the `blocking` argument to `True` in the <PyObject object="asset_check" /> decorator. In the following example, `check_upstream_asset` will block `downstream_asset` from executing.
...
When the `blocking` toggle is enabled, downstream assets will wait to execute until the check completes. If the check returns a failing <PyObject object="AssetCheckResult" /> or raises an exception, downstream assets will not execute. If the <PyObject object="AssetCheckSeverity"/> is set to `WARN` instead of `ERROR`, the downstreams will still execute.

Limitations:

- `blocking` is currently only supported with `@asset_check`. For checks defined in the same op as assets, you can explicitly raise an exception to block downstream execution.
