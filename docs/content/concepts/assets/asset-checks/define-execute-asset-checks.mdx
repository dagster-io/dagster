...
| --------------------------------------------- | ---------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | - |
| Define checks separately from targeted assets | Execute asset materializations and checks in their own ops | [`@asset_check`](#defining-checks-separately-from-assets), or [`@multi_asset_check`](#defining-multiple-checks) for multiple checks |   |
...
Using <PyObject object="multi_asset_check" decorator />, you can define multiple checks that execute in an op without an asset. This approach can help mitigate performance issues for expensive computations.
