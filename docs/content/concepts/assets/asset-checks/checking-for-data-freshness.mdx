...
- **For assets with materializations**, Dagster infers the asset's last update time using the its latest materialization timestamp. In this guide, we'll demonstrate how to use [`@asset`-decorated assets and a sensor](#defining-freshness-checks-for-materializable-assets) to achieve this.

- **For assets without materializations**, which are assets kept up-to-date outside of Dagster, freshness checks depend on asset observations to report the time the asset was last updated. In this guide, we'll demonstrate how to use [observable assets and a schedule](#defining-freshness-checks-for-source-assets) to achieve this.
...
## Defining freshness checks for assets kept up-to-date outside of Dagster

In this section, we'll demonstrate how to implement freshness checks for assets kept up-to-date outside of Dagster. A common use case is monitoring assets that your Dagster pipeline depends on.

### Step 1: Track the asset's last update time

To run freshness checks on assets kept up-to-date outside of Dagster, the checks need to know when the assets were last updated. [Observable assets](/concepts/assets/asset-observations#observable-source-assets) - in this case, <PyObject object="multi_observable_asset" decorator /> can be used to track the update times of these assets.
...
Next, we'll define a [schedule](/concepts/partitions-schedules-sensors/schedules) that regularly executes the function in the `source_tables` observable asset:
...
>>>>>>> e5d969334a (remove source asset from concept docs)
