...
How Dagster determines the last update time varies on the type of asset being targeted:

- **Assets where every update is recorded as a Dagster materialization event** – In this case, which is typical for assets orchestrated by Dagster, Dagster simply infers the asset's last update time using the timestamp on its latest materialization. In this guide, we'll demonstrate how to use [`@asset`-decorated assets and a sensor](#defining-freshness-checks-for-materializable-assets) to achieve this.

- **Assets whose updates aren't recorded as materializations, but that can be observed by Dagster to determine their last update time.** - In this case, which is common for assets that are upstream of a Dagster data pipeline, Dagster kicks off code to observe the asset on a regular cadence and fetch a last update timestamp from an external system. In this guide, we'll demonstrate how to use [observable assets](#defining-freshness-checks-for-observable-assets) to achieve this.
...
## Defining freshness checks for observable assets

In this section, we'll demonstrate how to implement freshness checks for observable assets, i.e. assets that are defined with an observation function that can check their last update time. A common use case is monitoring assets that your Dagster pipeline depends on.

### Step 1: Track the asset's last update time

The <PyObject object="multi_observable_asset" decorator /> can be used to track the update times of multiple assets.

The following example implements this using the <PyObject object="multi_observable_asset" decorator /> decorator. In it, multiple Snowflake tables are backed by an observation function that queries Snowflake to find the most recent time the tables were updated. The function yields the update time as metadata to be stored in the Dagster event log:
...
Next, we'll define a [schedule](/concepts/automation/schedules) that regularly executes the function in the `source_tables` observable asset:
...
These checks will automatically execute after the observations of the assets they target, so an additional schedule isn't needed.
...
| <PyObject object="ObserveResult" />                         | An object representing a successful observation of an asset. Used with observable assets to pass metadata. |
