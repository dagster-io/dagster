...
Using asset checks, you can define and execute data quality checks on your [data assets](/concepts/assets/software-defined-assets) directly in Dagster. Each asset check tests some property of a data asset, such as:
...
An asset check is a data quality check, [defined in code](/concepts/assets/asset-checks/define-execute-asset-checks), that targets one or more Dagster assets. You can use Python to define them, or if using dbt, load your [dbt tests into Dagster as asset checks](/integrations/dbt/reference#loading-dbt-tests-as-asset-checks).
