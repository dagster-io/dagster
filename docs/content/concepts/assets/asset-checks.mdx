...
You can use <PyObject object="define_asset_job" /> to define jobs that include sets of both assets and checks, and then execute those jobs via [sensors](/concepts/partitions-schedules-sensors/sensors) and [schedules](/concepts/partitions-schedules-sensors/schedules). By default checks are included with the assets they check. You can also define jobs that include only checks, or only assets.
