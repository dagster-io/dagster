...
Sometimes if a check fails, you want to prevent any downstream assets from materializing. There are different ways to do this depending on whether you're writing a separate `@asset_check` or including the check in the `@asset`.
