...
Note that the [Dagster Daemon](/deployment/dagster-daemon) needs to be running for auto-materialization to function. If you use the `dagster dev` command to run Dagster, the daemon will automatically run as part of it, so assets will immediately start to be auto-materialized when Dagster starts.
...
- They fall within the `time_window_partition_scope_minutes` (more on this below)
...
If a partition of `asset1` is re-materialized, e.g. manually from the UI, then the corresponding partition of `asset2` will be auto-materialized after.

To avoid accidentally auto-materializing enormous numbers of partitions, <PyObject object="AutoMaterializePolicy" pluralize /> have a `time_window_partition_scope_minutes`, which limits how far back in time partitions will be auto-materalized. It defaults to the latest partition. To materialize earlier partitions, launch a backfill or set `time_window_partition_scope_minutes` to a larger value.
