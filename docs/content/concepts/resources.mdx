...
Resources can be configured using environment variables, which is useful for secrets or other environment-specific configuration. If you're using [Dagster Cloud](/dagster-plus), environment variables can be [configured directly in the UI](/dagster-plus/managing-deployments/environment-variables-and-secrets).
