...
| <PyObject object="ConfigurableResourceFactory"/> | A base class extended to define resources which pass plain Python objects to user code.                                                                                                                                                 |
...
Because `credentials` requires runtime configuration through the launchpad, it must also be passed to the <PyObject object="Definitions" /> object, so that configuration can be provided at runtime. Nested resources only need to be passed to the <PyObject object="Definitions" /> object if they require runtime configuration.
...
If you are interested in adapting a legacy, function-styled I/O manager, see the [adapting function-style I/O managers section](#adapting-function-style-io-managers).
...
```python file=/concepts/resources/pythonic_resources.py startafter=start_resource_adapter endbefore=end_resource_adapter dedent=4
from dagster import (
    resource,
    Definitions,
    ResourceDefinition,
...
```python file=/concepts/resources/pythonic_resources.py startafter=start_io_adapter endbefore=end_io_adapter dedent=4
from dagster import (
    Definitions,
    IOManagerDefinition,
    io_manager,
...
#### Config with bare Python objects

If you want to attach any config to your Python objects, such as credentials or tokens, you can use <PyObject object="ConfigurableResourceFactory"/>. Similar to a <PyObject object="ConfigurableResource"/>, any fields on the factory will be treated as config fields. The factory should implement a `create_resource` method which returns the bare Python object.

```python file=/concepts/resources/pythonic_resources.py startafter=start_raw_github_resource_factory endbefore=end_raw_github_resource_factory dedent=4
...
#### Dependencies on bare Python objects
...
```python file=/concepts/resources/pythonic_resources.py startafter=start_raw_github_resource_dep endbefore=end_raw_github_resource_dep dedent=4
