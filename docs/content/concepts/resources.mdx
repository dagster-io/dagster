...
Resources provide a way to manage dependencies to external components and share implementations across multiple [ops](/concepts/ops-jobs-graphs/ops) or [assets](/concepts/assets/software-defined-assets) in a [job](/concepts/ops-jobs-graphs/jobs-graphs).
...
So, **why use resources**? Using resources has a variety of benefits, as they are:
...
As assets are functionally similar to ops, assets also use resource keys to access resources:

<!-- This hard-coded example is temporary. -->
...
def asset_requires_resources(context):
    context.resources.database.execute_query(CREATE_TABLE_1_QUERY)
```
...
<PyObject object="ResourceDefinition" pluralize /> can have a config schema, which allows you to customize behavior at runtime through run configuration.
