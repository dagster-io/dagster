...
| Name                                     | Description |
| ---------------------------------------- | ----------- |
| <PyObject object="executor" decorator /> | TODO        |
...
What executor is used is determined by two things. First, modes provide the _possible_ set of executors one can use. In order to set this use the `executor_defs` property on <PyObject module="dagster" object="ModeDefinition" />. Second, the `execution` config section of the run config determines the actual executor.
