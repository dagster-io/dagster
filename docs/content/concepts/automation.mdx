...
Sensors launch runs in response to a detected event. Sensors periodically check and execute logic to detect an event and conditionally launch runs. They are commonly used for situations where you want to materialize an asset on some externally observable trigger, such as:
