...
<Note>
  ...
  portion of the API is primarily for internal use by Dagit. For any of the
  queries below, we will be clear about breaking changes in release notes.
</Note>
...
Dagster also provides a Python client to interface with Dagster's GraphQL API from Python. Refer to the <Link href="/concepts/dagit/graphql-client">GraphQL Python client docs</Link> for more info.
