...
<Note>This guide is applicable only to Dagster Open Source (OS).</Note>

Workspace files contain a collection of user-defined [code locations](/concepts/code-locations) and information about where to find them. Workspace files can accept code locations defined using a <PyObject object="Definitions" /> object or a <PyObject object="repository" decorator /> decorator.

Prior to the introduction of <PyObject object="Definitions" />, workspace files were used to enable Dagster tools to load user code in tools like Dagit or the CLI. In a <PyObject object="Definitions" /> world, workspace files are only required to load multiple code locations in customized environments, such as part of an OSS deployment.

For local development within a single Python environment, <PyObject object="Definitions" /> users can use the `-m` or `-f` flags with our CLI tools, or set the `pyproject.toml` file to avoid using command line flags entirely.
...
Each entry in a workspace file is considered a code location.
...
- A target file or module. Similar to a workspace file, the target can either be a Python file or package.
- Host address
- Port or socket
