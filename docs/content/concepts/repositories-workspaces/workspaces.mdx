...
To accommodate incrementally migrating from <PyObject object="repository" decorator /> to <PyObject object="Definitions" />, code locations in a single workspace file can mix and match between definition approaches. For example, `code-location-1` could load a single <PyObject object="Definitions" /> file and `code-location-2` could load multiple repositories.
