...
description: Dagster Workspaces are collections of user-defined code locations and information about where they reside.
---

# Workspaces

A workspace is a collection of user-defined [code locations](/concepts/code-locations) and information about where to find them. Workspaces can accept code locations defined using a <PyObject object="Definitions" /> object or a <PyObject object="repository" decorator /> decorator.

Prior to the introduction of <PyObject object="Definitions" />, workspaces were used to enable Dagster tools to load user code in tools like Dagit or the CLI. In a `Definitions` world, workspaces are only required to load multiple code locations as part of an [open source (OSS) deployment](/deployment). For local development, `Definitions` users can utilize a [`pyproject.toml` file](/concepts/code-locations).
...
The structure of a workspace is encoded in a YAML document, named `workspace.yaml` by default, that tells Dagster where to find code locations. For example:
...
Each entry in `workspace.yaml` is considered a code location.
...
**If using <PyObject object="repository" decorator /> to define code locations**, you can identify a single repository within the package using the `attribute` key. The value of this key must be the name of a repository or the name of a function that returns a <PyObject object="RepositoryDefinition" />. For example:
...
<TabItem name="From a Python package">

### From a Python package

To load a code location from a local or installed Python package, use the `python_package` key in `workspace.yaml`:
...
**If using <PyObject object="repository" decorator /> to define code locations**, you can identify a single repository within the package using the `attribute` key:
...
When `dagit` is run, Dagit will load all the code locations defined by the workspace. Refer to the [CLI reference](/\_apidocs/cli#dagit) for more info and examples.
...
**Note**: If a code location is re-named or its configuration is modified in `workspace.yaml`, you'll need to stop and re-start any running schedules or sensors in that code location. You can do this in Dagit by navigating to the [**Instance status** page](/concepts/dagit/dagit#instance-status) and using the **Schedules** and **Sensors** tabs.

This is required because when you start chedule or a sensor, a serialized representation of the entry in your `workspace.yaml` file is stored in a database. The dagster-daemon process uses this serialized representation to identify and load your schedule or sensor. If the code location is modified and its schedules and sensors aren't restarted, the dagster-daemon process will use an outdated serialized representation, resulting in issues.
...
Then, in your `workspace.yaml`, configure a new gRPC server code location to load:
