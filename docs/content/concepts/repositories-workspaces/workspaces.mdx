...
Each entry in a workspace file is considered a code location. A code location can contain either a single <PyObject object="Definitions" /> object, a repository, or multiple repositories. Additionally, code locations in a single workspace file can mix and match between <PyObject object="Definitions" /> and repositories. For example, `code-location-1` could load a single <PyObject object="Definitions" /> file and `code-location-2` could load multiple repositories.
