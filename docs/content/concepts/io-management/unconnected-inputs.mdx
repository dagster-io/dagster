...
Dagster provides a couple ways to achieve this:

One option is to write a function to load the input and decorate it with <PyObject module="dagster" object="input_manager" />:
...
If you want to use `Table1InputManager` to store outputs, or you want to override the `load_input` method of the IO Manager used elsewhere in the job, another option is to implement an instance of <PyObject module="dagster" object="IOManager" />:
...
No match for endBefore value "end_load_unconnected_io"
```
...
So far, this is set up so that `op2` always loads `table_1` even if you execute the full job. To write this so that `op2` only loads `table_1` when no input is provided from an upstream op, we can rewrite the input manager as a subclass of the IO manager used for the rest of the job as follows:
