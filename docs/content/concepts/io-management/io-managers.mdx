...
### Customizing how assets are loaded downstream

In some cases, you may find that you want to write some custom logic for how an upstream asset is loaded in a downstream asset. For example, if an asset is set to store and load as a pandas DataFrame, but you want to load it with PySpark in a downstream asset, it may make more sense to write a new loader for the downstream asset than to update the upstream asset's IO Manager to handle loading as a pandas DataFrame or a PySpark dataframe.

If you know how an upstream asset is being stored, you can write a new IO Manager that subclasses the upstream asset's IO Manager with your new loading logic.

For example, if we store `upstream_asset` as a Pandas DataFrame, we can write a new IO Manager to load is as a NumPy array in `downstream_asset`
