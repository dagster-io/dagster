...
The default <PyObject object="PickledObjectFilesystemIOManager" /> IO Manager inherits from the `UPathIOManager` and therefore has all of these features too.
...
## Defining filesystem-based IO Managers

The <PyObject module="dagster._core.storage.upath_io_manager" object="UPathIOManager" /> can be easily used to write specific filesystem-based IO Managers. It already implements the `load_input` and `handle_output` methods. Instead, saving and loading for a given `universal_pathlib.UPath` has to be implemented. Here are some examples:

```python file=/concepts/io_management/upath_io_manager.py startafter=start_upath_io_manager endbefore=end_upath_io_manager
...
The default IO Manager has support for loading multiple upstream partitions. In this case, the downstream asset should use `Dict[str, ...]` type for the upstream `DagsterType` (otherwise an error will be raised). Here is an example of loading multiple upstream partitions:

```python file=/concepts/io_management/loading_multiple_upstream_partitions.py startafter=start_loading_multiple_upstream_partitions endbefore=end_loading_multiple_upstream_partitions
start = datetime.datetime(2022, 1, 1)

hourly = HourlyPartitionsDefinition(start_date=f"{start:%Y-%m-%d-%H:%M}")
...
def downstream(upstream: Dict[str, pd.DataFrame]) -> pd.DataFrame:  # Dict type annotation is important here
    # hourly_asset is now a dictionary which maps partition keys to partitions
    # the hourly partitions have a default behavior for mapping to daily partitions (24 hours -> 1 day).
    return pd.concat(list(upstream.values()))
```
