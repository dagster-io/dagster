...
Dagster ops have [inputs and outputs](/concepts/solids-pipelines/solids#inputs-and-outputs). When an op returns an output and a downstream op takes that output as an input, where does the data live in between? <PyObject module="dagster" object="IOManager" pluralize /> let the user decide.
