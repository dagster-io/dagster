...
Dagster solids have [inputs and outputs](/concepts/solids-pipelines/solids#inputs-and-outputs). When a solid returns an output and a downstream solid takes that output as an input, where does the data live in between? <PyObject module="dagster" object="IOManager" pluralize /> let the user decide.
...
Each solid output can have its own IOManager, or multiple solid outputs can share an IOManager. The IOManager that's used for handling a particular solid output is automatically used for loading it in downstream solids.
...
IOManagers are [resources](/concepts/modes-resources), which means users can supply different IOManagers for the same solid outputs in different situations. For example, you might use an in-memory IOManager for unit-testing a pipeline and an S3IOManager in production.
...
To define an IO manager, extend the <PyObject module="dagster" object="IOManager" /> class.
...
The easiest way to test an IO manager is to construct an <PyObject module="dagster" object="OutputContext" /> or <PyObject module="dagster" object="InputContext" /> and pass it to the `handle_output` or `load_input` method of the IO manager.
