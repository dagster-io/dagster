...
Code locations **in a workspace** are loaded in a different process and communicate with Dagster system processes over an RPC mechanism. This architecture provides several advantages:
...
```shell
pip install -e path/to/your/package_name
```
...
The `dagster_cloud.yaml` file is used to create and deploy code locations for Cloud deployments. Each code location entry in this file has a `code_source` property, which is used to specify how a code location is sourced. Code locations can be sourced from a Python file, package, or module:
...
To load a code location from a Python package, use the `package_name` property in your `dagster_cloud.yaml`:
...
Interested in how `Definitions` and repositories differ when it comes to defining code locations? Check out the following table for a high-level comparison:
