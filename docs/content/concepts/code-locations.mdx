...
The [`dagster_cloud.yaml`](/dagster-plus/managing-deployments/dagster-cloud-yaml) file is used to create and deploy code locations for Cloud deployments. Each code location entry in this file has a `code_source` property, which is used to specify how a code location is sourced. Code locations can be sourced from a Python file or module:
