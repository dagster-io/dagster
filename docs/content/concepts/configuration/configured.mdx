...
The `configured` API offers a way to provide configuration at definition time. When invoked on a Dagster entity, it returns an interchangeable object with the given configuration "baked in". It is available with the following definitions:
