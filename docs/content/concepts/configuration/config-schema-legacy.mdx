...
From the UI's [Launchpad](/concepts/dagster-ui#launchpad-tab), we supply config as YAML using the config editor. Refer to [the config schema guide](/concepts/configuration/config-schema#dagster-ui) for using runtime config in the Dagster UI.
