...
From the UI's [Launchpad](/concepts/webserver/ui#launchpad-tab), we supply config as YAML using the config editor. Refer to [the config schema guide](/concepts/configuration/config-schema#dagster-ui) for using runtime config in the Dagster UI.
...
By providing secrets through environment variables, your secrets won't be visible in your code or the UI's launchpad. Refer to the [Using environment variables and secrets in Dagster code guide](/guides/dagster/using-environment-variables-and-secrets) for more info and examples.
