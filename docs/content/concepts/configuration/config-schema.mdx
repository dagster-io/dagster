...
<Note>
  It is technically possible to access <code>context.op_config</code> inside ops (not
  assets) without defining a <code>config_schema</code>. However, this is not recommended.
</Note>
