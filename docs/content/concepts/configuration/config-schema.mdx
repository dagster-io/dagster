...
A common use case for configuration is passing secrets to connect to external services. Resources, which can be used to model connections to external services, accept secrets as configuration values. These secrets can be read from your environment variables given the name of the variable. For example, Dagster's out-of-the-box [Snowflake resource](/_apidocs/libraries/dagster-snowflake#dagster_snowflake.snowflake_resource) can be configured using environment variables:

```python file=/concepts/configuration/snowflake_resource_config.py
...
### Providing Per-Environment Configuration
...
For more examples of jobs, check out the following in our [Hacker News example](https://github.com/dagster-io/dagster/tree/master/examples/hacker_news_assets):

- [Config schema on a resource](https://github.com/dagster-io/dagster/blob/master/examples/hacker_news_assets/hacker_news_assets/resources/parquet_io_manager.py)
