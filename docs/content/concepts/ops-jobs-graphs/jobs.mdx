...
- Materializes a selection of [Software-defined Assets](/concepts/assets/software-defined-assets). To learn more about materializing and monitoring assets, visit [Asset jobs](/concepts/assets/asset-jobs).
...
## Creating jobs directly from ops

Jobs can be created in several ways. It can be built from software-defined assets or directly from ops.

<Note>
  If you are looking for ways to create a job from software-defined assets,
  visit <a href="/concepts/assets/asset-jobs">Asset jobs</a>.
</Note>

An op job can be created in the following ways:
...
The simplest way to create an op-based job is to use the <PyObject object="job" decorator /> decorator.
...
You can supply a <PyObject object="RunConfig"/> object or raw config dictionary. The supplied config will be used to configure the job whenever the job is launched. It will show up in the Dagster UI Launchpad and can be overridden.
...
For op jobs, you can supply a <PyObject object="PartitionedConfig" /> to create a partitioned job. This defines a discrete set of partitions along with a function for generating config for a partition. Job runs can be configured by selecting a partition.
