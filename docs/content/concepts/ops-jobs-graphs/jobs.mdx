...
- **Potential for improved efficency**: By applying concurrency limits to job runs, there may be benefits to your pipeline's efficiency. Refer to the [Limiting run concurrency guide](/guides/limiting-run-concurrency-in-data-pipelines) for more info and examples.
...
Jobs are supported for both Software-defined Assets and ops, but the implementation for each concept is unique. Refer to the following documentation for more info:
