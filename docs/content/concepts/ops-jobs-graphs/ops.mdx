...
```python file=/concepts/ops_jobs_graphs/ops.py startafter=start_generic_tuple endbefore=end_generic_tuple
...
Note that if you would like to specify a single tuple output and still utilize generic type annotations, this can be done by providing either a single <PyObject object="Out" /> to the op, or none.
