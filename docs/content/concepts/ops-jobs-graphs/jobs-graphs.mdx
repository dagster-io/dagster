...
You can run a job in a variety of ways - in the Python process where it's defined, via the command line, via a GraphQL API, or right in [Dagit](/concepts/dagit/dagit). Dagit centers on jobs, making it a one-stop-shop - you can manually kick off runs for a job and view all historical runs.
