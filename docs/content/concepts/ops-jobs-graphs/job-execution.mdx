...
Dagster provides several different ways to execute jobs. This guide explains different ways to do one-off execution of jobs: [Dagit](#dagit), [Dagster CLI](#dagster-cli), or [Python APIs](#python-apis).
...
To view your job in Dagit, you can use the [`dagster dev`](/\_apidocs/cli#dagster-dev) command:
...
Dagit Launchpad also offers a configuration editor to let you interactively build up the configuration. See details in [Dagit](/concepts/dagit/dagit#launchpad-tab).
