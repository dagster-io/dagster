...
A graph is a set of interconnected [ops](/concepts/ops-jobs-graphs/ops) or sub-graphs. While each op should perform a relatively simple task, a collection of ops can be assembled into a graph to accomplish complex tasks.
...
Graphs can also be used to back [software-defined assets](/concepts/assets/software-defined-assets). Graph-backed assets are useful if you have an existing graph that produces and consumes assets. 
...
- [That use dynamic outputs](#that-use-dynamic-outputs)
...
[Nesting graphs](/concepts/ops-jobs-graphs/nesting-graphs)
