...
- **Runtime, or materialization metadata** is information that changes after a materialization occurs. This could be how many records were processed, how long an asset took to materialize, etc.

### Definition metadata

Definition metadata is attached to the asset's definition using the `metadata` argument. In the following example, we attached a list of columns that the table produced by the asset should contain:
