---
title: Partitioned op jobs | Dagster
description: Partitioned op jobs enable launching backfills, where each partition processes a subset of data.
---

# Partitioned op jobs

<Note>
  This page is specific to <strong>op jobs</strong>. Looking for
  Software-defined Assets? Refer to the{" "}
  <a href="/concepts/partitions-schedules-sensors/partitioned-assets">
...
In this guide, we'll demonstrate how you can use partitions with ops and [jobs](/concepts/ops-jobs-graphs/jobs).
...
- [Config](/concepts/configuration/config-schema)
...
Not all jobs are partitioned by time. For example, here's a partitioned job where the partitions are continents:
