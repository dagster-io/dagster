...
You can launch and monitor backfills of a job using the [Partitions tab](/concepts/partitions-schedules-sensors/partitioned-ops#partitions-in-the-dagster-ui).
...
In the [Partitions](/concepts/partitions-schedules-sensors/partitioned-ops#date-partitioned-job) concept page, we defined a partitioned job called `do_stuff_partitioned` that had date partitions.
