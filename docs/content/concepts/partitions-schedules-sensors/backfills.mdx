...
You can open the backfill modal to launch backfills for a partitioned asset using the "Materialize" button on the [Asset detail page](https://docs.dagster.io/concepts/partitions-schedules-sensors/partitions#materializing-partitioned-assets).
...
To launch a backfill, click the "Materialize" button at the top right of the Partitions tab. This opens the "Launch backfill" modal, which lets you select the set of partitions to launch the backfill over. A run will be launched for each partition.
