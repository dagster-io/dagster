...
In the above code snippet, the body of the decorated function writes out data to a file, but you may also choose to delegate this writing operation to an [I/O manager](/concepts/io-management/io-managers). Dagster's [built-in I/O managers](/concepts/io-management/io-managers#built-in-io-managers) know how to handle partitioned assets. You can also handle them when writing your own I/O manager, following the instructions [here](/concepts/io-management/io-managers#handling-partitioned-assets).
...
def image_sensor(context: AssetExecutionContext):
    new_images = [
        img_filename
...
<Note>
  Currently, partition mappings are only supported for{" "}
