...
By default, these decorators create partitions as follows:

<PyObject object="hourly_partitioned_config" decorator /> - a partition is a single
hour spanning minute 0 to minute 0. For example, defining a partition:

```python file=/concepts/partitions_schedules_sensors/partition_defs.py startafter=start_hourly_partition_def endbefore=end_hourly_partition_def
...
will create partitions of the form `(2022-03-12T00:00, 2022-03-12T01:00), (2022-03-12T01:00, 2022-03-12T02:00), ...`

<PyObject object="daily_partitioned_config" decorator /> - a partition is single
day spanning midnight to midnight. For example, defining a partition:

```python file=/concepts/partitions_schedules_sensors/partition_defs.py startafter=start_daily_partition_def endbefore=end_daily_partition_def
...
will create partitions of the form `(2022-03-12T00:00, 2022-03-13T00:00), (2022-03-13T00:00, 2022-03-14T00:00), ...`

<PyObject object="weekly_partitioned_config" decorator /> - a partition is a week
spanning Sunday to Sunday. The first partition begins the first Sunday following
the given start date. For example, defining a partition:

```python file=/concepts/partitions_schedules_sensors/partition_defs.py startafter=start_weekly_partition_def endbefore=end_weekly_partition_def
...
will create partitions of the form `(2022-03-13T00:00, 2022-03-20T00:00), (2022-03-20T00:00, 2022-03-27T00:00), ...`

<PyObject object="monthly_partitioned_config" decorator /> - a partition is a month
spanning the first to the first. The first partition begins the on the soonest first
of the month following the given start date. For example, defining a partition:

```python file=/concepts/partitions_schedules_sensors/partition_defs.py startafter=start_monthly_partition_def endbefore=end_monthly_partition_def
...
will create partitions of the form `(2022-04-01T00:00, 2022-05-01T00:00), (2022-05-01T00:00, 2022-06-01T00:00), ...`

You can specify the partitions more granularly using `minute_offset`, `hour_offset`, and `day_offset` in the partitioned config decorator. For example, you can create a hourly partition that splits at the 15 minute mark by doing the following:

```python file=/concepts/partitions_schedules_sensors/partition_defs.py startafter=start_hourly_offset_partition_def endbefore=end_hourly_offset_partition_def
...
You can have a weekly partiton that splits on each Wednesday at 4:15pm by writing the following:

```python file=/concepts/partitions_schedules_sensors/partition_defs.py startafter=start_weekly_offset_partition_def endbefore=end_weekly_offset_partition_def
...
Or you can have your monthly partitions split on the 15th by writing:

```python file=/concepts/partitions_schedules_sensors/partition_defs.py startafter=start_monthly_offset_partition_def endbefore=end_monthly_offset_partition_def
...
If you want to test that your <PyObject object="PartitionedConfig" /> creates the partitions you expect, you can use the `get_partitions` or `get_partition_keys` functions.
...
def test_my_offset_partitioned_config():
    ...
    partition_key = my_offset_partitioned_config().get_partition_keys(datetime(2020, 2, 1))
    run_config = my_offset_partitioned_config().get_run_config(partition_key=partition_key[0])
    assert validate_run_config(do_stuff_partitioned, run_config)
```
