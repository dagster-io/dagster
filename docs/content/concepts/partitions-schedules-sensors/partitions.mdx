...
The <PyObject object="build_schedule_from_partitioned_job"/> function allows you to construct a schedule from a date partitioned job. If you wanted to create a schedule for `do_stuff_partitioned` job defined above, you could write:
...
Schedules can also be made from static partitioned jobs. If we wanted to make a schedule for the `continent_job` above that runs each partition, we could write:
