...
description: Partitions break data into smaller chunks, 
---
...
- Launch [backfills](/concepts/partitions-schedules-sensors/backfills), which are sets of runs that each process a different partition. For example, after making a code change, you might want to run your job on all partitions instead of just one.

---

## Implementation

Partitions are supported for both Software-defined Assets and op jobs, but the implementation for each concept is unique. Refer to the following documentation for more info:

- [Partitioned assets and jobs](/concepts/partitions-schedules-sensors/partitioned-assets)
- [Partitioned op jobs](/concepts/partitions-schedules-sensors/partitioned-op-jobs) (Advanced)
