...
As with other sensors, you can directly invoke run status sensors. If your run status sensor takes a context argument, you can provide it via <PyObject object="build_run_status_sensor_context" />. For example, if you had written the success status sensor above
```python file=/concepts/partitions_schedules_sensors/sensors/sensor_alert.py startafter=start_success_sensor_marker endbefore=end_success_sensor_marker
from dagster import run_status_sensor, RunStatusSensorContext, PipelineRunStatus
...
```
You could provide the `context` argument like this:
...
You can use the same function to build the context for `@run_failure_sensor`. Looking at the `my_slack_on_failure` example again
