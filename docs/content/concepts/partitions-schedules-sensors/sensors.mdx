...
If you had written the success status sensor above
...
```python file=concepts/partitions_schedules_sensors/sensors/sensors_alert.py startafter=start_run_status_sensor_testing_with_context_setup endbefore=end_run_status_sensor_testing_with_context_setup
ENOENT: no such file or directory, open '/Users/jamie/dev/dagster/examples/docs_snippets/docs_snippets/concepts/partitions_schedules_sensors/sensors/sensors_alert.py'
```
...
```python file=concepts/partitions_schedules_sensors/sensors/sensors_alert.py startafter=start_run_status_sensor_testing_with_context endbefore=end_run_status_sensor_testing_with_context
ENOENT: no such file or directory, open '/Users/jamie/dev/dagster/examples/docs_snippets/docs_snippets/concepts/partitions_schedules_sensors/sensors/sensors_alert.py'
```

You may notice that we trigger our run status sensor on `PipelineRunStatus.SUCCESS` but we filter the events from the job execution on `DagsterEventType.PIPELINE_SUCCESS`. There is an internal mapping between `PipelineRunStatus` and `DagsterEventType`
...
We can use the same pattern to build the context for <PyObject object="run_failure_sensor" />. Looking at the `my_slack_on_failure` example again
...
```python file=concepts/partitions_schedules_sensors/sensors/sensors_alert.py startafter=start_alert_sensor_testing_with_context_setup endbefore=end_run_status_sensor_testing_with_context_setup
ENOENT: no such file or directory, open '/Users/jamie/dev/dagster/examples/docs_snippets/docs_snippets/concepts/partitions_schedules_sensors/sensors/sensors_alert.py'
```

Then we can execute the job and create our context

```python file=concepts/partitions_schedules_sensors/sensors/sensors_alert.py startafter=start_alert_sensor_testing_with_context endbefore=end_alert_sensor_testing_with_context
ENOENT: no such file or directory, open '/Users/jamie/dev/dagster/examples/docs_snippets/docs_snippets/concepts/partitions_schedules_sensors/sensors/sensors_alert.py'
```

Note the additional function call `for_run_failure()` after creating the `context`. The `context` provided by <PyObject object="run_failure_sensor" /> is a subclass of the context provided by <PyObject object="run_status_sensor" /> and can be built using this additional call.
