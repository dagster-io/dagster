...
Multi-asset sensors, which can trigger job executions based on  multiple asset materialization event streams, can be handled using the <PyObject object="multi_asset_sesnor" decorator/> decorator.

In the body of the sensor, you have access to the event log information for each asset. If a monitored asset has not been materialized, the event log information will be `None`.


```python file=/concepts/partitions_schedules_sensors/sensors/sensors.py startafter=start_custom_asset_sensor_marker endbefore=end_multi_asset_sensor_marker
@asset_status_sensor(
...
You can use the provided `asset_events` to only yield `RunRequests` when a certain condition is met:

```python file=/concepts/partitions_schedules_sensors/sensors/sensors.py startafter=start_multi_asset_sensor_custom_fn_marker endbefore=end_multi_asset_sensor_custom_fn_marker
@asset_status_sensor(
...
```python file=/concepts/partitions_schedules_sensors/sensors/sensors.py startafter=start_custom_multi_asset_sensor_marker endbefore=end_custom_multi_asset_sensor_marker
import json
