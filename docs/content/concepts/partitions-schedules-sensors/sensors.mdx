...
On the overview page for a particular sensor, there is a `Test sensor` button. Clicking this button will perform a test evaluation of your sensor, and show you the results of that evaluation.
Screenshot pointing to button here.
When the button is clicked, you are prompted to provide a cursor value (to learn more about cursors, check out this part of the guide). If not using cursors, you can just leave this field blank. The cursor input is pre-filled with the existing cursor for the sensor.
Screenshot of cursor input page here.
When the `Evaluate` button is clicked, the sensor will fire, and the following window will show you the result of that evaluation, whether that be run requests, a skip reason, or a python error.
Screenshot of successful evaluation screen here.
If the run was successful, then for each produced run request, you have the option of opening the launchpad pre-scaffolded with the config produced by that run request. You will also see a new computed cursor value from the evaluation, and you have the option to persist that value.
Note that a test evaluation of the sensor will still run the sensor's underlying python function, meaning that any side effects contained within that sensor's function may be executed.



### Via the CLI

To quickly preview what an existing sensor will generate when evaluated, you can run the following CLI command:
...
To unit test sensors, you can invoke the sensor's python function directly. This will return all the run requests yielded by the sensor. The config obtained from these can be validated using the <PyObject object="validate_run_config" /> function:
...
To view all sensors, navigate to **Overview > Sensors tab**. Here, you can start and stop sensors, and view their frequency, last tick, and last run:
...
Click on any sensor to monitor all sensor evaluations on a timeline and view a table of runs launched by the sensor.
