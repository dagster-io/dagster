...
def asset_c_or_d_sensor(context, asset_events):
    ...
    if asset_events["asset_c"] is not None and asset_events["asset_d"] is None:
        ...
        return SkipReason(f"asset_c {'not' if not asset_events['asset_c'] else ''} materialized and asset_d {'not' if not asset_events['asset_d'] else ''} materialized.")
```
...
If the built in `multi_asset_sensor` does not meet your needs, you can work with the asset event streams directly to write a sensor that meets your needs. These asset event streams can be queried using the `instance` attribute off of the <PyObject object="SensorEvaluationContext" /> object.
