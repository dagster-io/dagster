...
In the body of the sensor, you have access to the materialization event records for each asset via the `context`. The context provides methods for fetching the materialization event records, as well as methods to handle partitioned materializations:

| Method                                                                                                      | Description                                                                                                                                                                                                          |
| ----------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <PyObject object="MultiAssetSensorEvaluationContext" method="latest_materialization_records_by_key" />      | Returns a dictionary mapping the `AssetKey` for each monitored asset to the most recent materialization record. If there is no materialization event, the mapped value will be `None`.                               |
| <PyObject object="MultiAssetSensorEvaluationContext" method="materialization_records_for_key"/>             | Returns a list of materialization event records for a specified `AssetKey`.                                                                                                                                          |
| <PyObject object="MultiAssetSensorEvaluationContext" method="latest_materialization_records_by_partition"/> | Returns a dictionary mapping the partition keys of a specified `AssetKey` to the most recent materialization record. Will only return a partition key in the mapping if a materialization exists for that partition. |
| <PyObject object="MultiAssetSensorEvaluationContext" method="all_partitions_materialized"/>                 | Given an asset key and a list of partition keys for the asset, returns `True` if all partitions are materialized, otherwise returns `False`. Will search through all materializations (ignoring the cursor).         |
| <PyObject object="MultiAssetSensorEvaluationContext" method="get_downstream_partition_keys"/>               | Given an upstream asset key, downstream asset key, and a partition in the upstream asset, returns a list of the corresponding partitions in the downstream asset.                                                    |
...
#### Triggering a partitioned run after an upstream partition is materialized
...
```python file=/concepts/partitions_schedules_sensors/sensors/sensors.py startafter=start_daily_asset_to_daily_asset endbefore=end_daily_asset_to_daily_asset
...
The <PyObject object="MultiAssetSensorEvaluationContext" method="get_downstream_partition_keys"/> accepts a partition key from the upstream asset and uses the existing <PyObject object="PartitionMapping"/> object on the downstream asset to fetch the corresponding partitions in the downstream asset.
...
The examples section at the bottom of the page explores additional use cases for triggering materializations by partition:

- [Updating a weekly asset partition when upstream daily partitions are materialized](#updating-a-weekly-asset-partition-when-upstream-daily-partitions-are-materialized)
- [Materializing a daily asset partition if both upstream daily partitions are materialized](#materializing-a-daily-asset-partition-if-both-upstream-daily-partitions-are-materialized)
