...
We have provided convenience functions for retrieving `DagsterRunStatus.SUCCESS` and `DagsterRunStatus.FAILURE` events. If you have a run status sensor triggered on another status, you can retrieve all events from `result` and filter based on your event type.
