...
You can also monitor every job in your Dagster instance by specifying `monitor_instance=True` on the sensor decorator. Note that `monitor_instance` cannot be used along with jobs specified via `monitored_jobs`.
