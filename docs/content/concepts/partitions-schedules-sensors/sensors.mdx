...
Multi-asset sensors, which can trigger job executions based on some combination of states from multiple asset materialization event streams, can be handled using the <PyObject object="asset_status_sesnor" decorator/> decorator.

When writing an `asset_status_sensor` you can specify which event to monitor and provide a custom function to determine whether the sensor should run.

In the default case, the asset status sensor will run whenever _all_ of the listed assets have the specified status.

```python file=/concepts/partitions_schedules_sensors/sensors/sensors.py startafter=start_asset_status_sensor_marker end_before=end_asset_status_sensor_marker
@asset_status_sensor(
...
def asset_a_and_b_sensor(context, asset_events):
    ...
    return RunRequest(run_key=context.cursor, run_config={"ops": {"logger_op": {"config": {"logger_str": logger_str}}}})
...
You can provide a custom function to run the sensor when a different condition is met:

```python file=/concepts/partitions_schedules_sensors/sensors/sensors.py startafter=start_asset_status_sensor_or_marker end_before=end_asset_status_sensor__or_marker
@asset_status_sensor(
    asset_keys=[AssetKey("asset_c"), AssetKey("asset_d")],
    asset_status=DagsterEventType.ASSET_MATERIALIZATION,
    trigger_fn=lambda x: any(x.values()),
...
### Asset status sensor with more customization

If the built in `asset_status_sensor` does not meet your needs, you can work with the asset event streams directly to write a sensor that meets your needs. These asset event streams can be queried using the `instance` attribute off of the <PyObject object="SensorEvaluationContext" /> object.
