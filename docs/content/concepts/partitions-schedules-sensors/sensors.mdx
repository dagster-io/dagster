...
By default, this sensor runs every 30 seconds. It checks the last modified time for "inputfile.csv" and returns <PyObject object="RunRequest"/> based on it. Only one run will be launched for each unique value passed to the `run_key` parameter, which means that only one run will be launched each time the file is modified.
