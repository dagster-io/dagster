...
To unit test sensors, you can invoke the sensor directly. This will return all the run requests yielded by the sensor. The config obtained from these can be validated using the <PyObject object="validate_run_config" /> function:
...
Using Dagit, you can monitor and operator sensors. Dagit provides multiple view that help with observing sensor evaluations, skip reasons, and errors.
