...
The asset reconciliation sensor automatically materializes any "unreconciled" partitions, following the same conditions for reconciliation as un-partitioned assets. When upstream partitions have not been materialized for a given partition, the asset reconciliation sensor will not trigger a materialization.
