...
def my_asset_sensor(context: FreshnessPolicySensorEvaluationContext):
    ...
    if context.current_minutes_late >= 10 and context.previous_minutes_late < 10:
        send_alert(
            f"Asset with key {context.asset_key} is now more than 10 minutes out of date."
        )
...
Setting a `FreshnessPolicy` on an asset tells dagster how up-to-date the data in that asset is expected to be. The asset reconciliation sensor can kick off runs to keep that data in line with the defined policy.
...
- The most recent materialization for that asset, to see what upstream data is incorporated into the current state of the asset.
- Any in-progress runs that will update that asset in the near future.
- The current and future constraints specifying what upstream data must be incorporated into the state of the asset by certain times.

The asset reconciliation sensor will attempt to optimize over all constraints specified by FreshnessPolicies on assets within its selection -- if two separate assets with FreshnessPolicies require data from the same upstream asset, a single run of the upstream asset will be kicked off to satisfy both constraints (if possible).
