...
- In Dagit using the **Schedules** tab:

  <Image
  alt="Schedules tab in Dagit"
...
If you manually start or stop a schedule in Dagit, that overrides any default status set in code.
...
<Image
src="/images/concepts/partitions-schedules-sensors/test-schedule-button.png"
width={320}
height={128}
/>

When the button is clicked, you are provided to select among a dropdown of mock scheduled execution times. Schedules are defined on a cadence, and the times provided in this dropdown are in the past and future along that cadence. The schedule being tested here runs at a cadence of every day at x time, and so I see 5 evaluation times in the future along that cadence, and 5 evaluation times in the past along that cadence.

<Image
src="/images/concepts/partitions-schedules-sensors/select-timestamp-page.png"
width={896}
height={449}
/>

Upon selecting an evaluation time and clicking the `Evaluate` button, the schedule will evaluate, and the following window will show the result of that evaluation. If the evaluation was successful and a run request was produced, you have the option to open the launchpad pre-scaffolded with the config corresponding to that run request.

<Image
src="/images/concepts/partitions-schedules-sensors/eval-schedule-result-page.png"
width={896}
height={449}
/>
...
First, verify that the schedule has been included in a <PyObject object="Definitions" /> object. This ensures that the schedule is detectable and loadable by Dagster tools like Dagit and the CLI:
...
Next, using Dagit, verify the schedule has been started:
...
1. Open the left sidenav and locate the job attached to the schedule. Schedules that have been started will have a **green clock icon** next to them:
   ...
   <Image
   alt="Started schedule icon next to schedule in left sidenav in Dagit"
   src="/images/concepts/partitions-schedules-sensors/schedule-started-icon-dagit.png"
   width={1031}
   height={391}
   />
   ...
   <Image
   alt="Start/stop toggle for schedules in Code Locations tab in Dagit"
   src="/images/concepts/partitions-schedules-sensors/schedule-page-toggle.png"
   width={1043}
   height={302}
   />

2. Next, verify that Dagit has loaded the latest version of your schedule code:

   1. Click **Deployment** in the top navigation.

   2. In the **Code locations** tab, click **Reload** (local Dagit) or **Redeploy** (Dagster Cloud).

   If Dagit is unable to load the code location - for example, due to a syntax error - an error message with additional info will display in the left sidenav.
...
Clicking the schedule in the left sidenav in Dagit opens the **Schedule details** page for the schedule.
...
<Image
alt="Next tick attribute highlighted on Schedule details page in Dagit"
...
1. First, check that the daemon is running:
   ...
   1. In Dagit, click **Deployment** in the top navigation.
...
2. Next, check the logs from the daemon process. The steps to do this will depend on your deployment - for example, if you're using Kubernetes, you'll need to get the logs from the pod that's running the daemon. You should be able to search those logs for the name of your schedule (or `SchedulerDaemon` to see all logs associated with the scheduler) to gain an understanding of what's going wrong.
   ...
   If the daemon output contains error indicating the schedule couldn't be found, verify that the daemon is using the same `workspace.yaml` file as Dagit. The daemon does not need to restart in order to pick up changes to the `workspace.yaml` file. Refer to the [Workspace files documentation](/concepts/code-locations/workspace-files) for more info.

   If the logs don't indicate the cause of the issue, move on to step three.

3. Lastly, double-check your schedule code:

   1. In Dagit, open the schedule's **Schedule details** page by clicking the schedule in the left sidenav.
