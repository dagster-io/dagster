...
We can write a schedule that will run every partition on each tick:
...
Or a schedule that will run a subselection of the partitions:
...
2. Next, verify that the UI has loaded the latest version of your schedule code:
   ...
   2. In the **Code locations** tab, click **Reload** (local webserver) or **Redeploy** (Dagster+).
