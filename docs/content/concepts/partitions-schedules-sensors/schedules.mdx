...
- **A**: Run only the data that’s needed and gain granular control over slices. For example, storing recent orders in hot storage and older orders in cheaper, cold storage.
- **B**: Divide large datasets into smaller, more manageable parts to speed up queries.
