...
Partition-based schedules are best when your pipeline operates on data that is partitioned by time - for example, a pipeline that fills in a daily partition of a table on each run. Non-partition-based schedules are best when you just want to run an operation on a fixed interval - for example, a pipeline that rebuilds the same table on each run.

### Partition-based schedules

Partition-based schedules are schedules created for partitioned jobs. A partitioned job is a job that can operate on a discrete set of config values - usually a sequence of datetimes.

For a discussion of how to define partition-based schedules, refer to the [partitions concept page](/concepts/partitions-schedules-sensors/partitions).

The scheduler will only kick off runs for times after both the partitioned job's `start_date` and the time when the schedule was turned on. You can kick off runs for times between the `start_date` and when the schedule is turned on by launching a [backfill](/concepts/partitions-schedules-sensors/backfills).
