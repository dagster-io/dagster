...
A schedule is a Dagster definition that is used to execute a [job](/concepts/ops-jobs-graphs/jobs) at a fixed interval, such as daily, hourly, or Monday at 9:00 AM. Each time at which a schedule is evaluated is called a tick. The schedule definition can generate run configuration for the job on each tick.
...
<!-- # WIP (old "Schedules" page content)
