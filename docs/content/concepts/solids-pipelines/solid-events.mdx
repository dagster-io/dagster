...
Yielding events from within the body of a solid is a useful way of communicating with the Dagster framework. The most critical event to the functionality of Dagster is the <PyObject object="Output"/> event, which allows output data to be passed on from one solid to the next.

### Event Metadata

Oftentimes, it may be useful to attach a set of structured metadata to an event.
