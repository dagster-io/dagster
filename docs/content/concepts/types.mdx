...
Dagster offers a special type called <PyObject object="Nothing" />, which is used when you need to model a dependency between ops where Dagster is passing no data along the edge. See details in the [Nothing dependencies](/concepts/ops-jobs-graphs/graphs#nothing-dependencies) example.
