...
You can use **resources** to access features of the execution environment to solids during pipeline execution. You can use **modes** to bind a set of resources (and other environment information) to a pipeline so that those resources can be available to the solids within a pipeline. You can provide multiple modes to a pipeline, each with different resources, to represent the execution environment that your pipeline will be run within.
