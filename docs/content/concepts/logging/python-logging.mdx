...
Dagster is compatible and configurable with [Python's logging module](https://docs.python.org/3/library/logging.html). At the moment, you can access the configuration options through your `dagster.yaml` file, which will apply the contained settings to any run launched from your instance.
You can specify and configure handlers and formatters to create custom log messages.

## Capturing Python Logs <Experimental />

By default, logs generated using the python logging module are not captured into the Dagster ecosystem. This means that they are not stored in the [TODO] Dagster Event Log,
and will not be associated with any Dagster metadata (such as step key, run id, etc.).

If a `python_log_level` is set (see: Configuring A Python Log Level), then these loggers will be set to
that level before a run is launched.
...
`python_log_level` in your dagster.yaml file. This will set the log level of all loggers managed by Dagster. By default,
this will just be the `context.log` logger. If there are custom python loggers that you wish to capture, see
[TODO] Capturing Python Logs.
...
setting the `python_log_level` to `INFO` will
