...
As part of its rich, extensible logging system, Dagster includes loggers. These are job-scoped logging handlers, which will be automatically invoked whenever [ops](/concepts/ops-jobs-graphs/ops) in a [job](/concepts/ops-jobs-graphs/jobs) log messages. This means that this out-of-the-box logger will track all execution events. Loggers can also be customized to meet your specific needs.
...
When you run Dagster jobs in Dagit, you'll notice that log messages are visible as colored messages in the console. For example:

- **Logs stream back to the Dagit frontend in real time**:

  <!-- ![Real time logs in Dagit](/images/concepts/logging/job-log-dagit.png) -->

  <Image
  alt="Real time logs in Dagit"
...
- **Filtering log messages based on execution steps and log levels**:
  ...
  <!-- ![Log filtering in Dagit](/images/concepts/logging/job-dagit-filter.png) -->

  <Image
  alt="Log filtering in Dagit"
...
Messages at level `ERROR` or above are highlighted both in Dagit and in the console logs, so they can be easily identified even without filtering:

<!-- ![ERROR level in logs in Dagit](/images/concepts/logging/job-dagit-error.png) -->

<Image
alt="ERROR level in logs in Dagit"
