...
When jobs are run, the logs stream back to the [**Run details** page](/concepts/dagster-ui#run-details) in real time. The UI contains two types of logs, which you can learn about using the tabs below.
