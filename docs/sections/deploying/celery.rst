...
- By running the Celery worker in the same directory to which we saved ``celery_pipeline.py``, so
  that our Dagster code would be available both to Dagit and to the worker: i.e., the worker and
  Dagit can both find the pipeline definition in the same file (``-f celery_pipeline.py``), under
  the same name (``-n parallel_pipeline``).
