...
### A pipeline with a config and a schedule

With the pipeline APIs, if you want to schedule runs with a pre-defined config, you need to do something like this:

```python file=/guides/dagster/graph_job_op/pipeline_with_schedule_return_config.py
from dagster import schedule, pipeline, solid
