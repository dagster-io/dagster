...
You'll probably also want to configure a GCS bucket to store intermediates via persistent [IO Managers](/concepts/io-management/io-managers). This enables reexecution, review and audit of op intermediate results, and cross-node cooperation (e.g., with the multiprocessing or Dagster celery executors).
...
Then, add the following YAML block in your job config:
...
With this in place, your job runs will store intermediates on GCS in the location `gs://<bucket>/dagster/storage/<job run id>/intermediates/<op name>.compute`.
