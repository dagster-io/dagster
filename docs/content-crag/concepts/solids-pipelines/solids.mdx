...
Ops are the functional unit of work in Dagster. An op's responsibility is to read its inputs, perform an action, and emit outputs. Multiple ops can be connected to create a [Job](/concepts/solids-pipelines/pipelines).
...
Ops are used to define computations. Ops can later be assembled into [Jobs](/concepts/solids-pipelines/pipelines). Ops generally perform one specific action and are used for batch computations. For example, you can use an op to:
...
Ops are used within a [Job or Graph](/concepts/solids-pipelines/pipelines). You can also execute a single op, usually within a test context, by directly invoking it. More information can be found at [Testing ops](/concepts/testing#testing-ops).
