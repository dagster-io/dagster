...
The `to_job` function accepts hooks as a parameter. You can pass the "slack_message_on_failure" hook above in a set as a parameter to `to_job`. Then, slack messages will be sent when any op in the job fails.
...
However, sometimes a job is a shared responsibility or you only want to be alerted on high-priority op executions. So we also provide a way to set up hooks on op instances which enables you to apply policies on a per-op basis.
...
ENOENT: no such file or directory, open '/Users/claire/dagster/examples/docs_snippets_crag/docs_snippets_crag/concepts/solids_pipelines/prod_solid_hooks.yaml'
```
