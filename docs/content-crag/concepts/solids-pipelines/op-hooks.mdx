...
### Applying a hook on every op in a graph

For example, you want to send a slack message to a channel when any op fails in a graph. In this case, we will be applying a hook on a graph, which will apply the hook on every op instance within in that graph.

You can simply use the hook created "slack_message_on_failure" above to decorate the graph "notif_all". Then, slack messages will be sent when any op in the graph fails.
...
However, sometimes a graph is a shared responsibility or you only want to be alerted on high-priority op executions. So we also provide a way to set up hooks on op instances which enables you to apply policies on a per-op basis.
...
In this case, we can mock the `slack_resource` using a helper function <PyObject module="dagster" object="ResourceDefinition" displayText="ResourceDefinition.hardcoded_resource()"/>, so it won't send slack messages when you are developing the pipeline.
