...
- **Root Input Managers (experimental)** - If the input to a job comes from an external source, such as a table in a database, often it makes sense to define a resource that's responsible for loading it. This makes it easy to swap out implementations in different modes and mock it in tests. <PyObject module="dagster" object="RootInputManager" />s, which can be referenced from <PyObject module="dagster" object="In" />s, are resources that load unconnected inputs.
- **Dagster Type Loaders** - <PyObject module="dagster" object="DagsterTypeLoader" />s provide a way to specify how to load inputs that depends on the type, but not on the mode. <PyObject module="dagster" object="DagsterTypeLoader" />s can be placed on <PyObject module="dagster" object="DagsterType" />s, which can be placed on <PyObject module="dagster" object="In" />s.
