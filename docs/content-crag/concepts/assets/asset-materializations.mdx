...
If you specified any metadata entries on the <PyObject object="Output" /> event while yielding it (see: [Op Event Docs](/concepts/ops-jobs/op-events#attaching-metadata-to-outputs)), these entries will automatically be attached to the materialization event for this asset.
