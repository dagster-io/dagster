...
Dagster provides a set of special run status sensor decorators for defining sensors that monitor job failure events. You can use <PyObject object="job_failure_sensor" /> to decorate a function that will run when a run fails.
...
<PyObject module="dagster_slack" object="make_slack_on_job_failure_sensor" /> helps you create a job
failure sensor that will message a given Slack channel:
...
<PyObject module="dagster.utils" object="make_email_on_job_failure_sensor" /> helps you create a job
failure sensor that will send emails via the SMTP protocol:
