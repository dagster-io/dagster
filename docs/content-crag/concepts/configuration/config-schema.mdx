...
config_example_job = config_example.to_job(
        config={"graph": {"uses_config": {"config": {"iterations": 1}}}}
    )
```
...
result = config_example.to_job().execute_in_process(
        run_config={"graph": {"uses_config": {"config": {"iterations": 1}}}}
    )
```
...
result = nests_configurable.to_job().execute_in_process(
        run_config={
            "graph": {"configurable_with_schema": {"config": {"nonexistent_config_value": 1}}}
        }
    )
```
