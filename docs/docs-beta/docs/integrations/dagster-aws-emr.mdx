...
status: 'published'
name: 'AWS EMR'
title: 'Using AWS EMR with Dagster'
...
The `dagster-aws` integration provides a robust framework for orchestrating data pipelines that leverage AWS services, including AWS EMR (Elastic MapReduce). This integration allows you to run and scale big data workloads using open-source tools such as Apache Spark, Hive, Presto, and more.
