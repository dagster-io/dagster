...
- A class-based interface (`Component`) for dynamically constructing Dagster definitions.
...
- An interface for `Components` to specify their own scaffolding logic, supporting the organization of integration-specific artifact files.

## Project Setup

First let's install some tools. The centerpiece of this tutorial is the `dg` command line tool, which lives in the published Python package `dagster-dg`. `dg` is designed to be globally installed and has no dependency on `dagster` itself. We will use Python package manager `uv` to install a globally available `dg`.

We install `uv` from Homebrew. We'll go ahead and install two other tools used in this tutorial from Homebrew as well: `duckdb` is used as a local database; `tree` is used to visualize project structure:
