...
The :py`~dagster.core.instance.DagsterInstance`{.interpreted-text
role="class"} defines everything Dagster needs for a single deployment:
...
APIs such as :py`execute_pipeline`{.interpreted-text role="func"}
directly.
...
:py`~dagster.core.serdes.ConfigurableClass`{.interpreted-text
role="class"}. This lets you specify what class to instantiate for each
...
-   default:
    :py`~dagster.core.storage.root.LocalArtifactStorage`{.interpreted-text
    role="class"}
...
-   key: `run_storage`
-   base class:
    :py`~dagster.core.storage.runs.RunStorage`{.interpreted-text
    role="class"}
-   default:
    :py`~dagster.core.storage.runs.SqliteRunStorage`{.interpreted-text
    role="class"}
-   example alternative:
    :py`~dagster_postgres.run_storage.PostgresRunStorage`{.interpreted-text
    role="class"}

Controls how the history of runs is persisted. Manages a collection of
:py`PipelineRun`{.interpreted-text role="class"}

### Event log storage

-   key `event_log_storage`
-   base class:
    :py`~dagster.core.storage.event_log.EventLogStorage`{.interpreted-text
    role="class"}
-   default:
    :py`~dagster.core.storage.event_log.SqliteEventLogStorage`{.interpreted-text
    role="class"}
-   example alternative:
    :py`~dagster_postgres.event_log.PostgresEventLogStorage`{.interpreted-text
    role="class"}

Controls how the structured event logs produced by each run are
persisted. Collections of
:py`~dagster.core.events.log.EventRecord`{.interpreted-text
role="class"} which contain :py`DagsterEvent`{.interpreted-text
role="class"}
...
-   base class:
    :py`~dagster.core.storage.compute_log_manager.ComputeLogManager`{.interpreted-text
    role="class"}
-   default:
    :py`~dagster.core.storage.local_compute_log_manager.LocalComputeLogManager`{.interpreted-text
    role="class"}
-   example alternative:
    :py`~dagster_aws.s3.compute_log_manager.S3ComputeLogManager`{.interpreted-text
    role="class"}
...
-   key: `scheduler`
-   base class: :py`~dagster.core.scheduler.Scheduler`{.interpreted-text
    role="class"}
-   default: `None`
-   example alternative:
    :py`~dagster_cron.cron_scheduler.SystemCronScheduler`{.interpreted-text
    role="class"}
...
-   key: `scheduler`
-   base class:
    :py`~dagster.core.storage.schedules.ScheduleStorage`{.interpreted-text
    role="class"}
-   default:
    :py`~dagster.core.storage.schedules.SqliteScheduleStorage`{.interpreted-text
    role="class"}
-   example alternative:
    :py`~dagster_postgres.schedule_storage.PostgresScheduleStorage`{.interpreted-text
    role="class"}
...
-   key: `run_launcher`
-   base class:
    :py`~dagster.core.launcher.RunLauncher`{.interpreted-text
    role="class"}
-   default: `None`
-   example alternative:
    :py`~dagster_graphql.launcher.RemoteDagitRunLauncher`{.interpreted-text
    role="class"}
...
::: {.literalinclude caption="dagster.yaml" language="YAML"}
dagster.yaml
:::
...
:py`python:multiprocessing.cpu_count`{.interpreted-text role="func"}.
