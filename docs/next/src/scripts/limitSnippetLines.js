 ...
 */
const limitSnippetLines = (content, fromTo, dedent, startAfter, endBefore) => {
  ...
  if (startAfter) {
    ...
    if (startIndex == -1) throw new Error(`No match for startAfter value "${startAfter}"`);
    elements = elements.slice(startIndex + 1);
  }
  if (endBefore) {
    let endIndex = elements.findIndex(l => l.includes(endBefore));
    if (endIndex == -1) throw new Error(`No match for endBefore value "${endBefore}"`);
  ...
  if (fromTo) {
    let desiredLineNumbers = parseLineNumbersToSet(fromTo, dedentedElements.length);
    result = result
      .filter((_, i) => desiredLineNumbers.has(i));
  }
  ...
  try {
    for (part of parts) {
      ...
      if (begEnd.length == 1) {
        ...
        let end = safeParseInt(begEnd[1] !== '' ? begEnd[1] : Math.max(start, total));
        if (start > end) {
