...
installations. (We recommend the built-in [scheduler and partition system](/docs/tutorial/advanced#scheduling-pipeline-runs) for
scheduling pipelines, and the [Celery-based executor](/docs/deploying/celery) for
large-scale deployments.) But if you have a large existing Airflow install,
this integration will allow you to follow an incremental adoption path.
...
file:/dagster_examples/intro_tutorial/airflow.py
```
...
file:/dagster_examples/intro_tutorial/hello_cereal_pipeline.py
```
