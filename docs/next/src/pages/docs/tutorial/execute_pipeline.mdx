...
<AnchorHeading tag="h1">Executing our first pipeline</AnchorHeading>

Assuming you've saved this pipeline as `hello_cereal.py`, we can
execute it via any of three different mechanisms:
...
<AnchorHeading tag="h2">Using dagit to execute a pipeline</AnchorHeading>
...
structure of the pipeline -- which also only has one node,{' '}
`hello_cereal.compute`.
...
<AnchorHeading tag="h2">Using the Python API to execute a pipeline</AnchorHeading>
