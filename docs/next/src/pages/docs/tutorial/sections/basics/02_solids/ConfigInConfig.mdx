...
```python literalinclude startLine=6 emphasize-lines=8-14
file:/dagster_examples/intro_tutorial/config_bad_1.py
...
```python literalinclude startLine=6 emphasize-lines=5-11
file:/dagster_examples/intro_tutorial/config_bad_2.py
...
```python literalinclude emphasize-lines=15,33-41,87
file:/dagster_examples/intro_tutorial/config.py
caption:config.py
lines:1-102
```
...
the `inputs` section, which we'll still use to set the{' '}
`csv_path` input, we now have a `config` section, where
we can set values defined in the `config` argument to <PyObject module="dagster" object="solid" displayText="@solid" />.

```YAML literalinclude
file:/dagster_examples/intro_tutorial/config_env_bad.yaml
