...
We publish a [Dagster Helm chart](https://github.com/dagster-io/dagster/tree/master/helm)
...
user code image specified in the `pipelineRun:` values YAML configuration (the run coordinator
...
using the `pipelineRun:` image, and each will be named `dagster-job-<hash>`.

### Celery-less K8s

The published helm chart also supports a celery-less deployment. In this case, we configure the <PyObject
