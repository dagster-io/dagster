...
In the [Default Deployment](/deploying/k8s#default-deployment) section, we go over the simplest way to get started:
* [Default Quickstart: Running Dagster on K8s! →](/deploying/k8s#default-quickstart)
* [Default Architecture and Walkthrough →](/deploying/k8s#default-architecture)
* [Default Components →](/deploying/k8s#default-components)

In the [Advanced Deployment](/deploying/k8s#advanced-deployment) section, we build off of the [Default Deployment](/deploying/k8s#default-deployment) 
and add more sophisticated features including solid-level isolation and queuing mechanisms.
* [Advanced Quickstart: Running Dagster in Production →](/deploying/k8s#advanced-quickstart)
* [Advanced Architecture and Walkthrough →](/deploying/k8s#advanced-architecture)
* [Advanced Components →](/deploying/k8s#advanced-components)
* [Helm Configuration Options →](/deploying/k8s#advanced-configuration)

Last, we include sections on:
* [Tips →](/deploying/k8s#tools-and-tips)
* [Debugging →](/deploying/k8s#debugging)


## Default Deployment

In this section, we cover the simplest deployment option from [quickstart](/deploying/k8s#default-quickstart), 
to [architecture](/deploying/k8s#default-architecture), to individual [components](/deploying/k8s#default-components).

### Default Quickstart

The following steps are how to get up and running with the [default architecture](/deploying/k8s#default-architecture). This example requires a Kubernetes cluster.

**Step 1:** Set up nvironment
...
--set runLauncher.type=K8sRunLauncher \
...
```shell
helm upgrade dagster dagster/dagster -f helm/dagster/values.yaml \
--set runLauncher.type=K8sRunLauncher \
--set userDeployments.enabled=true \
--set celery.enabled=false \
--set rabbitmq.enabled=false
...
3.  The Daemon checks (every <100ms) for all runs that are ready to be executed, finds the Pipeline Run from (2), and spins up a Run Worker 
(an ephemeral K8s Job) with image `company_repository/baz:6`.
...
<table>
  ...
  <tr>
    ...
    <td><a href="https://hub.docker.com/r/dagster/k8s-dagit">dagster/k8s-dagit</a> <i>(released weekly)</i></td>
  </tr>
  <tr style={{background:"#F8F8F8"}}>
    <td>Dagit</td>
    <td><a href="https://kubernetes.io/docs/concepts/workloads/controllers/deployment/">Deployment</a> behind a <a href="https://kubernetes.io/docs/concepts/services-networking/service/">Service</a></td>
    <td><a href="https://hub.docker.com/r/dagster/k8s-dagit">dagster/k8s-dagit</a> <i>(released weekly)</i></td>
  </tr>
  <tr>
    <td>Database</td>
    <td>PostgreSQL</td>
    <td> <a href="https://hub.docker.com/_/postgres">postgres</a> <i>(Optional)</i> </td>
  </tr>
  <tr style={{background:"#F8F8F8"}}>
    <td>Run Worker</td>
    <td><a href="https://kubernetes.io/docs/concepts/workloads/controllers/job/">Job</a></td>
    <td>User-provided or <a href="https://hub.docker.com/r/dagster/k8s-example">dagster/k8s-example</a> <i>(released weekly)</i> </td>
  </tr>
    <tr>
...
## Advanced Deployment
In this section, we build off of the [Default Deployment](/deploying/k8s#default-deployment) and add more sophisticated features.

### Advanced Quickstart

The following steps set up the [advanced architecture](/deploying/k8s#advanced-architecture). This example requires a Kubernetes cluster and a persistent file store (we use S3 
in the example). Read about [GCP documentation here](/deploying/gcp).

**Step 1-4:** [Follow Step 1-4 in the Default Quickstart](/deploying/k8s#default-quickstart)
...
```shell
helm upgrade dagster dagster/dagster -f /path/to/values.yaml \
--set runLauncher.type=CeleryK8sRunLauncher \
--set dagsterDaemon.queuedRunCoordinator.enabled=true \
--set dagsterDaemon.enabled=true \
--set userDeployments.enabled=true \
...
<table>
  ...
  <tr>
    <td>Celery</td>
    <td><a href="https://kubernetes.io/docs/concepts/workloads/controllers/deployment/">Deployment</a></td>
    <td><a href="https://hub.docker.com/r/dagster/k8s-celery-worker">dagster/k8s-celery-worker</a> <i>(released weekly)</i></td>
  </tr>
  <tr style={{background:"#F8F8F8"}}>
    <td>Daemon</td>
    <td><a href="https://kubernetes.io/docs/concepts/workloads/controllers/deployment/">Deployment</a></td>
  ...
  <tr >
    <td>Flower <i>(Optional)</i></td>
    <td><a href="https://kubernetes.io/docs/concepts/workloads/controllers/deployment/">Deployment</a> behind a <a href="https://kubernetes.io/docs/concepts/services-networking/service/">Service</a></td>
    <td><a href="https://hub.docker.com/r/mher/flower">mher/flower</a></td>
  </tr>
  <tr style={{background:"#F8F8F8"}}>
    <td>Run Worker</td>
    <td><a href="https://kubernetes.io/docs/concepts/workloads/controllers/job/">Job</a></td>
...
for each resource the user would like to limit) and multiple Celery workers per queue via the `celery` section of `values.yaml`.
...
Using Celery requires configuring the CeleryK8sRunLauncher and CeleryK8sExecutor ([See Configuration](/deploying/k8s#advanced-configuration)).
...
Building off the [prior description](/deploying/k8s#dagit), Dagit gracefully handles repository updates and 
errors with no downtime. Dagit is horizontally scalable via `dagit.replicaCount` in `values.yaml`. 

####  Database

Same as [prior description](/deploying/k8s#database).
...
Building off the [prior description](/deploying/k8s#run-worker), the main difference in this deployment is 
...
Building off the [prior description](/deploying/k8s#user-code-deployment), users create a separate 
...
####  Run Launcher
The [Run Launcher](/overview/run-launchers-executors/run-launcher) is responsible for 
allocating the computational resources for run execution.

<PyObject module="dagster_k8s" object="K8sRunLauncher" /> [Default]: The K8sRunLauncher
is used in the [Default Deployment](/deploying/k8s#default-deployment) example. It creates a new K8s Job per pipeline run.

<PyObject module="dagster_celery_k8s" object="CeleryK8sRunLauncher" />: 
The CeleryK8sRunLauncher is used in the [Advanced Deployment](/deploying/k8s#advanced-deployment) example. It creates a new K8s Job per step 
for step-level isolation and handling concurrency limits.

####  Executor
The Executor traverses the execution plan, enforcing execution order, conditional execution, handling retries, etc.

With the K8sRunLauncher, users can utilize the <PyObject module="dagster" object="in_process_executor" />, the 
the <PyObject module="dagster" object="multiprocess_executor" />, 
the <PyObject module="dagster_celery" object="celery_executor" />, 
<PyObject module="dagster_celery_docker" object="celery_docker_executor" />, 
or a custom Executor.

With the <PyObject module="dagster_celery_k8s" object="CeleryK8sRunLauncher" />, user can only use the 
<PyObject module="dagster_celery_k8s" object="celery_k8s_job_executor" />. 
As seen in the [Advanced Deployment](/deploying/k8s#advanced-deployment) example, this creates a Celery task for each Step, allowing for step-level isolation and concurrency control.
...
and managing the queue of runs. This is used in both the [Default Deployment](/deploying/k8s#default-deployment) and 
[Advanced Deployment](/deploying/k8s#advanced-deployment) examples and is our recommended approach.

K8s CronJob scheduler: Uses [Kubernetes CronJob](https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/) directly. 
In order to enable the Dagster K8s CronJob Scheduler, set `scheduler.k8sEnabled` to `true` in the `values.yaml`.
