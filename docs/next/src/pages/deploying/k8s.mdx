...
* [Default Quickstart →](/deploying/k8s#default-quickstart)
...
module="dagster_celery_k8s" object="CeleryK8sRunLauncher" /> and <PyObject module="dagster_celery_k8s" object="celery_k8s_job_executor" /> ([See Configuration](/deploying/k8s#advanced-configuration)).
...
Building off the [prior description](/deploying/k8s#dagit), but instead, it is configured with the <PyObject module="dagster_celery_k8s" object="CeleryK8sRunLauncher" />.

####  Database

Same as the [prior description](/deploying/k8s#database).
...
Same as the [prior description](/deploying/k8s#user-code-deployment).
...
The `K8sRunLauncher` is used in the [Default Deployment](/deploying/k8s#default-deployment) example. It creates a new Kubernetes Job per pipeline run.

<PyObject module="dagster_celery_k8s" object="CeleryK8sRunLauncher" />:

The `CeleryK8sRunLauncher` is used in the [Advanced Deployment](/deploying/k8s#advanced-deployment) example. It creates a new Kubernetes Job per step 
