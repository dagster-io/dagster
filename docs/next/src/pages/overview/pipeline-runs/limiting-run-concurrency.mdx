...
By default, runs are removed from the queue in the same order as they are submitted. Using the tag
`dagster/priority` (either in Dagit or on the pipeline definition), you can specify an integer value
...
runs have equal priority, their submission order is maintained.
