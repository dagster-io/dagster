...
* Restrict solids to pure business logic, and delegate the materialization of assets to AssetStores.
...
## Recording Asset Materializations in Asset Stores

If an AssetStore is responsible for mutating or creating an asset, then we can yield an <PyObject module="dagster" object="AssetMaterialization" /> event from its `set_asset` method.
