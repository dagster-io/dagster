...
<PyObject module="dagster" object="RootInputManager" />s are user-provided
objects that are responsible for loading the inputs of solids whose inputs are
not connected to an upstream solid's output.
