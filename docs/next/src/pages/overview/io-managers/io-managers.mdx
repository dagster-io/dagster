...
IOManagers are resources that specify both how to load inputs and handle outputs. You can also define resources that only load inputs or only handle outputs.
...
To accomplish this, you can define an IO manager whose `load_input` method supports both behaviors: loading based on the upstream output or loading based on config.
