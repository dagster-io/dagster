...
* PySpark DataFrames cannot be pickled, which means that [IO Managers](/overview/io-managers/io-managers) like the `fs_io_manager` won't work for them.
...
This example assumes that all the outputs within the pipeline will be PySpark DataFrames and stored in the same way.  To learn how to use different IO managers for different outputs within the same pipeline, take a look at the [IO Manager overview](/overview/io-managers/io-managers).
