...
* PySpark DataFrames cannot be pickled, which means that [Object Managers](/overview/object-managers/object-managers) like the `fs_object_manager` won't work for them.
...
This example assumes that all the outputs within the pipeline will be PySpark DataFrames and stored in the same way.  To learn how to use different object managers for different outputs within the same pipeline, take a look at the [Object Manager overview](/overview/object-managers/object-managers).
