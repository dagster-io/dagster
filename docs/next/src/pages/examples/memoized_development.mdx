...
<ExampleReferenceLink filePath="examples/memoized_development" />

This example is intended to describe how to use Dagster's versioning and memoization features.
As of release `0.10.0`, Dagster has functionality for tagging a solid or resource with the *version*
...
```python literalinclude caption=repo.py showLines startLine=0 emphasize-lines=13
file:/memoized_development/memoized_development/repo.py
startAfter:start_mypipeline
endBefore:end_mypipeline
```
...
```python literalinclude caption=solid_utils.py showLines startLine=0
file:/memoized_development/memoized_development/solids/solid_utils.py
```

And here is the definition of each solid, using file-hashing to compute the version:
```python literalinclude caption=dog.py showLines startLine=0
file:/memoized_development/memoized_development/solids/dog.py
```

```python literalinclude caption=tree.py showLines startLine=0
file:/memoized_development/memoized_development/solids/tree.py
```

```python literalinclude caption=sentence.py showLines startLine=0
file:/memoized_development/memoized_development/solids/sentence.py
```
...
```yaml literalinclude caption=california_config.yaml
file:/memoized_development/config/california_config.yaml
```
...
CLI to execute the pipeline. __Note that memoized execution is not yet supported from Dagit.__
...
```yaml literalinclude caption=maryland_config.yaml
file:/memoized_development/config/maryland_config.yaml
```
...
[![Open in Gitpod](https://gitpod.io/button/open-in-gitpod.svg)](https://gitpod.io/#EXAMPLE=memoized_development/https://github.com/dagster-io/dagster)

## Download

```
curl https://codeload.github.com/dagster-io/dagster/tar.gz/master | tar -xz --strip=2 dagster-master/examples/memoized_development
cd memoized_development
```
