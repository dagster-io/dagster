...
We first need to define the `Scheduler` on our <PyObject module="dagster" object="DagsterInstance" />.
For now, the only implemented scheduler is <PyObject module="dagster_cron" object="SystemCronScheduler"/>,
but this is pluggable (and you can write your own). To use the scheduler, add the following lines
...
```bash
dagster schedule up --preview -f scheduler.py
```

We see the preview like below:
...
```bash
dagster schedule up -f scheduler.py
```
...
```bash
dagster schedule start -f scheduler.py good_morning_schedule
```

It tells us:

```bash
Started schedule good_morning_schedule
```
...
We will find the schedule has been written to a script under the directory `/dagster_home/schedules/scripts/`.

```bash
45 6 * * * /Users/a16502/dagster_home/schedules/scripts/8c3aa045e25b5ec48557d5340777bc7db2e4f84e.sh
> /Users/a16502/dagster_home/schedules/logs/8c3aa045e25b5ec48557d5340777bc7db2e4f84e/scheduler.log
2>&1 # dagster-schedule: 8c3aa045e25b5ec48557d5340777bc7db2e4f84e
```
...
Clicking on the `good_morning_schedule` will lead us to the Schedules view.
We can see the schedule is turned on for 6:45 AM.
