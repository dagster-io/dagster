...
able to parametrize our solid logic. Appropriately parameterized solids are more testable, and also
more reusable. Consider the following more generic solid:
...
it can have [a lot of sections](/apidocs/execution####config_schema), but we'll only use one of
...
we won't know about the conditionality in the pipeline until runtime, when one of the outputs is not
yielded by `split_cereal`.
