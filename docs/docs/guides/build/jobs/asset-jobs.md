...
Jobs are loaded automatically with [`dg`](/api/dg) and there is no need to explicity define a [`Definitions`](/api/dagster/definitions) object for them. If you include schedules or sensors, the [project](/deployment/code-locations) will automatically include jobs that those schedules or sensors target.
