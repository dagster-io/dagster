...
| `container_context.yaml` | Allows you to set environment variables or resource limits for the code server or run containers used in your infrastructure (Kubernetes, ECS, Docker). | Not applicable | Dagster+ Hybrid only | [`dg plus deploy configure` CLI command](/api/clis/dg-cli/dg-plus#configure) |
