...
Getting external code to send information back to Dagster with Dagster Pipes requires adding a few lines of code:
...
- An instance of the Dagster Pipes context from <PyObject section="libraries" integration="pipes" module="dagster_pipes" object="PipesContext.get" />. You can access information like `partition_key` and `asset_key` with this context object. Refer to the [the API documentation](/integrations/libraries/pipes/dagster-pipes#dagster_pipes.PipesContext) for more information.
...
If your asset has data quality checks defined, you can report to Dagster that an asset check has been performed by using <PyObject section="libraries" integration="pipes" module="dagster_pipes" object="PipesContext" method="report_asset_check" />:
...
- Learn about other capabilities of executing external code in subprocess using Dagster Pipes in the [Subprocess reference](/guides/build/external-pipelines/using-dagster-pipes/reference)
