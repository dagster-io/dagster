...
Your Azure ML job will require an Azure ML environment that contains the `dagster-pipes` library. Since the Azure ML job will be communicating with the Dagster orchestrator through Azure Blob Storage, we will also need to install the `azure-identity` and `azure-storage-blob` Python libraries.

In your Azure ML dashboard, choose "Add a Custom Environment", select the environment source you want to use (such as `sklearn-1.5:33`), and edit the list of Python packages to install.
