...
| **Asset health status** | `status: "DEGRADED"` | Selects assets with a given [asset health status](/guides/observe/asset-health-status). Options are `"HEALTHY"`, `"WARNING"`, `"DEGRADED"`, `"UNKNOWN"`, or `"NOT_APPLICABLE"` | Dagster+ only (requires [Observability early access](/guides/observe)) |
