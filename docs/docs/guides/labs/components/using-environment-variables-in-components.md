...
To demonstrate how to configure components for different environments, we'll set up a basic ELT pipeline using Sling in an empty Dagster components project:
...
## Utilizing environment variables in a component

Next, we will configure a Sling connection which will sync a local CSV file to a Snowflake database, whose credentials will be provided via environment variables.
...
## Configuring environment variables for components in Dagster Plus

If you are using Dagster Plus, you can also use the `dg` CLI to push environment variables to your deployment.

First, you will need to authenticate with Dagster Plus:
...
To make it so that the rest of your team can run your project locally, you can run the following command to push each Snowflake credential to the `local` scope:
...
Finally, you can configure a different set of production environment variables for your Dagster Plus deployment:
