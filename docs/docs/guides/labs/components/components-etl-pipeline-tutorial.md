...
To confirm that the `dagster.components.sling_replication` component type is now available, run the `dg list component-type` command again:
...
We need to update the configuration of the `dagster.components.dbt_project` component to match the keys generated by the Sling component. Update `components/jdbt/component.yaml` with the configuration below:
