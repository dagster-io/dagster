...
Once your dbt project is available, you can write a function that runs your dbt commands using the <PyObject section="libraries" integration="dbt" object="dbt_assets" module="dagster_dbt"/> decorator and <PyObject section="libraries" integration="dbt" object="DbtCliResource" module="dagster_dbt"/>. Most dbt CLI commands and flags are supported - to learn more about using `@dbt_assets`, check out the [dagster-dbt](/integrations/libraries/dbt) and [reference](/integrations/libraries/dbt/reference).
