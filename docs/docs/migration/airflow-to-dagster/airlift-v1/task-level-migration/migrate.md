...
Previously, we completed the ["observe" stage](/migration/airflow-to-dagster/airlift-v1/task-level-migration/observe) of the Airflow migration process by encoding the assets that are produced by each task. We also introduced partitioning to those assets.
