...
- A Dagster+ account
- An existing Branch Deployments setup that uses [GitHub actions](/deployment/dagster-plus/deploying-code/ci-cd/ci-cd-in-hybrid#github) or [Gitlab CI/CD](/deployment/dagster-plus/deploying-code/ci-cd/ci-cd-in-hybrid#non-github). Your setup should contain a Dagster project set up for branch deployments containing:
