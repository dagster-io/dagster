...
By default, Dagster+ ingests [structured event logs and compute logs](/guides/monitor/logging/index.md#log-types) from runs and surfaces error messages from [projects](/deployment/code-locations) in the UI.
...
By default, Dagster+ surfaces error messages from your projects in the UI, including when runs fail, sensors or schedules throw an exception, or projects fail to load. You can mask these error messages in the case that their contents are sensitive.
...
Once set, error messages from your projects will be masked in the UI. A unique error ID will be generated, which you can use to look up the error message in your own logs. This error ID will appear in place of the error message in UI dialogs or in a run's event logs:
