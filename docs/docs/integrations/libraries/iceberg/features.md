...
| Schema evolution         | ✅        | https://py.iceberg.apache.org/api/#schema-evolution                                                   | More complicated than (for example) Delta Lake, since updates require diffing input table with existing Iceberg table. This is implemented by checking the schema of incoming data, dropping any columns that no longer exist in the data schema, and then using the `union_by_name()` method to merge the current schema with the table schema. Current implementation has a chance of creating a race condition when, for example, partition A tries to write to a table that has not yet processed a schema update. Should be covered by retrying when writing. |
...
| Upsert                   | ✅        | https://py.iceberg.apache.org/api/#upsert                                                             | Update existing rows and insert new rows in a single operation. Configure with `write_mode: "upsert"` and `upsert_options` in asset metadata.                                                                                                                                                                                                                                                                                                                                                                                                    |
