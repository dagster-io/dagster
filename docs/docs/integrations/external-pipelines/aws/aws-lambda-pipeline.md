...
This article focuses on using an out-of-the-box Amazon Web Services (AWS) Lambda resource. For further customization, use the [`open_pipes_session`](/integrations/external-pipelines/dagster-pipes-details-and-customization) instead.

:::

This article covers how to use [Dagster Pipes](/integrations/external-pipelines) with Dagster's AWS Lambda integration to invoke a Lambda function and execute external code.
...
- Imports [`PipesMappingParamsLoader`](/integrations/libraries/pipes/dagster-pipes#params-loaders) and <PyObject section="libraries" integration="pipes" object="open_dagster_pipes" module="dagster_pipes" /> from `dagster_pipes`
...
At this point you can execute the rest of your AWS Lambda code as normal, invoking various <PyObject section="libraries" integration="pipes" object="PipesContext" module="dagster_pipes" /> APIs as needed.
...
- Specified a resource for the asset to use, <PyObject section="libraries" integration="aws" module="dagster_aws" object="pipes.PipesLambdaClient" />, which is a pre-built Dagster resource that allows you to quickly get Pipes working with AWS Lambda.
...
Next, you'll add the asset and AWS Lambda resource to your project's code location via the <PyObject section="definitions" module="dagster" object="Definitions" /> object. This makes the resource available to [other Dagster definitions in the project](/guides/build/projects).
