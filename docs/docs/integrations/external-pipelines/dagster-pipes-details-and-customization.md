...
At present the only official Dagster Pipes integration library is Python's [`dagster-pipes`](/integrations/libraries/pipes/dagster-pipes), available on [PyPI](https://pypi.org/project/dagster-pipes). The library has no dependencies and fits in a [single file](https://github.com/dagster-io/dagster/blob/master/python_modules/dagster-pipes/dagster_pipes/__init__.py), so it may also be trivially vendored.
