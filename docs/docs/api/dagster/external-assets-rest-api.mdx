...
Authentication is required only if requests are being made against a [Dagster+ instance](/dagster-plus/). To authenticate, provide a valid [Dagster+ user token](/deployment/dagster-plus/management/tokens/user-tokens) using the `Dagster-Cloud-Api-Token` header:
