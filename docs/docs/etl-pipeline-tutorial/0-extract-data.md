...
When building assets, the first step is to scaffold the assets file with the [`dg scaffold` command](/api/clis/dg-cli#dg-scaffold). The `dg` CLI provides a number of commands to help structure and navigate Dagster projects. For more information, see the [`dg` CLI documentation](/api/clis/dg-cli):
...
We can use `dg` to ensure that everything we define in our module is loading correctly and that our project is deployable. Here we can use the [`dg check defs`](/api/clis/dg-cli#dg-check) command:
...
You can also materialize assets from the command line with [`dg` launch](/api/clis/dg-cli#dg-launch). You will need to pass an asset selection -- in this case, `*` selects all assets:
