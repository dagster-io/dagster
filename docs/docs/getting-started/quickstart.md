...
Use the [`dg scaffold defs`](/api/clis/dg-cli#dg-scaffold) command to generate an assets file on the command line:
...
At this point, you can list the Dagster definitions in your project with [`dg list defs`](/api/clis/dg-cli#dg-list). You should see the asset you just created:

<CliInvocationExample path="docs_snippets/docs_snippets/getting-started/quickstart/dg_list_defs.txt" />

You can also load and validate your Dagster definitions with [`dg check defs`](/api/clis/dg-cli#dg-check):
...
You can also run the pipeline by using the [`dg launch --assets`](/api/clis/dg-cli#dg-launch) command and passing an [asset selection](/guides/build/assets/asset-selection-syntax):
