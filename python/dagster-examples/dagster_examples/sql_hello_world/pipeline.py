...
def define_pipeline():
    sum_table_solid = solidic_sql.create_sql_solid(
        name='sum_table',
        inputs=[solidic_sql.create_table_input('num_table')],
        sql_text='SELECT num1, num2, num1 + num2 as sum FROM ({num_table})',
    )

    sum_sq_table_solid = solidic_sql.create_sql_solid(
        name='sum_sq_table',
        inputs=[solidic_sql.create_table_input_dependency(sum_table_solid)],
