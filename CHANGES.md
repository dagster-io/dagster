...
- Dagster’s metadata API has undergone a signficant overhaul. Changes include:
    - Name changes (old names are deprecated, but will function as aliases for the new names until 0.15.0):
    ...
    - Arbitrary metadata on `In`/`InputDefinition` and `Out`/`OutputDefinition` is deprecated. In 0.15.0, metadata passed for these classes will need to be resolvable to `MetadataValue` (i.e. be like metadata everywhere else in Dagster).
...
- Dagit
    ...
    - Dagit has moved from using `flask` to `starlette`. Custom run coordinators that used `flask` to read request headers can move to calling `get_request_header` on the `SubmitRunContext`.
...
- Ellipsis (“...”) is now an invalid substring of a partition key. This is because Dagit uses an ellipsis to display partition ranges.
...
- The `multiprocess` executor now supports choosing between `spawn` or `forkserver` for how its subprocesses are created. The default behavior has been changed to `forkserver` which attempts to intelligently preload modules to reduce the per-op overhead.
